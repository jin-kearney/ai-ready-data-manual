...
>
> **산출 연계:** 이 루브릭으로 매긴 우선순위는 정성 정본 [전체 목차/01 Tech Stack 비교 (솔루션×주제)](../전체%20목차/01%20Tech%20Stack%20비교%20(솔루션×주제).md)의 ✓/△/✗ 비교를 정량 근거로 뒷받침한다.
...
- [5. 데이터 준비 기능 요건 (주제별 게이트 질문 · 신설)](#sec5)
...
| **컴포넌트별 기능 요건(게이트 질문)** | **신설** | AXC는 컴포넌트별 기능 요건을 보안 필터에 게이트 질문으로 넣었다(예: "커스텀 정책 설정 가능?"). 같은 방식으로 **데이터 준비 기능 요건**을 주제별 게이트로 신설(§5) |
| AXC 실제 Player 점수·Black/White List | **참고만** | 상속 안 함, 대조용(§8) |

요지: **바꾸는 건 딱 세 군데다 — ① 보안 게이트의 LLM/에이전트 전용 2개 항목 ② 비용 시나리오의 토큰 파라미터 ③ 성능 P2 프로토콜 목록 확장.** 나머지 33점 체계는 AXC 그대로다. 데이터 준비 *기능* 적합성은 AXC가 쓰던 방식(게이트 질문)을 그대로 빌려 §5로 신설한다.
...
flowchart TB
    POOL[우리 Tech Stack Player 목록<br/>주제별 후보]:::src --> GATE{보안 게이트 + 데이터준비 기능 요건<br/>AXC 필터 방식 · 통과/탈락}:::gate
    ...
    CUT -->|이상| RANK[주제별 우선순위 순위<br/>+ 환경별 1순위]:::gate
...
- 통제수준(High/Mid/Low)은 AXC처럼 주제별로 참고한다(High 주제는 게이트·기능 요건을 더 엄격히).
...
<a id="sec5"></a>
## 5. 데이터 준비 기능 요건 (주제별 게이트 질문 · 신설)

AXC는 컴포넌트별 *기능 요건*을 보안 필터에 **게이트 질문**으로 넣었다(예: AI Security엔 "커스텀 정책 설정 가능?", Observability엔 "이상 탐지 있는가?"). **같은 방식**으로, 데이터 준비 기능 요건을 주제별 게이트 질문으로 신설한다 — 필수 기능이 없으면 그 주제에서 후보 탈락.

이렇게 하면 **33점 점수는 AXC와 동일하게 유지**(성능은 컴포넌트 무관)하면서, 데이터 준비 적합성은 AXC가 쓰던 게이트 자리에서 본다.

| 주제 | 데이터 준비 필수 기능 (게이트 질문 — "예/아니오") |
|---|---|
| **A-1 카탈로그** | 우리 핵심 원천(SAP·MES·QMS·LIMS·SharePoint)에 커넥터를 제공하는가? · 컬럼 단위 계통을 자동 수집하는가? |
| **A-2 메타데이터** | 기술·비즈니스 메타를 함께 관리하는가? · 자동 분류/태깅이 되는가? |
| **A-3 용어집** | 네이티브 비즈니스 용어집·동의어 매핑을 제공하는가? · 한국어를 지원하는가? |
| **B-1 전처리/파싱** | 복잡한 표 구조를 보존하는가? · 온프렘/로컬 실행이 가능한가? · 한국어 OCR을 지원하는가? |
| **B-2 라벨링/주석** | AI 1차 라벨(pre-label)을 지원하는가? · 라벨 합의·오류 탐지가 되는가? · 온프렘이 가능한가? |
| **B-3 온톨로지/그래프** | 우리 용처(LPG/RDF)에 맞는 그래프 모델인가? · 다중 홉 추론·GraphRAG를 지원하는가? |
| **C-1 데이터 Observability** | 신선도·완전성·분포 이상을 자동 탐지하는가? · 계통과 연계되는가? |
| **C-2 데이터 품질 게이트** | 품질 규칙으로 파이프라인을 차단(게이트)할 수 있는가? · 예외 승인 흐름이 있는가? |
| **C-3 계통(Lineage)** | 컬럼 단위 계통인가? · OpenLineage 등 자동 수집을 지원하는가? |
| **D-1 Physical/IoT** | OT 프로토콜(OPC UA·Modbus·MQTT)을 수집하는가? · 산업 historian과 연계되는가? |
| **D-2 Tool 연계** | OpenAPI·MCP 표준을 준수하는가?(별첨 C) · Tool 명세 레지스트리·버전 관리가 되는가? |
| **D-3 Prompt 자산화** | 프롬프트 버전·메타데이터 관리가 되는가? · 평가와 연계되는가? |
| **E-1 데이터 Product화** | 데이터 상품 정의·발행을 지원하는가? · 복사 없는 공유가 되는가? |
| **E-2 합성데이터** | 분포·상관을 보존하는가? · 재식별 위험을 검증하는가? · 제조 도메인에 적용 가능한가? |
| **E-3 AI 평가데이터** | 평가셋 구축·버전·골든셋 관리가 되는가? · 회귀 평가를 지원하는가? |
| **E-4 Feedback Loop** | 추론 로그·피드백을 수집하는가? · 재학습 데이터로 환류되는가? |
| **F-1 DataOps** | 오케스트레이션·스케줄·CI/CD를 지원하는가? · 계통과 자동 연동되는가? |
| **F-2 생애주기** | 티어링·아카이브·만료(TTL)를 자동화하는가? · 보존정책을 적용하는가? |
| **F-3 디지털화(OCR·STT)** | 한글 손글씨·도면을 처리하는가? · 다국어 STT를 지원하는가? |
| **F-4 권한·비식별** | 행/열 접근통제가 되는가? · PII 탐지·마스킹이 되는가? · 재식별 점검을 지원하는가? |
...
- **우선순위 = 게이트 통과 + 33점 내림차순.** 동률은 비용 → 성능 순.
...
```
주제 | Player | 게이트(보안+기능) | 성능(30) | 비용(3) | 종합(33) | 20점 컷 | 순위 | 환경별 1순위 | AXC 대조
```
...
1. **Player 목록 확정** — 주제별 후보를 모은다(이 루브릭의 입력).
2. **게이트** — §2 보안 + §5 데이터 준비 기능 요건. 하나라도 미충족이면 탈락. OSS는 CVSS 점검.
3. **성능 채점** — §3 표로 10개 지표(OSS는 5개 ×2).
4. **비용 채점** — §4 Medium 시나리오(데이터량 파라미터) 적용 → TCO → 3점.
5. **종합·컷·정렬** — §6. 33점 합산, 20점 컷, 우선순위·환경별 1순위.
6. **AXC 대조** — §8. 겹치는 Player가 있으면 AXC 점수와 나란히 점검.
7. **정본 반영** — 사용자 승인 시 2층 정본에 정량 컬럼 추가, 별도 커밋(CLAUDE.md 반영 게이트 준수).

> 이 절차는 주제 가이드 생산([`ai-ready-manual-guide`](../가이드%20작성) 스킬)과 별개로 돌리는 평가 패스다. 가이드의 솔루션 섹션이 §5 기능 요건의 원천이 된다.
...
- **Layer 2(Data)는 AXC 대조 자체가 없다:** AXC는 카탈로그·계통 등 데이터 계층 Player를 평가하지 않았다 — 이 주제는 대조 없이 우리 루브릭만으로 매긴다.
