...
>
> **기본 원칙 — AXC 준용, 비용은 제외:** AXC의 채점 체계(보안 게이트 + 성능 30 + 비용 3, 20점 컷) 중 **보안 게이트와 성능 30점을 그대로 쓰고, 비용(3점)은 두지 않는다**(고객 결정 — 우선순위는 성능 30점만으로 매기고, 20점 컷은 성능 30점 기준으로 유지). 그 밖에 AXC는 *AI 에이전트·LLM* 평가 기준이므로 **데이터 준비 관점에서 안 맞는 항목만 외과적으로 바꾼다.** 무엇을 그대로 두고 무엇을 바꾸는지는 [§0](#sec0)에 한 표로 정리한다.
>
> **무엇을 가져오고 무엇을 안 가져오나:** 평가의 *기준(축·세부지표·배점)*은 가져온다. AXC가 매긴 *실제 점수·Black/White/Recommendation List*는 우리 판정으로 상속하지 않고 **대조(캘리브레이션)용**으로만 본다([§7](#sec8)).
>
...
- [4. 데이터 준비 기능 요건 (솔루션 단위별 게이트 질문 · 신설)](#sec5)
- [5. 종합 점수·우선순위](#sec6)
- [6. 적용 절차 — Player가 나오면 어떻게 돌리나](#sec7)
- [7. AXC 실제 결과는 참고만 (캘리브레이션)](#sec8)
- [8. 주의·리스크](#sec9)
- [참고자료](#refs)
...
|---|---|---|
| **채점 체계**(보안 게이트 → 성능 30, 20점 컷) | **수정** | AXC 33점(성능 30 + 비용 3) 중 **비용(3점)을 제거**(고객 결정). 성능 30점만·20점 컷은 성능 30점 기준으로 유지 |
...
| **비용 6항목·TCO 3년 산식·정규화** | **제거** | 비용 축을 두지 않음(고객 결정) |
...
| 별첨 C 표준 프로토콜(OpenAPI·MCP·AD 인증) | **그대로** | §3 P2·§4 D-2에 반영 |
| 별첨 D 환경별 선정 규칙(AWS/Azure/온프렘) | **그대로** | §5. 단 우리는 온프렘 1순위를 반드시 채움 |
| 별첨 E OSS 소스 CVSS(BlackDuck) | **그대로** | §2 OSS 게이트 |
| **컴포넌트별 기능 요건(게이트 질문)** | **신설** | AXC는 컴포넌트별 기능 요건을 보안 필터에 게이트 질문으로 넣었다(예: "커스텀 정책 설정 가능?"). 같은 방식으로 **데이터 준비 기능 요건**을 **03의 12개 솔루션 단위별** 게이트로 신설(§4) |
| **평가 단위(채점 대상)** | **수정** | AXC는 컴포넌트(Component) 단위로 평가. 우리는 03이 묶은 **솔루션 단위**(통합 플랫폼 1개가 여러 주제 커버)로 본다 — §4·§5·§6 |
| AXC 실제 Player 점수·Black/White List | **참고만** | 상속 안 함, 대조용(§7) |

요지: **비용(3점)을 통째로 뺐고**(성능 30점만·20점 컷 유지), 그 밖에 바꾸는 건 두 군데 — **① 보안 게이트의 LLM/에이전트 전용 2개 항목 ② 성능 P2 프로토콜 목록 확장.** 나머지 보안 게이트·성능 30점 체계는 AXC 그대로다. 추가로 **평가를 도는 단위를 03의 12개 솔루션 단위로 맞췄다**(개별 주제가 아니라) — 데이터 준비 *기능* 적합성은 AXC가 쓰던 방식(게이트 질문)을 그대로 빌려 **솔루션 단위별로** §5에 신설한다.
...
flowchart TB
    ...
    GATE -->|미충족 / OSS CVSS≥7.0 / 필수기능 없음| OUT[Black-List<br/>사용 제한]:::n
    GATE -->|통과| PERF[성능 30점<br/>3 Pillar × 10 지표]:::cat

    PERF --> CUT{성능 20점 이상?}:::gate
    CUT -->|미만| LOW[우선순위 후순위]:::n
    CUT -->|이상| RANK[White-List · 추천<br/>단위별 순위 + 환경별 1순위]:::gate
    RANK --> CANON[03 솔루션 평가 판 점수 칸 채움<br/>+ 2층 정본 01 Tech Stack 강화]:::loop

    AXC[AXC 실제 점수·List<br/>참고 · 대조]:::cross -.캘리브레이션.-> PERF
...
- 순서는 AXC를 따른다 — **게이트(통과/탈락) → 성능 30점 → 20점 컷 → 우선순위.** (AXC 33점 체계에서 비용 3점을 뺀 변형이다.)
...
<a id="sec5"></a>
## 4. 데이터 준비 기능 요건 (솔루션 단위별 게이트 질문 · 신설)
...
이렇게 하면 **성능 30점 점수는 그대로 유지**(성능은 컴포넌트 무관)하면서, 데이터 준비 적합성은 AXC가 쓰던 게이트 자리에서 03과 같은 솔루션 축으로 본다.
...
> **묶이지 않은 주제:** 03은 RFP 5영역에 직접 대응하지 않는 **E-1 데이터 Product화·F-1 DataOps·F-2 생애주기**를 솔루션 단위에서 제외했다. 이 평가 판에도 해당 단위를 두지 않는다 — 나중에 이 주제에 별도 Player 목록이 생기면 같은 성능 30점 루브릭으로 추가 평가한다.
>
> 기능의 *정도 차*(예: 커넥터 5종 중 3종 vs 5종)는 2층 정본의 정성 비교(✓/△/✗)로 남긴다 — 성능 30점 점수에는 섞지 않는다(성능 컴포넌트 무관 원칙 유지).
>
...
AXC의 종합·선정 방식을 따르되, 비용 축은 두지 않는다.

- **종합 점수 = 성능 30점 만점.** (AXC 33점 체계에서 비용 3점을 뺀 변형)
- **20점 컷.** 성능 30점 중 20점 미만은 우선순위 후순위.
- **우선순위 = 솔루션 단위 안에서 게이트 통과 + 성능 30점 내림차순.** 동률은 상위 기둥(P1 제품 검증성 → P3 운영 안정성) 점수, 그래도 같으면 2층 정본의 정성 비교로 가른다. 순위는 솔루션 단위별로 따로 매긴다(카탈로그 플랫폼 후보끼리, 품질·관측 후보끼리).
- **환경별 1순위(별첨 D 규칙):** 클라우드는 자체 Managed 우선, 없으면 연동되는 OSS 최고점. 온프렘은 OSS 최고점(하드웨어 종속·국내 선도사례는 예외). 우리는 **온프렘 1순위를 반드시 채운다.**
- **산출을 AXC 리스트 용어로 정리한다** — 보안 탈락 = **Black-List(사용 제한)**, 성능 20점 이상 통과 = **White-List · 추천(Recommendation)**. 여기에 **솔루션 단위별 우선순위 + 환경별 1순위**를 더한다. (그룹 전체에 강제하는 공식 List 확정은 지주 권한.)

출력 형식 — [03](03%20제안%20솔루션%20구성%20(영역별%20솔루션%20단위).md) §3의 빈 점수 칸과 같은 행으로 채운다:

```
솔루션 단위 | Player | OSS/SaaS·온프렘 | 게이트(보안+기능) | 성능(30) | 20점 컷 | 단위 내 순위 | 환경별 1순위 | AXC 대조
```

---

<a id="sec7"></a>
## 6. 적용 절차 — Player가 나오면 어떻게 돌리나

1. **Player 목록 확정** — [03](03%20제안%20솔루션%20구성%20(영역별%20솔루션%20단위).md) §3에 이미 정리된 12개 솔루션 단위별 후보 표가 입력이다(빈 점수 칸을 이 패스로 채운다).
2. **게이트** — §2 보안 + §4 솔루션 단위별 데이터 준비 기능 요건. 하나라도 미충족이면 그 단위에서 탈락. OSS는 CVSS 점검.
3. **성능 채점** — §3 표로 10개 지표(OSS는 5개 ×2).
4. **컷·정렬** — §5. 성능 30점, 20점 컷, 솔루션 단위 안에서 순위·환경별 1순위.
5. **AXC 대조** — §7. 겹치는 Player가 있으면 AXC 점수와 나란히 점검.
6. **판/정본 반영** — 사용자 승인 시 03 §3 점수 칸을 채우고, 2층 정본 01에 정량 컬럼 추가, 별도 커밋(CLAUDE.md 반영 게이트 준수).

> 이 절차는 주제 가이드 생산([`ai-ready-manual-guide`](../가이드%20작성) 스킬)과 별개로 돌리는 평가 패스다. 가이드의 솔루션 섹션이 §4 기능 요건의 원천이 되고, 03이 솔루션 단위·후보·온프렘 여부를 제공한다.

---

<a id="sec8"></a>
## 7. AXC 실제 결과는 참고만 (캘리브레이션)
...
- **같은 이름 다른 대상.** C-1 Observability(데이터 vs AI 런타임), E-3 평가(데이터셋 vs 플랫폼)는 단어만 겹친다 — §4 게이트에서 대상 구분.
- **점수 과신 금지.** 우선순위는 의사결정 보조다. 두산 원천 연결은 PoC로만 확증된다.
- **권한 경계.** 평가 결과는 Black-List·White-List·추천으로 정리하되, 그룹 전체에 강제하는 공식 List 확정은 지주 권한이다.
...
- **AXC 평가결과서** — [01. 산출물_Player 평가결과서_v260602.xlsx](AXC%20참고자료/01.%20산출물_Player%20평가결과서_v260602.xlsx) (준용: 보안 항목·별첨 A 성능·별첨 C 프로토콜·별첨 D 선정·별첨 E OSS. 별첨 B 비용은 미사용)
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.6 | 2026-07-01 | **산출을 AXC 리스트 용어로 정리** — 보안 탈락=Black-List(사용 제한), 성능 20점 이상 통과=White-List·추천. §1 구조도·§5 종합·§8 주의 반영(공식 List 확정은 지주 권한 유지). |
| v0.5 | 2026-07-01 | **비용(3점) 축 제거 — 평가 = 보안 게이트 + 성능 30점, 20점 컷은 성능 30점 기준으로 유지**(고객 결정). §4 비용 평가 섹션 삭제, §5~§9를 §4~§8로 재번호(앵커 id 유지), §0 판정표·§1 구조도·§5 종합·§6 절차·출력형식에서 비용 제거. AXC 33점 체계에서 비용만 뺀 변형임을 명시. |
