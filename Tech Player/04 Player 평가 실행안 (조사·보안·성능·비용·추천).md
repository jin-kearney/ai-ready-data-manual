...
### (14개 카테고리 후보 전수 조사 → 보안·성능 평가 → 추천, 그림으로 보기)

> **이 문서 = "어떻게 돌리나"를 그림으로 보여주는 실행안.** 평가의 *규칙·배점*은 [Tech 솔루션 평가 기획안](Tech%20솔루션%20평가%20기획안%20(AXC%20방법론%20준용).md)이, 평가할 *후보 목록·판*은 [03 제안 솔루션 구성](03%20제안%20솔루션%20구성%20(영역별%20솔루션%20단위).md)이 정한다. 이 04 문서는 그 둘을 **실제로 돌리는 흐름 — 후보를 전수 조사하고, 보안 게이트 → 성능 30점으로 걸러, 카테고리별로 추천하기까지 —** 를 그림 중심으로 정리한다. 특히 **성능을 어떻게 매기는지**를 자세히 그린다.
>
...
- [3. 2단계 — 순서대로 평가: 보안 → 성능](#s3)
- [4. 보안 게이트 — 통과 / 탈락](#s4)
- [5. 성능 평가 30점 — 어떻게 매기나 (핵심)](#s5)
- [6. 종합·추천 산출 — 성능 30점 → 20점 컷 → 순위](#s7)
- [7. 적용 절차 요약](#s8)
- [참고자료](#refs)
...
후보를 모으고(조사), 위에서 아래로 걸러(보안 → 성능), 남는 것을 카테고리 안에서 줄 세운다(추천). 보안 게이트·성능 30점 채점은 AXC를 그대로 따르되, 비용 축은 두지 않는다.
...
![Player 평가 퍼널 — 후보 풀에서 보안·성능을 거쳐 추천까지 좁혀지는 흐름](04%20이미지/funnel-evaluation.svg)
...
| ② 메타·카탈로그·글로서리 | **통합 1개** | 한 플랫폼이 카탈로그·메타·용어집을 함께 제공 | ★ 데이터 카탈로그 플랫폼 |
| ③ 어노테이션·온톨로지 | **2개로 분리** | 라벨링 도구와 그래프DB는 다른 제품군 | 라벨링·어노테이션 / 그래프DB·지식그래프 |
| ④ 품질·계보·감사 | **4개로 분리** | 품질·계보·접근통제·비식별이 각각 다른 제품군 | 품질·옵저버빌리티 / 계보 / 접근통제·보안 / 비식별·마스킹 |
| ⑤ AI·RAG·Agent 연계 | **4개로 분리** | 명세·프롬프트·평가·합성이 각각 다른 도구 | API·MCP 레지스트리 / 프롬프트 관리 / 평가·피드백 / 합성데이터 |

- **② 는 통합의 대표 사례다.** A-1·A-2·A-3(+ 계보·기본 접근통제)을 한 카탈로그 플랫폼이 함께 제공하므로 하나로 묶는다.
- **④ 는 분리의 대표 사례다.** 품질·계보·접근통제·비식별이 각각 다른 제품군이라 4개로 나눈다.
...
| 보안·성능 | (여기서는 빈칸 — 이후 평가 단계에서 채운다) |

> **왜 주제(20개)가 아니라 카테고리(14개)로 조사하나:** 한 통합 플랫폼이 여러 주제를 함께 커버하기 때문이다(예: 카탈로그 플랫폼 하나가 A-1·A-2·A-3 + 계보 + 기본 접근통제). 주제별로 쪼개 채점하면 같은 제품을 여러 번 평가하게 된다. 그래서 03이 묶은 카테고리 축으로 한 번씩만 채점한다.
...
한 후보가 거치는 순서는 정해져 있다. **보안은 통과/탈락 관문**이고, 통과한 후보만 **성능 점수**를 받는다.
...
성능이 후보를 가르는 몸통이다. 제품·컴포넌트 종류와 무관하게 통하는 일반 품질 지표로 **3개 기둥(Pillar) 아래 10개 지표를 구성했으며, 지표마다 최대 3점씩 부여해 총 30점**이다. 데이터 준비 도구에도 그대로 성립한다(AI 색이 있던 곳은 이미 걸러냈다).
...
두 후보를 성능으로 채점하면 아래처럼 나온다. 성능 30점 중 20점 컷을 넘는 후보만 순위에 든다.

![성능 점수 예시 — 후보 A는 25.5점으로 통과, 후보 B는 16점으로 20점 컷 미달](04%20이미지/example-scorecard.svg)
...
<a id="s7"></a>
## 6. 종합·추천 산출 — 성능 30점 → 20점 컷 → 순위

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','fontSize':'13px'}}}%%
flowchart LR
    A["성능 30점"]:::cat --> B{"20점 이상?"}:::gate
    B -->|미만| L["후순위"]:::n
...
```
카테고리 | Player | OSS/SaaS·온프렘 | 보안 게이트 | 성능(30) | 20점 컷 | 단위 내 순위 | 환경별 1순위
```
...
flowchart LR
    ...
    T3 --> T5["4 종합·컷·정렬<br/>(§6)"]:::gate
    T5 --> T6["5 AXC 대조"]:::cross
    T6 --> T7["6 판/정본 반영<br/>(승인 시·별도 커밋)"]:::loop
    classDef n fill:#B7CDE6,stroke:#6B9AD1,color:#10243f;
    classDef src fill:#79C3E8,stroke:#3F9BD4,color:#0b3a52;
    classDef cat fill:#2456A4,color:#fff,stroke:#163a73,stroke-width:2px;
    classDef gate fill:#3F9BD4,stroke:#2456A4,color:#fff;
