...
3. [How — 어떻게 준비·운영하나](#how)
    ...
    - [3.2 평가셋 구성 방법 — 규모·표본·구성비](#scomp)
    - [3.3 채점 방법 — 코드·사람·AI](#s32)
    - [3.4 운영 — 회귀 테스트·실패 사례·버전 관리](#s33)
...
이 오류 유형 태그는 [3.4절](#s33)에서 실패 사례를 평가셋에 추가할 때, 어떤 종류의 오답이 많은지 패턴을 보는 데 쓰인다.
...
4. **채점 연결** — 각 항목을 어떤 방법으로 채점할지 연결한다(코드·사람·AI — [3.3](#s32)).
...
### 3.2 평가셋 구성 방법 — 규모·표본·구성비

"전체의 10%를 골든셋으로" 같은 고정 비율 규칙이 있는 것은 아니다. 규모를 정하는 셈법이 과제 성격에 따라 둘로 갈린다.

- **학습시키는 모델(분류·예측)** — 가진 라벨 데이터를 학습용과 평가용으로 나눈다(train/test 분할). 평가용으로 보통 10~20%를 떼어 둔다(예: 80/10/10). 흔히 듣는 "10%"가 여기서 나온다.
- **LLM·RAG·Agent** — 평가셋이 학습 데이터의 비율이 아니다. 따로 추려서 만들고, 규모는 비율이 아니라 **신뢰도와 커버리지**로 정한다.

골든셋은 "얼마나 많은가"보다 "잘 골랐는가"가 중요하다. 규모·표본·구성비 세 가지로 설계한다.

**① 규모 — 슬라이스당 신뢰도로 정한다.** 점수를 믿으려면 표본이 충분해야 한다. 표본이 적으면 같은 성능도 우연히 높거나 낮게 나온다. 표본 수가 늘수록 점수의 오차 범위가 줄어든다(가장 보수적으로 본 95% 신뢰 기준).

| 평가셋 규모(슬라이스당) | 정확도 추정 오차(대략) |
|---|---|
| 50건 | ±14% |
| 100건 | ±10% |
| 250건 | ±6% |
...
> **슬라이스(slice)란** 따로 떼어 성능을 보고 싶은 묶음이다(결함 유형별·라인별·질문 유형별). 한 과제 안에서도 슬라이스마다 위 표를 적용한다 — 결함 유형 5개를 각각 신뢰 있게 보려면 "유형당 수십~수백 건"이 필요하다. 합격률 80% 기대·오차 ±5%·95% 신뢰면 슬라이스당 약 250건이 기준이다[\[4\]](#ref4). 처음엔 슬라이스당 대표 30건 + 엣지 20건 정도로 시작해, 중요한 과제일수록 키운다.

**② 표본 — 층화 추출(골고루 + 희소 케이스 과대표집).** 운영 데이터를 그냥 무작위로 10% 뽑으면 드문 결함은 거의 안 들어와, 정작 틀리면 안 되는 케이스를 평가하지 못한다. 그래서 결함 유형·라인·질문 유형 같은 축으로 묶고 각 묶음에서 골고루 뽑되, **드물지만 틀리면 치명적인 케이스는 실제 빈도보다 많이** 넣는다[\[17\]](#ref17).

**③ 구성비 — 의도적으로 섞는다.** 정해진 표준 비율은 없고, 네 종류를 의도적으로 섞어 설계한다.

| 케이스 종류 | 무엇인가 | 제조 예시(가상) |
|---|---|---|
| 대표 | 가장 흔한 일상 질문 | "Delamination의 주원인은?" |
| 엣지 | 드물지만 틀리면 치명적 | 두 결함이 동시에 보이는 복합 사례 |
| 부정 | "해당 없음·모름"이 정답 | 근거 문서에 답이 없는 질문(지어내면 안 됨) |
| 애매 | 판단이 갈리는 경계 | Blister와 Delamination 중간 증상 |

평가셋은 학습·few-shot 예시에 쓴 데이터와 겹치지 않게 분리하고([2.1](#s21)), 회귀용으로 고정해 두는 동결 셋과 실패 사례로 자라는 셋을 나눠 운영한다([3.4](#s33)).
...
**구성 방법**
- <a id="ref17"></a>**[17]** LLM Evals FAQ (평가셋 규모·층화 표본) — Hamel Husain — <https://hamel.dev/blog/posts/evals-faq/>
...
| 2026-06-29 | 0.2 | 고객 — "평가셋을 어떻게 구성하는지 방법론도(예: 10%를 골든셋으로 등)" | §3.2 「평가셋 구성 방법 — 규모·표본·구성비」 신설: ① "10% 골든셋"은 표준 아님 명확화(학습 모델 train/test 분할 10~20% vs LLM·RAG는 신뢰도·커버리지로) ② 규모=슬라이스당 신뢰도(표본수↔오차 표: 50건 ±14%~1,000건 ±3%, 슬라이스당 ~250건 기준) ③ 표본=층화 추출+희소 케이스 과대표집 ④ 구성비=대표·엣지·부정·애매 4종 의도 혼합. 누수 분리·동결/성장 셋 분리 한 줄. 참고자료 [17] Hamel Husain 추가. 하위 절 재번호(채점→3.3·운영→3.4)·TOC·교차참조 갱신 | §3.2 신설·§3.1·§2.4·TOC·참고자료 |
