...
3. [Tech Stack — 솔루션 검토](#tech)
    - [3.1 솔루션 유형](#s41)
    - [3.2 선정 기준](#s42)
4. [How — 어떻게 준비·운영하나](#how)
    - [4.1 구축 절차](#s51)
    - [4.2 운영 — 버전·의존성 관리](#kq3)
    - [4.3 운영 — 성능 저하 감지·개선](#kq5)
    - [4.4 작성 규칙 — 잘 쓴 자산과 못 쓴 자산](#s54)
    - [4.5 역할 (RACI)](#s55)
...
본 가이드는 검증된 Prompt와 Harness를 개인의 메모가 아니라 조직이 함께 쓰는 데이터 자산으로 만드는 방법을 다룬다. 왜·언제 자산화하는지(1장), 무엇을 갖추고 적용하면 무엇이 달라지는지(2장), 어떤 솔루션으로(3장), 실제로 어떻게 구축·운영하는지(4장), 인접 주제와 경계는 어디인지(5장)를 차례로 본다. 다루는 대상은 AI 에이전트를 만드는 일이 아니라, 그 AI가 실행할 Prompt·Harness를 조직의 데이터 자산으로 준비하는 일이다.
...
자산화는 반복·고영향 업무를 고르는 데서 시작해, 업무 절차를 Prompt로 바꾸고, 의존 자산을 연결하고, 평가를 통과하면 버전을 등록하고, Harness로 묶어 재사용하는 흐름으로 진행한다. 아래에서 그 구축 절차(4.1)와 배포 후 버전·의존성 관리(4.2)·성능 저하 대응(4.3), 자산을 잘 쓰는 작성 규칙(4.4), 역할 분담(4.5)을 차례로 본다.
