...
- **① 기계 가독형 신호로의 전환.** 사람만 해석하던 결함 사진·클레임이 "라벨이 붙은 학습 신호"가 되어, 분류·예측 자동화가 가능해진다. (Raw → 라벨·해설 → AI-Ready Data)
- **② 판단 기준 일관 + 재작업 감소.** 표준 분류 체계와 가이드라인으로 누가 작업해도 같은 결과가 나오고, 재학습·재라벨링 비용이 줄어든다.
- **③ 지능형 자산화·재사용.** 한 번 표준 스키마로 잘 라벨링한 데이터셋은 여러 AI 과제(불량 예측·자동 분류·원인 추천)에 재사용된다.

> 🏭 **두산전자 예시.** 동박적층판(CCL) 외관 검사에서, 라벨 없는 결함 사진 5만 장은 그 자체로는 AI 학습에 쓸 수 없다. "크랙/딤플/오염/스크래치" 4종 표준 라벨과 결함 위치(박스)를 붙이는 순간, 동일 데이터가 외관 불량 자동 분류 모델의 학습 데이터로 살아난다.
...
데이터에 붙이는 의미 정보는 깊이에 따라 세 층으로 나뉜다. 이 세 층이 이 가이드 전체에서 쓰는 정본(canonical) 모델이다.
...
### 6.2 라벨링 시키는 법 — AI 1차 라벨 + 사람 검수 (KQ4)

**핵심 원칙: 사람이 전수 검수하지 않는다.** 라벨링은 AI가 1차로 붙이고(Pre-labeling), 사람은 **신뢰도가 낮거나 업무 리스크가 큰 건만** 검수하는 사람 검수 개입(HITL, Human-in-the-Loop) 구조로 한다.

- **AI 1차 라벨(Pre-labeling):** 사전학습 모델이나 [SAM 2](https://ai.meta.com/research/sam2/)(이미지 자동 분할) 같은 도구로 초안 라벨을 자동 생성.
- **사람 검수:** 사람은 처음부터 끝까지 다 다는 게 아니라, AI 라벨의 확신도(confidence)에 따라 분기된 건만 본다.

### 6.3 품질 맞추는 법 — Pilot → 작업자 간 일치도(IAA) → 가이드 보정 (KQ3)

본 라벨링에 들어가기 전, 소규모 시범(Pilot)으로 "여러 사람이 같은 데이터를 같게 라벨하는지"를 측정한다. 이 측정값이 **작업자 간 일치도(IAA, Inter-Annotator Agreement)**다.

- 일치도가 낮으면 → 가이드라인이 모호하다는 신호 → **가이드라인을 고치고 다시 시범**한다(④↔⑤ 반복).
- 측정 지표는 작업자 수·데이터 성격에 따라 고른다([Cohen's κ](https://en.wikipedia.org/wiki/Cohen's_kappa) 등). 상세 → [Backup 6-A](#backup-6-a-iaa-측정-지표와-해석-기준).

> **정답 기준셋(Gold Standard).** 시범·검수의 기준이 되도록, 전문가가 합의해 확정한 소량의 모범 정답 데이터를 만들어 둔다. 작업자 결과를 이 Gold Standard와 비교해 정확도를 잰다.

### 6.4 검수 분기 — 확신도 기반 라우팅
...
| KPI | 쉬운 의미 | 방향 | 측정 목적 |
|---|---|---|---|
| **작업자 간 일치도(IAA)** | 여러 사람이 같게 라벨하는 정도 | ↑ | 가이드라인·기준의 명확성 |
| **라벨 오류율(LER, Label Error Rate)** | 잘못 달린 라벨 비율 | ↓ | 학습 데이터 신뢰도 |
| **처리량·건당 비용** | 단위 시간당 라벨 수 / 1건당 비용 | 처리량↑·비용↓ | 효율성 |
| **QA 통과율·재작업률** | 검수 합격 비율 / 다시 한 비율 | 통과율↑·재작업↓ | 품질 안정성 |
| **AI Pre-label 채택률(ALAR)** | AI 1차 라벨이 그대로 채택된 비율 | ↑ | 자동화 성숙도 |

> 이 지표들은 [2.2 기대 효과](#22-기대-효과)(일관·효율·자산화)와 직접 연결된다. IAA↑·LER↓는 "기준 일관", 처리량↑·ALAR↑는 "효율·자동화"의 측정값이다.
