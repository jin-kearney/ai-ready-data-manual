...
|---|---|---|---|
| CCL 검사 결과 → 설비 합·불 자동 판정 | 높음 (사람 검토 없이 즉시 판정) | 큼 (오판 시 불량 유출·라인 정지) | 1 — 최우선, 전 차원 엄격 |
...
두 축이 모두 높은 첫 행(설비 자동 판정용 데이터)은 여섯 차원 전부에 엄격한 합격선을 걸고, 사람이 한 번 걸러 쓰거나 영향이 작은 아래 행은 핵심 규칙만 걸거나 샘플로 점검한다. 직접성이 높아도 영향이 작으면(참고용 검색) 우선순위는 내려간다. 한꺼번에 전부 통제하지 않고, 두 축이 모두 높은 데이터에서 기준선을 만든 다음 범위를 넓힌다.
...
| 관리 항목 | 관리 기준 | 관리 목적 |
|-----------|-----------|-----------|
| 품질 규칙 | AI 활용 대상 데이터는 품질 규칙을 정의한다. | 데이터 품질 기준 일관성 확보 |
| Quality Gate | AI 학습·추론 전 Quality Gate를 반드시 수행한다. | AI 활용 가능 데이터 통제 |
| AI 활용 제한 | Quality Gate를 통과하지 못한 데이터는 AI 활용 대상에서 제외한다. | AI 결과 신뢰성 확보 |
| 규칙 변경 관리 | 품질 기준 변경 시 Quality Rule과 Quality Gate를 함께 갱신한다. | 운영 기준 최신성 유지 |
| 운영 점검 | 품질 규칙과 Quality Gate 운영 현황을 정기적으로 점검한다. | 지속적인 품질 개선 |
| 데이터 오너십 | 품질 관리 대상 데이터마다 품질을 책임지는 데이터 오너를 지정한다. | 품질 책임 소재 명확화 |
| 역할·책임(R&R) | 품질 규칙 정의·검증·예외 승인의 역할과 책임을 구분하여 운영한다. | 운영 책임 체계 확보 |
| 생애주기(Lifecycle) | 데이터 생성부터 폐기까지 단계별 품질 관리 기준을 정의한다. | 전 생애주기 품질 유지 |
...
상시 점검하는 핵심 지표는 다섯 개다. 각 구간(구축·운영·운영 품질)에서 하나씩 잡아, 게이트를 얼마나 걸었고·얼마나 걸러내며·얼마나 빨리 고치는지를 한 화면으로 본다. 절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세우고 목표 대비로 관리한다. 초기에는 이 다섯 개로 시작하고, 운영이 성숙하면 [별첨 전체 KPI 목록](#kpi-all)까지 넓힌다.

| 구분 | KPI | 정의 | 산식 | 목표 방향 |
|------|------|------|------|-----------|
| 구축 수준 | Quality Gate Coverage | AI 활용 전 Quality Gate가 적용되는 데이터 비율 | Quality Gate 적용 데이터 / AI 활용 데이터 | 높을수록 |
| 구축 수준 | 운영 기준 정의율 | 품질 관리 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관리 대상 | 높을수록 |
| 운영 수준 | Quality Gate Pass Rate | Quality Gate를 통과한 데이터 비율 | 통과 데이터 / 검증 데이터 | 높을수록 |
| 운영 수준 | 데이터 품질 이슈 발생률 | 운영 중 품질 오류가 발생한 비율 | 품질 이슈 건수 / 운영 데이터 건수 | 낮을수록 |
| 운영 품질 | 품질 개선 Lead Time | 품질 이슈 발견 후 개선 완료까지 소요 시간 | 평균 개선 시간 | 짧을수록 |

통과율(Pass Rate)이 지나치게 높으면 게이트가 느슨한 것은 아닌지, 지나치게 낮으면 원천 품질이나 합격선을 재검토할 것인지 함께 본다.
...
[7.2 핵심 운영 KPI](#s72)의 다섯 개를 포함한 품질 KPI 전체다. 처음부터 전부 재지 말고 핵심 다섯 개로 시작해, 운영이 자리 잡으면 아래 지표까지 단계적으로 넓힌다.
