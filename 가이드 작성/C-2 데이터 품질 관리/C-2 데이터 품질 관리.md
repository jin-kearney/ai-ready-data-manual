...
| 품질 관리를 어떻게 측정하나 | Quality Gate 적용률·통과율·품질 이슈 발생률로 점검한다 | [7](#kq5) |
...
품질 관리는 데이터를 신뢰할 수 있게(Trustworthy) 만드는 단계에서 AI 투입 직전에 "써도 되는가"를 판정하는 마지막 관문을 맡는다.
...
|---|---|---|---|
| 도장 검사 결과 → 설비 합·불 자동 판정 | 높음 (사람 검토 없이 즉시 판정) | 큼 (오판 시 불량 유출·라인 정지) | 1 — 최우선, 전 항목 엄격 |
...
| 규칙 유형 | 점검 내용 | 도장 검사 데이터 예시 | 연결 항목 |
|---|---|---|---|
...
품질 점검은 사람이 일일이 보지 않고 규칙으로 자동 판정하는 것을 기본으로 한다. 결측·범위·형식·중복·최신성은 규칙으로 합·불을 명확히 판정할 수 있으므로 자동화한다. 분포 이상이 잡혔으나 원인이 불분명한 경우처럼 규칙으로 명확히 판정하기 어려운 경우에만 사람이 개입한다.
...
게이트가 미달 데이터를 탐지하는 것에서 끝나는 것이 아니라, 원인을 식별하고 수정해야 한다. 위반 처리 흐름과 역할은 다음과 같다. 파란 영역(품질 게이트)의 【자동】 단계는 시스템이 처리하고, 초록 영역(데이터 오너·엔지니어·품질 담당)의 【사람】 단계는 담당자가 수동으로 판단·조치한다.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','fontSize':'13px'}}}%%
sequenceDiagram
    box rgb(219,232,245) 자동 — 시스템(품질 게이트)
    participant G as 품질 게이트
    end
    box rgb(194,230,214) 사람 — 수동 판단·조치
    participant O as 데이터 오너
    participant E as 데이터 엔지니어
    participant Q as 품질 담당
    end
    G->>O: 【자동】 미달 탐지·알림 (어느 규칙·어느 데이터)
    O->>O: 【사람】 원인 분류 (원천 오류 / 규칙 과민 / 일시 결측)
    alt 원천 데이터 문제
        O->>E: 【사람】 원천 수정·재적재 요청
        E->>G: 【사람】 고친 데이터 제출
        G->>G: 【자동】 재점검 → 통과분 승격
    else 규칙·기준 문제
        O->>Q: 【사람】 합격선·규칙 재검토 요청
        Q->>G: 【사람】 기준 갱신 등록
        G->>G: 【자동】 갱신 기준으로 재점검
    else 한시 사용 불가피
        O->>Q: 【사람】 예외 승인 요청 (사유·기간·범위)
        Q->>G: 【사람】 예외 등록
        G->>G: 【자동】 예외 적용 → 한시 통과
    end
```
...
| 데이터 품질 관리(C-2)가 하는 것 | 인접 주제 | 인접 주제가 하는 것 | 연계 포인트 |
|---|---|---|---|
| AI 투입 전 합격 판정 | [C-1 Observability](../C-1%20Observability/C-1%20Observability.md) | 운영 중 데이터 이상을 실시간 감지·알림 | 분포·이상치 규칙이 접점 |
| 현시점 사용 가능 판정 | [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md) | 출처·이동·변환 이력을 사후 추적 | 출처 불명 판정 시 C-3 정보 참조 |
| 품질 판정 | [F-4 AI 데이터 권한 보안](../F-4%20AI%20데이터%20권한%20보안/F-4%20AI%20데이터%20권한%20보안.md) | 접근 권한·동의·민감정보 마스킹 | 같은 관문에서 함께 통제하되 책임은 분리 |
| 적재 결과의 품질 판정 | [B-1 데이터 전처리](../B-1%20데이터%20전처리/B-1%20데이터%20전처리.md) | 비정형 데이터를 구조화·적재 | 적재 결과를 Quality Gate에서 판정 |
| 그 기준으로 품질 판정 | [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md) | 데이터 품질 기준을 기록 | A-2 기준으로 C-2가 판정 |
...
품질 게이트를 가운데 두고 보면, 전처리·적재 결과가 게이트를 통과해야 AI에 투입되며, 관측(C-1)·계통(C-3)이 판정 근거를 대고 권한·보안(F-4)이 같은 관문에서 함께 통제한다.
...
이 절은 품질 관리가 실제로 잘 되고 있는지 숫자로 확인하는 KPI를 정의한다. 지표의 오너십·역할과 책임은 4장(운영)과 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)에서, 데이터 보존·폐기 시간축은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','clusterBkg':'#EEF4FB','clusterBorder':'#6B9AD1','fontSize':'13px'}}}%%
flowchart LR
    STD["운영·책임 체계<br/>(4장 · F-1 · F-2)"]:::std
    K["KPI로 점검<br/>Gate 적용률·통과율·품질 이슈"]:::mid
    G{"목표(SLA)<br/>대비 평가"}:::gate
    STD --> K --> G -->|미달 시 개선| STD
    classDef std fill:#2456A4,color:#fff,stroke:#163a73,stroke-width:2px;
    classDef mid fill:#B7CDE6,stroke:#6B9AD1,color:#10243f;
    classDef gate fill:#4FA47D,stroke:#2f6b50,color:#fff;
```

KPI 목표치는 절대값을 미리 못 박지 않는다. 먼저 현재 수준(기준선)을 측정하고, 데이터 오너와 목표치·기한을 SLA로 합의한 뒤 목표 대비 실적으로 관리한다.

| KPI | 왜 필요한가 | 측정 방법 (산식 · 주기) | 목표 설정 |
|-----|-------------|--------------------------|-----------|
| Quality Gate 적용률 | AI에 쓰는 데이터가 실제로 품질 검문을 거치는지 확인한다. 미적용 데이터는 검증 없이 투입된다 | 게이트 적용 데이터 / AI 활용 데이터 · 월 1회 | AI 활용 데이터 100%를 목표로 우선순위 데이터부터 확대 |
| Quality Gate 통과율 | 원천 데이터 품질 수준을 보여준다. 급락·급등은 원천 품질이나 합격선을 다시 볼 신호다 | 통과 데이터 / 검증 데이터 · 월 집계 | 절대 목표보다 적정 범위 관리(추세·이상치 감시) |
| 품질 이슈 발생률 | 통과시킨 데이터가 실제로 문제없었는지, 즉 게이트의 실효성을 본다 | 통과 후 품질 이슈 건수 / 운영 데이터 건수 · 월 집계 | 기준선 대비 하향, SLA 합의 |

통과율은 높다고 좋은 것이 아니다 — 지나치게 높으면 게이트가 느슨한지, 지나치게 낮으면 원천 품질·합격선을 재검토한다. 운영이 성숙하면 품질 규칙 정의율·품질 개선 Lead Time·재발률 등으로 넓힌다.
...
|---|---|---|---|---|
| rule_id | 규칙 고유 식별자 | DQ-PNT-0007 | 필수 | 데이터 엔지니어 |
| 대상 데이터 | 규칙을 적용할 데이터셋·컬럼 | 도장검사결과.측정값 | 필수 | 데이터 오너 |
...
품질 규칙 정의서의 빈 양식과, 도장 검사 데이터로 한 건을 채운 예시다.
...
| 2026-07-01 | 0.10 | 정본 예시 교체 (고객) — §4.1 등 | 정본 worked example을 'CCL(동박적층판)'에서 범용 제조 외관검사인 '도장 검사'로 통일 변경. §4.1 점검 리포트 SVG 제목 도장검사결과_월간, §3.2·§4.3 예시·별첨 대상 데이터(도장검사결과)·rule_id(DQ-PNT-0007)까지 일괄 교체. 규칙·수치·구조·기존 측정 예시(경화 온도·두께) 불변 | §3.2·4.1(이미지)·4.3·별첨 |
| 2026-07-01 | 0.11 | §4.3 자동 판정 다이어그램 정리 (고객) | 어색한 '판정 모호'·'모호·이상 신호?' 표현 정리. R에서 보낸 뒤 다시 묻는 중복 판단 노드(AMB) 제거 → R의 세 번째 분기를 '규칙으로 판단 어려움'으로 사람 검토에 직접 연결(합격/차단은 '확인 후'로 라벨). 본문 '규칙으로 판정하기 모호하므로'→'규칙으로 명확히 판정하기 어려운 경우' | §4.3 |
| 2026-07-01 | 0.12 | §4.4 위반 처리 다이어그램 자동/수동 구분 (고객) | 시퀀스 다이어그램을 두 박스(자동=품질 게이트 / 사람=오너·엔지니어·품질 담당)로 묶고 각 동작에 【자동】·【사람】 표기. 게이트의 자동 재점검·예외 적용 단계(G→G)를 명시. 도입 문장에 색·자동/수동 읽는 법 추가 | §4.4 |
| 2026-07-01 | 0.9 | 2.1 "체계 내 위치" 중복 제거 (고객) — C-1·C-2·C-3 공통 | C-1·C-2·C-3의 2.1이 같은 신뢰(Trustworthy) 삼각 분업 설명·다이어그램을 세 파일에 반복하고 6장 Where와도 겹쳐 읽기 불편 → 2.1을 "정의와 역할"로 개칭하고 정의 + 한 줄 위치 포인터만 남김. 삼각 분업 문단은 삭제, 삼각관계 mermaid는 6장 Where로 이동(설명 문장 동반). 다른 주제와의 경계를 6장으로 일원화. What 도입문·목차 앵커 문구 정합 | 2장·6장·목차 |
| 2026-07-01 | 1.0 | KPI 섹션 재설계 (고객) — C-1·C-2·C-3 공통 | "관리 기준 vs KPI 층이 다르다" 도입문과 관리 기준 8행 표가 혼란(거버넌스 3행이 모두 "운영 기준 정의율" 하나로 수렴, KPI 없는 행 다수)해 처음부터 재설계. 관리 기준 프레임과 보일러플레이트 목표 문장을 걷어내고, 구축→운영→운영 품질 3층으로 그 주제 고유 핵심 지표 4개만 남긴 표(구분·지표·무엇을 재나·산식·목표 방향)로 교체. 별첨 "전체 KPI 목록"은 흡수(산식을 본문으로), 추가 지표는 한 줄로 나열. 오너십·R&R·생애주기 운영 기준은 F-1·F-2 포인터로. 별첨의 항목 사전·용어 등은 유지 | 7장·목차·별첨 |
| 2026-07-01 | 1.1 | KPI 섹션 운영 기준 복원·2소절화 (고객) — C-1·C-2·C-3 공통 | "운영 기준과 KPI가 잘 적용되는지 파악"하도록 재구성. 도입문에 '품질·출처·변경 이력·계보 관리 체계로 신뢰성 저하·Hallucination 방지' 목적 복원. 7.1 운영 기준(품질 규칙·Quality Gate + 데이터 오너십·R&R·생애주기)과 7.2 핵심 KPI(구축·운영·운영 품질, 산식 포함)로 분리. 운영 기준 적용 여부는 KPI '운영 기준 정의율' 하나로만 측정해 옛 '3행→1KPI' 중복 없이 반영. 목차 7.1·7.2 갱신 | 7장·목차 |
| 2026-07-01 | 1.2 | KPI 섹션 다이어그램 추가 (고객) — C-1·C-2·C-3 공통 | 7장이 표·글만 있어 잘 안 와닿는다는 피드백 → 도입부에 '운영 기준 정의 → 구축·운영·운영 품질 단계별 KPI 점검 → 목표(SLA) 대비 미달 시 개선' 순환 mermaid 추가(주제별 KPI 라벨, 02 표준 컬러). 한 줄 설명문 동반 | 7장 |
| 2026-07-01 | 1.3 | 7.1 운영 기준 실질 정의 (고객) — C-1·C-2·C-3 공통 | "지정한다/구분한다/정의한다"만 있고 실제 내용이 없던 순환 표를 걷어내고, 데이터 오너십·역할과 책임(R&R)·생애주기(Lifecycle)를 구체 내용으로 정의. 오너십=오너 1명 지정·품질 합격선 승인·미달 활용 여부 판단, R&R=데이터 오너/품질 담당/예외 승인자 역할·책임 표, 생애주기=수집→적재→활용→보관/폐기 중 품질 기준 유지·합격선 갱신 | 7.1 |
| 2026-07-01 | 1.4 | 운영 기준 공통 분리·KPI 정비 (고객) — C-1·C-2·C-3 공통 | ① C-1·C-2·C-3에 중복되던 운영 기준(오너십·R&R·생애주기)을 신설 [별첨 C. 운영 기준서](../별첨%20C%20운영%20기준서/별첨%20C.%20운영%20기준서.md)로 분리, 7.1 삭제하고 7장 도입에서 링크. ② 7장을 이 주제 KPI만 남기고 표를 'KPI·왜 필요한가·측정 방법(산식·주기)·목표 설정'으로 재구성 — '높을수록/낮을수록' 방향 열 제거, 컨설팅식(기준선 측정→오너와 SLA 합의) 목표 설정으로 교체. ③ KPI를 꼭 필요한 3개로 축소(Gate 적용률·통과율·품질 이슈 발생률), 운영 기준 정의율은 별첨 C로 이관. ④ 순환 다이어그램 단순화, 목차 7.1/7.2·질문 표 갱신 | 7장·목차·별첨 C |
| 2026-07-01 | 1.5 | 별첨 C 폐지·F로 연결 (고객) | 운영 기준(오너십·R&R·생애주기)은 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)(7.1 관리 기준)·[F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)가 이미 다루고 이 가이드도 4장(운영)에 데이터 오너·품질 담당·승인자 역할이 있어, 별첨 C 운영 기준서는 중복 → **삭제**. 7장 KPI 섹션을 자기 KPI로 완결시키고 오너십·역할은 4장·F-1, 보존·폐기는 F-2로 연결. 도입문·다이어그램·질문 표의 별첨 C 참조 제거 | 7장·질문 표 |
| 2026-07-01 | 1.6 | 2.1 경계 문장 제거 (고객) | 2.1 정의와 역할에서 "실시간 이상 감지(C-1)·사후 이력 추적(C-3)·접근 권한 통제(F-4)와의 경계는 6장에서 정리한다" 문장 삭제 — 다른 주제와의 관계는 6장 Where에서만 다루고 본문 정의에는 두지 않음. 정의와 역할만 남김 | 2.1 |
| 2026-07-01 | 1.7 | 6장 Where 표 컬럼 순서 통일 (고객) | Where 표를 B-1 형식('[주제]가 하는 것·인접 주제·인접 주제가 하는 것·연계 포인트' 4열)에 맞춤. 기존 3열('인접 주제·그 주제의 역할·C-2와의 경계')을 4열로 전환 — 경계 서술을 'C-2가 하는 것'과 '연계 포인트'로 분리 | 6장 |
