...
운영 과정에서는 품질 기준 준수 여부와 Quality Gate 운영 현황을 지속적으로 점검하고 개선한다.

데이터 품질 관리의 상세 관리 기준과 KPI는 **7장 KPI**에서 정의한다.
...
데이터 품질 관리는 AI 활용에 필요한 데이터가 정의된 품질 기준을 지속적으로 만족하도록 관리하는 것을 목표로 한다.

따라서 데이터 품질은 **무엇을 관리할 것인가(관리 기준)**와 **얼마나 잘 관리되고 있는가(KPI)**를 함께 정의하여 운영한다.
