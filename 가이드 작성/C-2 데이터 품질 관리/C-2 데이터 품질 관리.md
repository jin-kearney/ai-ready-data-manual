...
| 품질 관리를 어떻게 측정하나 | 구축(게이트 적용률)·운영(게이트 통과율·품질 이슈 발생률)·운영 품질(품질 개선 Lead Time) 세 층의 지표로 잰다 | [7.1](#kq5) |
...
데이터 품질·출처·변경 이력·계보(Lineage) 관리 체계를 통해 AI 활용 시 데이터 신뢰성 저하와 Hallucination을 방지하려면, 품질 관리가 무엇을 지켜야 하는지(운영 기준)와 그것이 잘 적용되는지(KPI)를 함께 정한다.

운영 기준을 정하고, 데이터가 구축→운영→운영 품질 단계를 지나며 각 단계의 KPI로 점검한 뒤, 목표에 못 미치면 운영 기준·규칙을 손봐 다시 돌린다.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','clusterBkg':'#EEF4FB','clusterBorder':'#6B9AD1','fontSize':'13px'}}}%%
flowchart LR
    STD["운영 기준 정의<br/>품질 규칙·Quality Gate<br/>+ 오너십·R&R·생애주기"]:::std
    T1["구축<br/>Quality Gate 적용률<br/>운영 기준 정의율"]:::t1
    T2["운영<br/>Gate 통과율<br/>품질 이슈 발생률"]:::t2
    T3["운영 품질<br/>품질 개선 Lead Time"]:::t3
    G{"목표(SLA)<br/>대비 점검"}:::gate
    STD --> T1 --> T2 --> T3 --> G
    G -->|미달 시 개선| STD
    classDef std fill:#2456A4,color:#fff,stroke:#163a73,stroke-width:2px;
    classDef t1 fill:#DCE8F5,stroke:#6B9AD1,color:#10243f;
    classDef t2 fill:#B7CDE6,stroke:#6B9AD1,color:#10243f;
    classDef t3 fill:#79C3E8,stroke:#3F9BD4,color:#0b3a52;
...
운영 기준은 품질 관리를 누가·무엇을·어느 단계까지 책임지고 관리하는지 정한 약속이다. 아래 세 가지를 정의한다.

**데이터 오너십** — 품질 관리 대상 데이터마다 데이터 오너 1명을 지정한다. 오너는 그 데이터의 품질 합격선(어느 수준이면 AI에 써도 되는가)을 승인하고, 미달 데이터의 활용 여부를 최종 판단한다.

**역할·책임(R&R)** — 품질 규칙 정의부터 예외 승인까지의 책임을 아래와 같이 나눈다.

| 역할 | 책임 |
|------|------|
| 데이터 오너 (현업 부서) | 품질 합격선 승인, 미달 데이터 활용 여부 최종 판단 |
| 품질 담당 (데이터·품질팀) | 품질 규칙 정의·검증, Quality Gate 운영 |
| 예외 승인자 (품질 책임자) | 합격선 미달 데이터의 예외 활용 승인 |

**생애주기(Lifecycle)** — 데이터가 수집→적재→활용→보관/폐기를 거치는 동안 품질 기준을 유지한다. 신규 제품·공정이 생기면 합격선을 새로 정하고, 기준이 현실과 어긋나면 데이터 오너 합의로 갱신한다.

<a id="s72"></a>

### 7.2 핵심 KPI

운영 기준이 잘 적용되는지 아래 지표로 점검한다. 구축(얼마나 넓게 게이트를 걸었나)→운영(얼마나 걸러 내나)→운영 품질(얼마나 빨리 개선하나) 순으로 본다.

| 구분 | 지표 | 무엇을 재나 | 산식 | 목표 방향 |
|------|------|-------------|------|-----------|
| 구축 | Quality Gate 적용률 | AI 활용 데이터 중 품질 게이트가 적용된 비율 | 게이트 적용 데이터 / AI 활용 데이터 | 높을수록 |
| 구축 | 운영 기준 정의율 | 품질 관리 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관리 대상 | 높을수록 |
| 운영 | Quality Gate 통과율 | 게이트 검증을 통과한 데이터 비율 | 통과 데이터 / 검증 데이터 | 적정 유지 |
| 운영 | 품질 이슈 발생률 | 통과 후 운영 중 품질 오류가 발생한 비율 | 품질 이슈 건수 / 운영 데이터 건수 | 낮을수록 |
| 운영 품질 | 품질 개선 Lead Time | 품질 이슈 발견부터 개선 완료까지 걸린 시간 | 평균 개선 시간 | 짧을수록 |

절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다. 통과율은 지나치게 높으면 게이트가 느슨한 것은 아닌지, 지나치게 낮으면 원천 품질이나 합격선을 재검토할지 함께 본다.
