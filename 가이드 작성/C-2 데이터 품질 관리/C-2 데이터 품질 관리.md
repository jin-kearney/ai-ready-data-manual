...
2. [What — 무엇인가·무엇을 갖추나](#what)
    - [2.1 데이터 품질 관리의 역할과 체계 내 위치](#s21)
    - [2.2 품질 기준 — 여섯 가지 차원](#s22)
    - [2.3 AI에 쓰면 안 되는 데이터](#s23)
    - [2.4 Quality Gate — 투입 전 관문](#s24)
3. [When — 어디부터 게이트를 거나 (적용 우선순위)](#when)
    - [3.1 기본 필수 대상](#s31)
...
<!-- KQ→섹션 매핑(자가 점검): KQ1 품질 기준→2.2(kq1) / KQ2 사용 제한→2.3(kq2) / KQ3 Quality Gate→2.4(kq3) / KQ4 예외 승인→4.4 운영(kq4) / KQ5 측정→4.5 운영 성과 측정(kq5). -->
...
이 가이드는 데이터 품질 관리가 왜 필요한지(1장), 무엇을 기준으로 판정하고 무엇을 갖추는지(2장), 어디부터 적용하는지(3장), 어떻게 구축·운영하고 성과를 어떻게 측정하는지(4장), 어떤 솔루션을 검토하는지(5장), 인접 주제와 어떻게 나뉘는지(6장)를 다룬다. AI는 들어온 데이터를 그대로 받아들여 결과를 산출하므로, 빈칸·오류·철 지난 값이 섞인 데이터를 거르지 않으면 잘못된 데이터가 그대로 AI 결과로 이어진다.

## 이 가이드가 답하는 5가지 질문

| 핵심 질문 | 한 줄 답 | 본문 |
|---|---|---|
| AI 활용을 위한 데이터 품질 기준은 무엇인가 | 완전성·정확성·일관성·최신성·유효성·유일성 여섯 차원으로 보고, 업무·용도별 합격선을 정한다 | [2.2](#kq1) |
...
| 품질 관리를 어떻게 측정하나 | 품질 통과율·미달 차단 건수·개선 리드타임으로 측정한다 | [4.5](#kq5) |
...
데이터가 많아도 그 안에 빈칸·오류·철 지난 값이 섞여 있으면 AI는 그것을 그대로 학습하고 그대로 답한다. 사람이 보는 보고서는 사람이 이상한 값을 걸러 읽지만, AI는 잘못된 데이터를 걸러내지 못하고 흡수해 대규모로 재생산한다. 품질 관리는 이 위험을 투입 전에 차단하는 통제 장치다.
...
| 철 지난 데이터로 현재를 판정함 | 갱신이 멈춘 옛 검사 데이터가 최신값처럼 섞여, AI가 현재 라인 상태를 잘못 판단한다 |
...
공통점은 데이터가 없는 것이 아니라, 있는 데이터를 거르지 않고 AI에 투입하는 데 있다. 품질의 고전적 정의가 "용도 적합성(Fitness for Use)"[\[1\]](#ref1)인 까닭이 여기 있다. 품질은 절대적인 완벽함이 아니라 "이 AI 용도에 쓸 수 있는가"라는 상대적 판정이다.
...
이 장은 품질 관리가 무엇이고 무엇을 기준으로 판정하는지를 정의한다. 구축 절차와 운영 방법은 [4장](#how)에서 다루고, 여기서는 체계 내 위치, 품질 기준(여섯 차원), 사용 제한 기준, 투입 전 관문(Quality Gate)을 정의한다.

<a id="s21"></a>

### 2.1 데이터 품질 관리의 역할과 체계 내 위치

데이터 품질 관리는 데이터가 AI 활용 기준을 충족하는지 판정해, 통과한 데이터만 AI에 투입되게 하는 통제 체계다. 데이터를 정제·교정하는 작업 자체가 아니라, 정제된 데이터가 "쓸 수 있는 수준인가"를 판정하고 못 미치면 막는 관문이다.

품질 관리는 데이터를 신뢰할 수 있게(Trustworthy) 만드는 단계의 판정 담당이다. 같은 단계에서 [C-1 Observability](../C-1%20Observability/C-1%20Observability.md)가 운영 중 이상을 실시간 감지하고 [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md)가 사후 이력을 추적한다면, C-2는 AI 투입 직전에 "써도 되는가"를 판정한다. 접근 권한·개인정보 같은 보안 통제는 [F-4 AI 데이터 권한 보안](../F-4%20AI%20데이터%20권한%20보안/F-4%20AI%20데이터%20권한%20보안.md)이 같은 관문에서 함께 본다.
...
품질 판정 결과가 사용 가능 여부를 가르므로, 품질 관리는 데이터가 AI로 가는 길목에 선 마지막 관문이다. 인접 주제와의 경계는 [6장](#where)에서 정리한다.
...
품질을 막연히 좋다·나쁘다로 말하지 않고, 여섯 가지 차원으로 나눠 본다. 본 가이드는 이 여섯 차원을 품질을 보는 정본 기준으로 삼아 끝까지 일관되게 쓴다.[\[2\]](#ref2)
...
기본 대상에 게이트를 건 이후에는 두 가지 축으로 우선순위를 매겨 합격선을 차등 적용한다. 하나는 그 데이터가 **AI에 직접 투입되는가**(사람 검토 없이 바로 쓰이는가), 다른 하나는 그 데이터가 **틀렸을 때 업무 영향이 큰가**(안전·품질 판정·규제 보고에 닿는가)다.

| 후보 데이터 | AI 투입 직접성 | 틀렸을 때 영향 | 적용 기준 |
|---|---|---|---|
| 안전·품질 판정 데이터 | 직접 | 큼 | 1 — 엄격 기준·다중 규칙 |
| 불량 원인 RAG 검사 데이터 | 직접 | 큼 | 1 — 엄격 기준 |
| 월간 집계 원천 | 직접 | 중간 | 2 — 기본 기준 |
| 사내 참고 검색 문서 | 직접 | 작음 | 3 — 샘플 점검 |

영향이 크고 투입이 직접적인 데이터는 더 엄격한 기준과 더 많은 규칙으로 보고, 영향이 작은 흐름은 기본 기준이나 샘플 점검으로 둔다. 한꺼번에 전부 통제하지 않고, 영향이 큰 흐름에서 기준선을 만든 다음 범위를 넓힌다.
...
| AI 동작 | 9999를 정상값으로 학습, 옛 데이터로 추세 오집계, 중복으로 불량 건수 부풀려짐 | 점검 리포트로 차원별 합·불을 확인, 미달 시 투입 차단 |
| 미달 처리 | 결과가 틀어진 사후에야 인지, 이미 나간 결과 수습 | 투입 직전 차단, 예외 승인으로만 한시 사용 |

점검 결과는 아래와 같은 리포트로 남아, 어느 차원이 합격선을 넘었고 무엇이 막혔는지 확인할 수 있다.

![데이터 품질 점검 리포트 모형 — 여섯 차원 점수 막대, 다섯 개 품질 규칙의 합격선·실측·판정, 그리고 최신성 미달로 투입을 차단한 종합 게이트 판정](이미지/품질-점검리포트-모형.svg)

이 사례에서는 대부분의 차원이 합격선을 넘었으나 최신성이 91%로 기준(95%)에 미달해, 게이트가 투입을 자동 차단했다. 담당자는 갱신이 늦은 원천을 고쳐 다시 게이트를 통과시키거나, 분석이 급하면 예외 승인(승인자·사유·기간·범위 기록)을 받아 한시적으로만 쓴다.
...
- **① 품질 기준 정의.** [2.2](#kq1)의 여섯 차원을 그 데이터의 용도에 맞춰 합격선으로 정한다. "결측률 2% 이하", "최신성 95% 이상"처럼 숫자로 둔다.
- **② 규칙으로 변환.** 차원별 합격선을 데이터에 바로 적용할 수 있는 점검 규칙으로 옮긴다([4.3](#s43)).
...
구축 절차의 ②단계에서는 여섯 차원을 실제 점검 가능한 규칙으로 옮긴다. 차원은 추상적이지만 규칙은 데이터에 바로 적용할 수 있는 구체적 조건이다.

| 규칙 유형 | 점검 내용 | CCL 검사 데이터 예시 | 연결 차원 |
|---|---|---|---|
...
규칙으로 대부분을 자동 판정하고 사람은 좁은 회색지대만 본다. 이렇게 해야 월 수만~수십만 건의 검사 데이터를 사람 손 없이 점검할 수 있다. 규칙 한 건을 정의하는 항목과 빈 템플릿은 [별첨](#a1)에 둔다.

> **권장 — 합격선은 메타데이터로 관리한다.** 차원별 합격선과 규칙 정의는 데이터에 흩어 두지 않고 [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md)에 함께 기록한다. 그래야 같은 데이터에 같은 기준이 일관되게 적용되고, 기준 변경 이력이 남는다.
...
품질 관리의 성과는 데이터를 얼마나 많이 점검했는가가 아니라, 신뢰할 수 있는 데이터만 AI로 갔는가로 본다. 다음 세 지표를 정본으로 둔다.[\[9\]](#ref9)

| 지표 | 측정 목적 | 산식 | 방향 |
|---|---|---|---|
| 품질 기준 통과율 | AI 투입 대상 데이터가 품질 기준을 통과하는 비율(체계의 종합 건강도) | (통과 점검 수 ÷ 전체 점검 수) × 100 | 높을수록 좋음 (↑) |
| 품질 미달 차단 건수 | 게이트가 실제로 막은 저품질 데이터량(게이트가 작동하는가) | 기간별 게이트 실패로 격리·차단된 건수 | 모니터 |
| 품질 개선 리드타임 | 품질 이슈 발생부터 해결까지 걸린 시간(고치는 속도) | 이슈별 (해결 시각 − 발생 시각) 평균 | 낮을수록 좋음 (↓) |
...
> **2층 연결:** 솔루션을 주제 가로질러 묶어 평가·선정하려면 → [Tech Stack 비교 정본](../../Tech%20Player/01%20Tech%20Stack%20비교%20(솔루션×주제).md). 본 절은 데이터 품질 관점에서 솔루션의 기능을 비교한다.
...
| 선정 기준 | 확인 포인트 |
|---|---|
| 규칙 기반 vs ML 이상탐지 | 합격선을 명시할 수 있으면 규칙, 무엇이 틀릴지 모르는 대규모 데이터는 ML, 보통 혼합한다 |
| 데이터 반출 제한 | 사외 반출이 제한되면 오픈소스 자체 호스팅이나 온프레미스 가능 상용을 검토한다 |
| 기존 데이터 플랫폼 | 이미 Databricks·AWS·GCP·Azure를 쓰면 내장형이 반출 없이 게이트를 만드는 1순위다 |
| 게이트 삽입 | 파이프라인에 게이트로 끼워 실패 시 차단·격리할 수 있는가 |
...
데이터 품질 관리는 품질 규칙을 정의하는 것보다 운영 과정에서 품질을 지속적으로 유지하고 개선하는 것이 중요하다.

따라서 KPI는 품질 규칙의 존재 여부가 아니라 **품질 수준, 운영 수준, 개선 수준**을 지속적으로 관리하기 위한 기준으로 활용한다.

| 구분 | KPI | 정의 | 산식 | 활용 목적 |
|------|------|------|------|-----------|
| 구축 수준 | Quality Rule Coverage | 관리 대상 데이터 중 품질 규칙이 정의된 비율 | 품질 규칙 정의 데이터 / 관리 대상 데이터 | 품질 기준 구축 수준 관리 |
|  | Quality Gate Coverage | AI 활용 전 Quality Gate가 적용되는 데이터 비율 | Quality Gate 적용 데이터 / AI 활용 데이터 | 품질 통제 범위 관리 |
|  | AI Ready Data Coverage | AI 활용 기준을 충족한 데이터 비율 | AI 활용 가능 데이터 / 전체 데이터 | AI 활용 준비 수준 관리 |
| 운영 수준 | Quality Gate Pass Rate | Quality Gate를 통과한 데이터 비율 | 통과 데이터 / 검증 데이터 | 운영 품질 수준 관리 |
|  | 데이터 품질 이슈 발생률 | 운영 중 품질 오류가 발생한 비율 | 품질 이슈 건수 / 운영 데이터 건수 | 품질 안정성 관리 |
|  | 품질 개선 Lead Time | 품질 이슈 발견 후 개선 완료까지 소요 시간 | 평균 개선 시간 | 운영 효율성 관리 |
| 운영 품질 | Rule 최신성 | 품질 규칙이 최신 기준으로 유지되는 수준 | 최신 규칙 / 전체 규칙 | 품질 기준 최신성 관리 |
|  | 재발률 | 동일 품질 이슈가 반복 발생한 비율 | 재발 건수 / 전체 품질 이슈 | 지속 개선 수준 관리 |
|  | 품질 자동화율 | 품질 검증이 자동 수행되는 비율 | 자동 검증 데이터 / 전체 검증 데이터 | 운영 자동화 수준 관리 |
...
| Level | 기준 |
|---------|------|
| Level 1 | 기본 품질 규칙을 수동으로 관리 |
| Level 2 | 주요 데이터에 Quality Rule을 적용 |
| Level 3 | AI 활용 데이터에 Quality Gate를 운영 |
| Level 4 | 품질 검증과 개선이 자동화되고 KPI를 지속적으로 관리 |
| Level 5 | AI 활용 전 과정에서 품질 관리와 개선이 자동으로 수행되는 수준 |

데이터 품질 관리의 목표는 모든 데이터를 동일한 수준으로 관리하는 것이 아니라, AI 활용에 필요한 데이터가 일관된 품질 기준을 만족하고 지속적으로 개선되는 수준(Level 5)에 도달하는 것이다.
