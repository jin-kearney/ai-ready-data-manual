...
본 절은 품질 관리가 실제로 잘 이루어지고 있는지 수치와 지표로 확인하는 KPI를 정의한다. 지표의 오너십·역할과 책임은 4장(운영)과 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)에서, 데이터 보존·폐기 시간축은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다.
...
| KPI | 목적 | 측정 방법 (산식 · 주기) | 목표 설정 |
|-----|-------------|--------------------------|-----------|
| Quality Gate 적용률 | AI 활용 데이터가 품질 검증을 실제로 거치는지 확인한다 | 게이트 적용 데이터 / AI 활용 데이터 · 월 1회 | AI 활용 데이터 100%를 목표로 우선순위 데이터부터 확대 |
| Quality Gate 통과율 | 원천 데이터 품질 상태/수준을 보여준다 (급락·급등은 원천 품질이나 합격선 재검토 필요) | 통과 데이터 / 검증 데이터 · 월 집계 | 절대 목표보다 적정 범위 관리(추세·이상치 감시) |
| 품질 이슈 발생률 | 통과시킨 데이터가 실제로 문제없었는지 등 게이트의 실효성을 본다 | 통과 후 품질 이슈 건수 / 운영 데이터 건수 · 월 집계 | 기준선 대비 하향, SLA 합의 |

통과율은 높다고 하여 반드시 좋은 것은 아니다 - 지나치게 높거나 낮을 경우 원천 품질, 합격선, 게이트를 재검토할 필요가 있으며, 운영 수준이 성숙하다면 품질 규칙 정의율·품질 개선 Lead Time·재발률 등으로 확장한다.
