...
데이터 품질 관리가 실제로 작동하는지 숫자로 확인하는 계기판이다. 구축(얼마나 넓게 게이트를 걸었나)·운영(얼마나 걸러 내나)·운영 품질(얼마나 빨리 개선하나) 세 층으로 나눠 본다. 처음부터 다 재지 말고 아래 핵심 지표로 시작해, 운영이 자리 잡으면 뒤에 적은 지표까지 단계적으로 넓힌다.
...
절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다. 통과율은 지나치게 높으면 게이트가 느슨한 것은 아닌지, 지나치게 낮으면 원천 품질이나 합격선을 재검토할지 함께 본다. 운영이 성숙하면 품질 규칙 정의율·AI 활용 가능 데이터 비율·규칙 최신성·재발률·품질 자동화율까지 넓힌다.

오너십·역할과 책임(R&R)·생애주기 같은 운영 기준은 별도 지표로 재기보다 품질 관리 대상마다 정해 두고 정기 점검한다([F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)·[F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다).
