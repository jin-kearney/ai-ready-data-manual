...
**핵심 차이 (현업 눈높이):** LPG는 관계를 "일급 엣지"로 저장해 포인터를 따라가듯 탐색하므로 경로가 깊어져도 빠르다. RDF는 사실을 잘게 쪼개므로(전역 고유 식별자) 깊은 경로를 따라갈 때 트리플 조인이 폭증해 성능이 떨어질 수 있다.[^memgraph] 반대로 RDF는 OWL로 "하위 클래스면 상위 클래스다" 같은 추론을 표준으로 지원하고, 외부 기관과 같은 어휘로 데이터를 주고받는 데 강하다.

> 어려운 약어(RDF·OWL·LPG·SPARQL·openCypher·GQL)의 한 줄 풀이는 [[Appendix A]](#appendix-a-기술-표준-rdfowlsparql-한-줄-풀이) 참고. 메인에서는 "어떤 기준으로 고르나"에 집중한다.
...
**쿼리 언어는 형식을 따라온다:** LPG → **openCypher / Cypher**(사실상 표준, 2024년 ISO GQL로 국제표준화), RDF → **SPARQL**(W3C 표준). 형식을 고르면 쿼리 언어 선택지도 함께 좁혀진다.
...
형식을 정했으면 저장소를 고른다. 핵심은 **"교체"가 아니라 "혼용"**이라는 점이다. 커니도 RDB와 Graph DB를 함께 쓴다.

| 기준 | RDB(관계형 DB)로 충분 | Graph DB가 필요 |
|---|---|---|
| 관계 깊이 | 1~2홉(단순 외래키 조회) | 3홉 이상 경로 탐색 |
| 쿼리 유형 | 집계·보고·트랜잭션(CRUD) | 패턴 매칭·경로 탐색·유사도 |
| 스키마 | 고정 스키마 안정적 | 연결 구조가 자주 변함 |
| 🏭 제조 예시 | 생산 실적 집계, 부품 BOM 테이블 | 설비-공정-불량 인과 추적, 공정 의존성 |

**혼용(폴리글랏) 패턴 — 제조 현장 권장:**
- **RDB**: 생산 실적·부품 마스터·품질 이력 등 **정형·트랜잭션 데이터**
- **Graph DB**: 설비-공정-부품-불량 간 **관계·경로 데이터**(온톨로지 인스턴스)

> 형식이 저장소를 좁힌다: **LPG 선택 → Neo4j·Memgraph·ArangoDB**, **RDF 선택 → Ontotext GraphDB·Apache Jena·Amazon Neptune(RDF 모드)**. 그래서 6.2(형식)를 6.3(저장소)보다 먼저 정한다.

### 6.4 3단계 — 추론 적재 방식: T-Box / A-Box 트레이드오프

온톨로지는 두 층으로 나뉜다. 이 구분이 "추론 결과를 언제 계산할지"를 정하는 기준이 된다.

| 구분 | 한 줄 풀이 | 🏭 두산 예시 |
|---|---|---|
| **T-Box**(Terminological Box, 개념 스키마) | "어떤 개념이 있고 어떤 관계를 맺나"를 정의한 **설계 규칙**. 사람이 설계, 크기 작음 | "부품은 설비의 하위 구성이다", "하위 구성 고장은 상위 설비에 영향을 준다" |
| **A-Box**(Assertional Box, 인스턴스 데이터) | T-Box 규칙에 맞춰 채워지는 **실제 운영 데이터** | "V-belt #1234는 D-402 선반의 구성품", "D-402 2026-06-17 진동 이상" |

**추론으로 A-Box를 풍부하게:** T-Box 규칙을 바탕으로 추론 엔진이 A-Box에 없던 사실을 도출한다(예: "V-belt 고장 → D-402 설비 영향"). 이 파생 사실을 **언제 계산하느냐**가 아키텍처 결정이다 — 트레이드오프가 있다.

| 방식 | 동작 | 저장 용량 | 쿼리 속도 | 적합한 경우 |
|---|---|---|---|---|
| **사전 적재**(Forward Chaining / Materialization) | 적재 시 추론 결과를 미리 계산해 저장 | ↑ 커짐(파생 사실까지 저장) | 빠름(미리 계산됨) | 데이터 안정적·쿼리 빈번 |
| **질의 시 추론**(Backward Chaining / Query-time) | 질문 들어올 때 필요한 추론만 수행 | ↓ 작음(원본만 저장) | 느릴 수 있음 | 데이터 변경 잦음·쿼리 드묾 |

> **🏭 제조 현장 권고**: 설비 마스터·공정 구조처럼 **안정적인 데이터**는 사전 적재로 쿼리 성능을 확보하고, 품질 이벤트·알람처럼 **실시간 생성되는 데이터**는 질의 시 추론 또는 점진적(Incremental) 방식을 섞는다.[^ontotext] 즉 "사전 추론하면 용량↑, 매번 추론하면 Query 시간↑"의 균형점을 데이터 변경 빈도와 쿼리 빈도로 정한다.

[^ontotext]: Ontotext, "What Is Inference?" / "Reasoning with Big Knowledge Graphs" — forward-chaining(materialization)과 query-time reasoning의 트레이드오프 설명. (https://www.ontotext.com/knowledgehub/fundamentals/what-is-inference/)

### 6.5 4단계 — 제조 표준 프레임워크 적용 판단: IOF

바닥부터 개념을 정의하는 대신, 제조 업계 **표준 온톨로지 프레임워크**를 가져다 쓰는 선택지가 있다. 대표가 **IOF**다.

> **IOF**(Industrial Ontologies Foundry, 산업 온톨로지 파운드리): OAGi가 운영하는 제조·유지보수·공급망용 **표준 참조 온톨로지** 모음. 상위 온톨로지 BFO(ISO/IEC 21838-2) 위에 IOF Core·유지보수·공급망 온톨로지를 모듈로 제공한다. (공식: [github.com/iofoundry/ontology](https://github.com/iofoundry/ontology))

**중요 — IOF 도입이 무조건 좋은 것은 아니다.** 채택 여부 자체가 설계 방법론의 일부다.
...
IOF의 비용은 **BFO 학습 곡선**(연속자/발생자 등 철학적 구분)과 현장 개념을 표준 틀에 매핑하는 공수다. IOF Core도 아직 성숙 단계(v1 beta 수준)이고 상용 도입 사례가 적다.

> **🏭 권고**: 두산 계열사가 내부 AI 데이터 준비를 목적으로 처음 구축한다면, **IOF를 전면 즉시 채택하기보다 IOF 어휘를 참조하되 기업 맞춤 온톨로지로 시작**하고, 외부 연계 필요성이 생길 때 IOF 정렬을 점진적으로 확대하는 접근이 현실적이다.
...
| 항목 | 확인 질문 |
|---|---|
| 다중 홉 지연 | 3·5·10홉 탐색 쿼리의 응답 시간이 허용 범위인가(실시간 시 목표 설정) |
...
> 개별 도구의 표현력·추론 지원·현업 접근성·기존 데이터 연동·운영 부담은 위 **6.2~6.6의 아키텍처 결정**과 직접 연결된다(형식→추론 필요→저장소→진단). 도구별 상세 비교는 [[Appendix B]](#appendix-b-솔루션-상세-비교표)를 참고한다.
...
| **Property Graph / LPG** | (레이블) 속성 그래프 | 노드(Node)와 엣지(Edge)에 속성값을 직접 붙일 수 있는 그래프 모델. Neo4j 등이 대표적. RDF보다 단순하고 빠르지만 추론 기능은 약하다 | [neo4j.com](https://neo4j.com/product/neo4j-graph-database/) |
| **openCypher / Cypher** | 오픈사이퍼 / 사이퍼 | LPG(속성 그래프)를 검색하는 쿼리 언어. Neo4j가 만들어 오픈소스로 공개, 가장 널리 쓰임 | [opencypher.org](https://opencypher.org/) |
| **GQL** | Graph Query Language (ISO/IEC 39075) | 2024년 발행된 최초의 국제표준 그래프 쿼리 언어. Cypher를 모태로 설계 | [gqlstandards.org](https://www.gqlstandards.org/) |
| **T-Box / A-Box** | 개념 스키마 / 인스턴스 데이터 | T-Box=개념·관계를 정의한 설계 규칙(작음), A-Box=그 규칙에 맞춰 채운 실제 데이터(큼). 추론으로 A-Box를 풍부하게 만들 수 있다 | [w3.org/TR/owl2-primer](https://www.w3.org/TR/owl2-primer/) |
| **Materialization** | (추론) 사전 적재 | 추론으로 도출되는 사실을 적재 시점에 미리 계산해 저장하는 방식. 쿼리는 빨라지나 용량↑·갱신 비용↑. 반대는 질의 시 추론 | [ontotext.com](https://www.ontotext.com/knowledgehub/fundamentals/what-is-inference/) |
| **IOF / BFO** | 산업 온톨로지 파운드리 / 기초 형식 온톨로지 | IOF=OAGi가 운영하는 제조 표준 참조 온톨로지 모음. BFO(ISO/IEC 21838-2)는 IOF가 올라타는 최상위 온톨로지. 도입 여부는 외부 연계·팀 역량으로 판단(§6.5) | [github.com/iofoundry](https://github.com/iofoundry/ontology) |
...
| [LPG vs. RDF — Memgraph Docs](https://memgraph.com/docs/data-modeling/graph-data-model/lpg-vs-rdf) | memgraph.com | 형식 비교·다중 홉 성능 (§6.2) |
| [RDF vs Property Graph — TigerGraph](https://www.tigergraph.com/blog/rdf-vs-property-graph-choosing-the-right-foundation-for-knowledge-graphs/) | tigergraph.com | 형식 선택 기준 (§6.2) |
| [Build knowledge graphs with RDF and openCypher — AWS](https://aws.amazon.com/blogs/database/build-and-deploy-knowledge-graphs-faster-with-rdf-and-opencypher/) | aws.amazon.com | 형식이 DB에 선행함 (§6.1·6.2) |
| [SPARQL 1.1 Query Language — W3C](https://www.w3.org/TR/sparql11-query/) | w3.org | RDF 쿼리 언어 표준 (§6.2) |
| [openCypher](https://opencypher.org/) · [GQL Standards](https://www.gqlstandards.org/) | opencypher.org / gqlstandards.org | LPG 쿼리 언어·ISO GQL (§6.2) |
| [What Is Inference? — Ontotext](https://www.ontotext.com/knowledgehub/fundamentals/what-is-inference/) | ontotext.com | T-Box/A-Box·사전적재 vs 질의시추론 (§6.4) |
| [Industrial Ontologies Foundry — GitHub](https://github.com/iofoundry/ontology) · [NIST IOF Core](https://www.nist.gov/publications/industrial-ontologies-foundry-iof-core-ontology) | github.com / nist.gov | IOF·BFO 표준 프레임워크 (§6.5) |
| [Neo4j Security](https://neo4j.com/product/neo4j-graph-database/security/) · [Neo4j Pricing](https://neo4j.com/pricing/) | neo4j.com | 보안·비용 체크리스트 근거 (§6.6) |
...
| 2026-06-18 | 0.2 | 고객/허훈석 컨설턴트 — "업무·데이터 파악에 더해 **아키텍처를 어떤 기준으로 선택했는지**가 방법론으로 정리돼야 한다" 외 7건 | 기존 §6 솔루션·도구 검토를 **§6 아키텍처 설계·선택 방법론**으로 확장: ① 결정 순서(형식→저장소→쿼리→추론적재→표준→진단) ② LPG vs RDF 형식 선정(DB에 선행)·쿼리 언어·**커니 LPG 채택/RDF 배제 결정 박스(허훈석 코멘트 포함)** ③ RDB vs Graph DB 혼용 ④ T-Box/A-Box 추론 적재 트레이드오프 ⑤ IOF 적용 판단 ⑥ 보안·성능·비용 진단 체크리스트. 기존 도구 유형·조합은 6.7~6.8로 이동, 별첨 A에 LPG·openCypher·GQL·T-Box/A-Box·IOF·BFO 풀이 추가 | §6 전체, 별첨 A, 참고자료 |
