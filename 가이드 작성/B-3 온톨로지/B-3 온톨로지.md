...
🏭 **시나리오 — 용접 라인 결함:** 최종 검사에서 표면 박리(desoldering)가 검출됐다. 왜 발생했나?

실제 인과 사슬(용접 품질 지식그래프 특허 CN 114943415A 기반 [[src-250]]):

> 판재 표면 주름 → 용접점 접촉 저항 증가 → 같은 전류 I에서 발열량 증가(공식 E = I²RT) → 너깃 과열 → 박리

이 사슬은 **다섯 개의 서로 다른 데이터 원천**에 걸쳐 있다 — 입고 QC(표면 주름 측정), MES 센서(너깃 온도), 엔지니어링 매뉴얼(물리 공식), 설비 로그(접촉 저항), 검사 보고서(박리 결함).

어떤 텍스트 덩어리 하나도 전체 사슬을 담고 있지 않다. 벡터 임베딩에서 "박리"와 "판재 주름"의 의미적 거리는 거의 0에 가깝다 — 둘은 완전히 다른 어휘를 쓰기 때문이다. 지식그래프에 명시된 `causes`·`relatedParameter` 같은 관계만이 이 사슬을 **한 번의 쿼리로** 따라갈 수 있다.
...
| 질문 | 온톨로지 없음 | 온톨로지 있음 |
|---|---|---|
| "이 결함은 무슨 유형인가?" | 전문(full-text) 검색 | 계층 탐색: 박리 → 접합불량 → 용접결함 |
| "원인이 뭔가?" | PFMEA 키워드 매칭 | `hasCause` 관계를 직접 따라가 접촉저항이상으로 |
| "관련 공정 파라미터는?" | 분석가 수작업 | `relatedParameter` 따라가 입고 판재 표면 거칠기로 |
| "조치는?" | C/S Report 검색 | `hasCorrectiveAction` 따라가 캡 교체 SOP #WELD-047로 |
| "왜 라인3이 라인1·2보다 나쁜가?" | 답할 수 없음 | `hasEquipment` 따라가 용접건 G07 상태=캡마모 HIGH 확인 |

**동료심사(peer-reviewed) 근거:** 제조 FMEA 고장 원인 식별을 다룬 2025년 arXiv 사전논문(arXiv [2510.15428](https://arxiv.org/abs/2510.15428), 2025-10 제출)은 관계형·다중 홉 질문에서 **F1@20이 0.267(표준 RAG)에서 0.523(온톨로지 가이드 지식그래프)으로 약 2배 향상**됐다고 보고했다. 현 시점 이 주장에 대한 가장 강한 독립 검증 수치이나, 공식 출판본을 확인한 뒤 정식 인용하는 것을 권한다.

검색 정확도를 다룬 비교 연구(arXiv [2511.05991](https://arxiv.org/html/2511.05991v1))는 온톨로지 KG + 텍스트 청크가 정답률 **90%**, 벡터 RAG가 **60%**로 **+30%p** 차이를 보고했다. *단, 평가 셋이 작아(n=20) 방향성 근거로만 보고, 자체 도메인 PoC로 기준선을 측정해야 한다.*
...
| **C. AI 에이전트가 여러 원천을 가로질러 분석·추천해야 한다** | 한 시스템에 갇힌 에이전트는 MES↔PFMEA↔정비 이력을 교차 참조 못 함 |
...
> 👉 "이번 달 라인3 박리율이 라인1·2보다 왜 높은가?" — 온톨로지가 있을 때 무엇이 달라지는지 두산밥캣 용접 품질 사례로 본다.

**상황:** 밥캣 유압 암 조립 공장. 용접 라인은 교대당 800~1,200대를 생산한다. 품질 엔지니어가 이번 달 라인3 박리율이 3.2%로 상승(라인1·2는 0.8%)한 것을 발견하고 AI 에이전트에 질문한다.

### 5.1 적용 전/후 대비

**Before (온톨로지 없음)** — AI는 단절된 3개 저장소에 벡터 유사도 검색만 한다.
- PFMEA 파일: "박리" 항목 → 일반 원인 나열("접촉 저항·전극 상태 확인")
- MES: PFMEA와 공유 스키마 없음 → 라인3 파라미터 로그 교차 참조 불가
- 설비 로그: 별도 시스템, "박리"와 개념 다리 없음 → 아예 검색 안 됨

> **에이전트 답변:** "박리는 접촉 저항 부족이나 전극 캡 마모로 생길 수 있습니다. SOP-WELD-047 §3.2를 확인하세요." (일반론 — 어느 라인이 문제든 동일한 답)

**After (온톨로지 + 지식그래프)** — 같은 질문. T-Box에 다음이 선언돼 있다:
1. `박리 ⊏ 접합불량 ⊏ 용접결함` (계층)
2. `전극캡마모 –causes→ 접촉저항증가 –causes→ 너깃과열 –causes→ 박리` (4홉 인과 사슬)
3. `용접건 –정비주기→ 캡교체간격` (속성-관계 연결)
4. `MES:용접라인 –hasEquipment→ 용접건` (시스템 간 다리)

AI가 그래프를 탐색한다(LPG 그래프 DB의 openCypher 쿼리):
```cypher
...
**검색 결과:**
- 라인3 용접건 G07: `전극캡마모 = HIGH` (마지막 교체 후 38일, 임계 30일)
- 라인1·2: 용접건 G03·G05 → `전극캡마모 = NORMAL` (12일·19일 전 교체)

> **에이전트 답변:** "라인3의 박리율 상승(3.2% vs 기준 0.8%)은 용접건 G07의 높은 전극 캡 마모와 관련됩니다. 캡은 38일 전 교체됐고 교체 임계는 30일입니다. 권장 조치: 즉시 캡 교체 — SOP-WELD-047 §3.2. 라인1·2는 정비 주기 내입니다."

### 5.2 온톨로지가 구체적으로 가능케 한 것

| 단계 | 온톨로지가 선언한 것 | AI가 할 수 있게 된 것 |
|---|---|---|
| MES 라인3 ↔ 용접건 G07 연결 | `용접라인 –hasEquipment→ 용접건` | SQL 조인 없이 그래프 탐색으로 시스템 간 결합 |
| 캡 마모 → 박리 연결 | T-Box의 4홉 `causes` 사슬 | 한 쿼리로 인과 경로 탐색 |
| 라인3 vs 라인1·2 비교 | 모든 라인이 공유하는 `용접건` 클래스 | 라인 간 균일 비교 |
| SOP 조치 검색 | `용접결함 –hasCorrectiveAction→ SOP_조치` | 출처 링크와 함께 조치 직접 반환 |

**PoC 목표 예시(실제 PoC에서 자체 설정):** 1군 결함의 평균 수동 근본원인 분석 시간을 약 4시간 → 약 20분으로 단축. 현 분석가 기준선 대비 PoC에서 측정. [as-is에서 채움]
...
이 트리플들이 그래프 DB에 누적되면 AI가 "표면균열 발생 → 열처리불량이 원인(31/48건) → 열처리조건재설정으로 조치(성공률 87% — **예시 수치**, 실제 구축 시 측정값으로 대체)"를 추론할 수 있다.

> 이 탐색을 효율적으로 만든 아키텍처(RDB+Graph DB 폴리글랏 위 LPG, Cypher 쿼리)는 [§6](#6-아키텍처-설계선택-방법론), 이 T-Box를 만든 구축 절차는 [§7](#7-구축)에서 다룬다. 맨 끝의 별도 계열사 예시는 두지 않는다(중복 방지).
...
- [CN Patent 114943415A — 금속 용접 결함 근본원인 지식그래프](https://patents.google.com/patent/CN114943415A/en) (§2.2 인과 사슬)
