...
> 용어 — **노드**는 업무의 한 개념(고객·제품 등), **관계**는 두 개념의 연결(고객—주문—제품), **속성**은 노드에 딸린 값(고객명·제품코드)이다.

---

## 0. 이 문서의 위치 — 정론과 각론

메인 가이드(정론)는 "온톨로지가 무엇이고, 어떤 구성요소로, 어떤 9단계 절차로 만드는가"를 다룬다. 그 절차를 따라가다 보면 **노드 하나, 관계 하나를 둘지 말지·어디에 붙일지**를 매번 판단해야 한다. 이 각론은 그 **판단 기준**이다 — 정론이 "무엇을 해야 하나"라면, 각론은 "이 결정을 무슨 기준으로 내리나"에 답한다.

```
[정론] 메인 가이드 §3 구성요소 · §4 9단계 절차      ← 무엇을 해야 하나
   │  (이 문서가 그 절차의 판단 기준을 채운다)
[각론] 설계·운영 원칙 (이 문서)                      ← 각 결정을 무슨 기준으로
   │
[산출물] 별첨 B 코어 기획서 · 별첨 C 유즈케이스 기획서   ← 그 기준으로 채운 결과
```

> 정론을 모르면 먼저 [메인 가이드 `B-3 온톨로지.md`](../B-3%20온톨로지.md)를 본다. 설계 결과를 적어 넣는 빈 양식은 [별첨 B 코어 기획서](B-3%20별첨%20B%20—%20코어%20온톨로지%20설계%20기획서.md)·[별첨 C 유즈케이스 기획서](B-3%20별첨%20C%20—%20유즈케이스%20레이어%20설계%20기획서.md)이고, 그 앞단의 현실 정리는 [별첨 E As-Is 분석서](B-3%20별첨%20E%20—%20As-Is%20분석서.md)·[별첨 D Discovery Workshop](B-3%20별첨%20D%20—%20Discovery%20Workshop%20운영%20가이드.md)다.
...
> 메인/서브 구분은 본문 [§3 구성요소](../B-3%20온톨로지.md#sec-what)의 핵심 클래스와 그 하위·세부에 대응한다.
...
> 현장에서 이런 암묵지를 끌어내는 진행법은 [별첨 D Discovery Workshop](B-3%20별첨%20D%20—%20Discovery%20Workshop%20운영%20가이드.md)에 있다.
...
> "근거 없는 개념·관계는 만들지 않는다"는 본문 [§출발점 As-Is](../B-3%20온톨로지.md#sec-asis)의 원칙과 같다 — 데이터 소스가 확인 안 된 것은 [별첨 E](B-3%20별첨%20E%20—%20As-Is%20분석서.md)에 "미확인"으로 남긴다.
...
> 이 "구조는 그래프, 값은 데이터베이스" 구분은 본문 [§3 구성요소](../B-3%20온톨로지.md#sec-what)의 설계도(T-Box) vs 사례 데이터(A-Box) 구분과 같다.
...
> 이 온톨로지 스키마가 본문에서 말하는 설계도(T-Box)다. 코어 설계의 스키마 결과는 [별첨 B 코어 기획서](B-3%20별첨%20B%20—%20코어%20온톨로지%20설계%20기획서.md)에 적어 둔다.
...
| 절 | 점검 항목 |
|---|---|
| 1.1 | 그 자체로 식별되는 개념인가? 메인·서브 중 무엇이며, 메인이면 다른 핵심 개념과 둘 이상 이어지는가? 식별 키를 정했는가? |
| 1.2 | 서브 구조가 목적(핵심 질의)에 답하는 데 쓰이는가? 서브면 메인 하나에 종속되는가? |
| 1.3 | 어느 데이터·항목으로 잇는가? (데이터로 안 나오는 지식이면 인터뷰·문서를 근거로 출처를 적었나?) 사건성 사실을 사건 노드를 거쳐 표현했나? 성격이 다른 노드를 합치지 않았나? 같은 대상인가? 중복이면 이유를 스키마에 적었나? 이름·방향·순서가 맞나? |
| 1.4 | 값(측정·거래)을 노드로 만들고 있지 않은가? |
| 1.5 | 끊긴 데가 생기지 않는가? 같은 개념을 같은 이름으로, 정해진 표기 규칙으로 부르는가? |
| 2 | 온톨로지 스키마에 등록했는가? 검증·테스트를 거쳤는가? 백업이 있는가? |
...
- 설계 결과를 적는 빈 양식: [별첨 B 코어 기획서](B-3%20별첨%20B%20—%20코어%20온톨로지%20설계%20기획서.md) · [별첨 C 유즈케이스 기획서](B-3%20별첨%20C%20—%20유즈케이스%20레이어%20설계%20기획서.md)
- 그 앞단(현실 정리·현장 수집): [별첨 E As-Is 분석서](B-3%20별첨%20E%20—%20As-Is%20분석서.md) · [별첨 D Discovery Workshop](B-3%20별첨%20D%20—%20Discovery%20Workshop%20운영%20가이드.md)
- 표준 용어: [A-3 Glossary](../../A-3%20Glossary/A-3%20Glossary.md)
- 원천(내부): 두산 「온톨로지 설계 원칙」·「온톨로지 구축 방법론 요약」
