...
**👉 한 줄 요약:** PDF·PPT·Excel·이미지는 내부 구조가 전혀 다르므로 **유형마다 무엇을 어떻게 뽑을지가 다르다** — 특히 "표"를 살리는 일이 유형마다 어렵다.

| 유형 | 무엇을 추출하나 | 핵심 난점 (왜 어렵나) |
|---|---|---|
| **PDF** | 문단·표·이미지·페이지 구조 | 인쇄 레이아웃 기반이라 읽기 순서·표 구조가 파일에 없음. 다단(多段) 혼합, 테두리 없는 표, 스캔본은 OCR 필요 |
| **PPT(X)** | 슬라이드 제목·본문·도형 텍스트·표·노트 | 텍스트박스 순서(Z-order)가 뒤죽박죽. 도형·차트 안 텍스트는 별도 처리 |
| **Excel(XLSX)** | 시트·셀·수식·병합셀 구조 | 병합셀은 첫 칸에만 값(나머지 빈칸). 색·테두리에 담긴 의미는 자동 인식 불가. 여러 시트 연관은 수동 정의 |
| **이미지/스캔본** | 이미지 속 텍스트·표 | OCR(글자 인식) + TSR(표 구조 인식) 필요. 기울기·저해상도가 정확도 떨어뜨림 |
...
같은 PDF라도 **단순 추출은 표를 뭉개고, 레이아웃 인식 파싱은 행/열을 살린다.** (유형별 추출 기법·라이브러리 상세 → [Backup 3-A](#backup-3-a-문서-유형별-추출-기법-상세))
