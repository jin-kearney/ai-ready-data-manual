...
Tool 명세는 에이전트가 외부 시스템·데이터를 **읽거나 바꿔야 할 때** 만든다. 문서를 요약하거나 사내 자료를 찾아 답하는 정도라면 Tool 명세가 아니라 전처리·검색 자료를 준비하면 된다(이는 [B-1 데이터 전처리](../B-1%20데이터%20전처리/B-1%20데이터%20전처리.md)와 A 그룹이 맡는다). 즉 에이전트가 단순히 말로 답하는 데서 그치지 않고, 시스템을 직접 조회·실행해야 할 때가 적용 시점이다.
