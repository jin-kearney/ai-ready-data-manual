...
합성데이터 구축은 일곱 단계로 이루어진다. 이 **정본 7단계**는 이 가이드 전체에서 일관된 기준으로 사용된다.
...
합성데이터 체계는 단순히 데이터를 생성하는 기술이 아니다. 무엇을 합성할지(대상), 어떻게 만들지(생성 방식), 잘 만들었는지 어떻게 확인할지(검증)라는 세 가지 구성 요소를 함께 갖추어야 한다.
...
합성데이터 구축은 단순히 도구를 실행해 데이터를 생성하는 작업이 아니다. 생성 목적을 정의하고 보존할 특성을 결정한 뒤 적절한 방식을 선택하고, 검증과 위험 관리를 거쳐 운영 체계까지 구축해야 한다. 합성데이터의 품질은 생성 모델 자체의 수준보다 이 절차를 얼마나 체계적으로 수행했는지에 의해 결정된다.
...
합성데이터는 생성 자체보다 생성 이후의 관리가 더 중요하다. 잘못 운영된 합성데이터는 AI 모델을 오히려 더 나쁜 방향으로 이끌 수 있다.
