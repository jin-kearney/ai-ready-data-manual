...
관측 체계를 지속적으로 운영하기 위해 아래를 정의한다.

| 운영 기준 | 무엇을 지키나 |
|------|-------------|
| 관측 대상 | AI 활용 핵심 데이터 흐름을 관측 대상으로 등록한다 |
| 이상 판단·알림 | 고정 한계값과 평소 추세로 이상을 판단하고, 심각도별 알림 경로를 운영한다 |
| 데이터 오너십 | 관측 대상마다 이상 대응을 책임지는 데이터 오너를 지정한다 |
| 역할·책임(R&R) | 이상 감지·알림·복구의 역할과 책임을 구분한다 |
| 생애주기(Lifecycle) | 데이터 생성부터 폐기까지 단계별 관측 기준을 정의한다 |
