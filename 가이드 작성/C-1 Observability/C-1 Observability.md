...
건설·산업 장비를 만들어 전 세계 현장에 공급하는 계열사의 장비 텔레매틱스 수집 파이프라인을 가상 예시로 든다. 현장의 장비가 엔진온도·유압·연료·가동시간을 1분 주기로 클라우드에 보내고, 그 데이터가 데이터 레이크와 피처 테이블을 거쳐 예지보전 AI로 흐른다. 어느 새벽, 한 지역의 통신 장애로 수집이 끊긴 상황을 비교한다.
...
구축 절차의 ②·③단계에서는 4축마다 거는 지표와 판단 방식·정상 기준·심각도를 정한다. 관측 대상 흐름 하나를 등록할 때 아래 항목을 채워 알림 규칙을 만든다. 빈 템플릿을 그대로 복사해 흐름마다 1건씩 작성한다.

| 관측 축 | 점검 지표 | 판단 방식 | 정상 기준(예시값) | 이상 시 심각도 |
|---|---|---|---|---|
| 최신성 | 마지막 적재 후 경과 시간 | 고정 한계값 | 5분 이내 | 긴급 |
| 양 | 시간당 적재 건수 | 평소 추세 | 평소 범위 ±30% | 주의 |
| 구조 | 컬럼·타입·단위 변경 | 변경 감지 | 변경 없음 | 주의 |
| 값 분포 | 결측률 / 범위 이탈 | 평소 추세 + 물리 한계 | 결측 5% 이내, 음수 없음 | 경고 |

빈 템플릿:

```text
대상 흐름: ____________________ (예: 텔레매틱스 수집 → 피처 테이블)
[최신성]  지표:________  방식: 고정/추세  정상기준:________  심각도:____
[양]      지표:________  방식: 고정/추세  정상기준:________  심각도:____
[구조]    지표:________  방식: 변경감지    정상기준:________  심각도:____
[값 분포] 지표:________  방식: 고정/추세  정상기준:________  심각도:____
```
