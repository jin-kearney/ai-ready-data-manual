...
| Observability 운영 수준을 어떻게 측정할 것인가? | 구축(관측 커버리지)·운영(이상 감지 시간·놓친 이상 건수)·운영 품질(오탐률) 세 층의 지표로 잰다 | [7](#kq5) |
...
데이터 품질·출처·변경 이력·계보(Lineage) 관리 체계를 통해 AI 활용 시 데이터 신뢰성 저하와 Hallucination을 방지하려면, 관측이 무엇을 지켜야 하는지(운영 기준)와 그것이 잘 적용되는지(KPI)를 함께 정한다.
...
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','clusterBkg':'#EEF4FB','clusterBorder':'#6B9AD1','fontSize':'13px'}}}%%
flowchart LR
    STD["운영 기준 정의<br/>관측 대상·이상 판단·알림<br/>+ 오너십·R&R·생애주기"]:::std
    T1["구축<br/>관측 커버리지<br/>운영 기준 정의율"]:::t1
    T2["운영<br/>이상 감지 시간<br/>놓친 이상 건수"]:::t2
    T3["운영 품질<br/>오탐률"]:::t3
...
운영 기준은 관측을 누가·무엇을·어느 단계까지 책임지고 관리하는지 정한 약속이다. 아래 세 가지를 정의한다.

**데이터 오너십** — 관측 대상 데이터마다 데이터 오너 1명을 지정한다. 오너는 그 데이터의 정상 기준(무엇이 정상 범위인가)을 승인하고, 이상이 확인되면 업무 조치 여부를 최종 판단한다.

**역할·책임(R&R)** — 이상 감지부터 복구까지의 책임을 아래와 같이 나눈다.

| 역할 | 책임 |
|------|------|
| 데이터 오너 (현업 부서) | 정상 기준 승인, 이상 시 최종 판단·업무 조치 |
| 관측 담당 (데이터·플랫폼팀) | 관측 규칙·임계값 설정, 알림 운영, 이상 1차 확인 |
| 운영 담당 (IT 운영) | 멈춘 파이프라인 복구·재처리 |

**생애주기(Lifecycle)** — 데이터가 수집→적재→활용→보관/폐기를 거치는 동안 관측을 유지한다. 신규·변경 데이터는 평소 추세가 쌓일 때까지 기준선 학습 기간을 따로 두고, 폐기 예정 데이터는 관측 대상에서 정리한다.

<a id="s72"></a>

### 7.2 핵심 KPI

운영 기준이 잘 적용되는지 아래 지표로 점검한다. 구축(얼마나 넓게 갖췄나)→운영(얼마나 빨리 잡나)→운영 품질(알림을 믿을 수 있나) 순으로 본다.

| 구분 | 지표 | 무엇을 재나 | 산식 | 목표 방향 |
|------|------|-------------|------|-----------|
| 구축 | 관측 커버리지 | 핵심 데이터 흐름 중 관측이 적용된 비율 | 관측 등록 파이프라인 / 전체 핵심 파이프라인 | 높을수록 |
| 구축 | 운영 기준 정의율 | 관측 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관측 대상 | 높을수록 |
| 운영 | 이상 감지 시간(MTTD) | 이상 발생부터 알림까지 걸린 평균 시간 | 평균(알림 시각 − 이상 발생 시각) | 짧을수록 |
| 운영 | 놓친 이상 건수 | 관측이 못 잡고 다른 경로로 발견된 이상 건수 | 기간 내 외부 경로로 발견된 이상 건수 | 적을수록 |
| 운영 품질 | 오탐률 | 전체 알림 중 실제 이상이 아닌 알림 비율 | (전체 알림 − 실제 이상) / 전체 알림 | 낮을수록 |

절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다. 오탐률과 놓친 이상 건수는 한쪽을 무리하게 낮추면 다른 쪽이 늘 수 있어 함께 본다.
