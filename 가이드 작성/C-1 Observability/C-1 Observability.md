...
| 핵심 질문 | 한 줄 답 | 본문 |
|---|---|---|
| 어떤 데이터 이상을 관찰해야 하는가? | 최신성·양·구조·값 분포 4축을 정해 두고 각 축마다 지표를 설정한다 | [2.2](#kq1) |
...
이 네 가지 축은 데이터 관측 업계에서 흔히 쓰는 다섯 기둥(5 Pillars) 가운데 네 가지다[1]. 다섯 번째 기둥인 계통은(Lineage) [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md)가 별도로 맡으므로 관측 지표에서는 제외하되, C-1은 이상이 발생했을 때 그 데이터가 어디로 흘러가 어떤 영향을 주는지 파악할 때 계보를 활용한다([2.4](#s24)).

네 축이 흐르는 데이터 위에서 동시에 켜져 있는 모습은 관제 화면 한 장으로 표현할 수 있다.
...
| 구분 | 적용 전 | 적용 후 |
|---|---|---|
| 수집 중단 감지 | 아침 출근 후 대시보드가 비어 있는 것을 보고 약 7시간 뒤에 인지 | 새 레코드 유입 중단 후 즉시 최신성 축 경보 발생 |
| AI 분석 | 전날의 정상 데이터를 기준으로 분석을 계속해 점검 시점을 놓침 | 최신성 이상을 감지하면 해당 AI 분석 결과를 자동 보류하고 경고 제공 |
| 원인 파악 | 데이터·모델·통신 중 어디가 문제인지 며칠간 추적 | 최신성 축만 이상, 값 분포는 정상 → 데이터 수집 단계 문제로 즉시 범위 축소 |
...
네 축의 감지·알림은 원리가 동일하다. 관측 솔루션이 각 축의 지표를 **주기적으로 계산**해 **정상 기준과 비교**하고, 기준을 벗어나면 해당 축에 지정한 **심각도로 경보**를 올려 [2.4](#s24)의 경로로 발송한다. 축마다 다른 점은은 "무엇을·언제 확인해 무엇과 비교하는가"뿐이다. 흐름 하나를 등록할 때 4축을 아래와 같인 정의한다.

| 관측 축 | 감지 항목 및 시점 | 판단 방식 | 정상 기준(예시값) | 이상 시 심각도 |
|---|---|---|---|---|
...
| 전처리 산출물이 운영 중에도 유효한지 관측 | [B-1 데이터 전처리](../B-1%20데이터%20전처리/B-1%20데이터%20전처리.md) | 비정형 데이터를 AI가 읽을 구조로 변환 | 양식 변경으로 전처리가 훼손되면 C-1 구조·분포 축이 감지 |
| A-2 기준으로 이상 판단 | [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md) | 필드·단위·갱신 주기 등 속성 정의 | A-2가 정한 단위·주기·허용값이 이상 판단 기준 |

신뢰(Trustworthy) 세 축의 관계는 다음과 같다. 관측(C-1)은 운영 중 실시간 신호를 띄우고 계통(C-3)은 사후 이력 근거를 대며, 품질 게이트(C-2)가 그 둘을 근거로 AI 투입 가부를 판정한다.
...
이 절은 데이터 관측이 실제로 잘 되고 있는지 숫자로 확인하는 KPI를 정의한다. 지표의 오너십·역할과 책임은 4장(운영)과 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)에서, 데이터 보존·폐기 시간축은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다.
...
| 이상 감지 시간(MTTD) | 문제를 얼마나 빨리 잡는지 본다. 늦으면 잘못된 데이터가 AI로 흘러간다 | 평균(알림 시각 − 이상 발생 시각) · 월 집계 | 기준선 대비 단축, 심각도별 SLA 합의 |
| 오탐률 | 알림을 믿을 수 있는지 본다. 오탐이 잦으면 담당자가 알림을 무시한다 | (전체 알림 − 실제 이상) / 전체 알림 · 월 집계 | 담당자가 감당 가능한 수준으로 SLA 합의 |

오탐률은 놓친 이상(관측이 못 잡고 다른 경로로 발견된 건)과 함께 본다 — 한쪽을 무리하게 낮추면 다른 쪽이 늘기 때문이다. 운영이 성숙하면 해결 시간(MTTR)·관측 자동화율 등으로 넓힌다.
