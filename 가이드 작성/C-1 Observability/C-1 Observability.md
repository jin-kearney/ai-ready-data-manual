...
데이터 관측은 데이터를 신뢰할 수 있게(Trustworthy) 만드는 단계에 속한다. 사용 여부를 판정하는 [C-2 데이터 품질 관리](../C-2%20데이터%20품질%20관리/C-2%20데이터%20품질%20관리.md)가 상위의 관문이라면, C-1은 운영 중 실시간 신호로, [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md)는 사후 이력으로 그 판정에 근거를 뒷받침한다. C-1이 보는 것은 "지금 이 순간 흐름이 정상인가"이고, 다른 주제와의 경계는 [6장](#where)에서 정리한다.
...
C-1은 이 그림의 왼쪽 아래 신호원이다. 흐름이 멈췄거나 틀어졌다는 신호를 가장 먼저 띄워, 품질 게이트와 후속 대응이 손쓰게 한다.
...
- **③ 알림 규칙 설정.** 4축마다 지표를 설정하고, 이상 시 심각도 등급·경로를 [2.4](#s24)의 표를 참조하여 지정한다.
- **④ 시범 운영.** 며칠~몇 주 시범으로 돌리며 오탐을 본다. 너무 자주 울리면 기준을 느슨하게, 놓친 이상이 있으면 더 민감하게 조정한 뒤 본 운영으로 넘긴다. 운영하며 기준을 다듬는 되돌림 고리는 [4.4](#s44)에서 이어진다.

> **권장 — 한 흐름으로 끝까지 먼저.** 처음부터 모든 흐름의 규칙을 채우려 하지 않는다. [3장](#when)에서 고른 1순위 흐름 하나에 위 네 단계(연결→기준→알림→시범 운영)를 4축 모두 걸어 끝까지 돌려 본다. 그 한 흐름에서 오탐을 잡고 경보가 실제 경로로 도달하는지 확인한 뒤, 같은 방식([4.3](#s43) 템플릿)을 다음 흐름으로 넓힌다. 관측은 "전 데이터에 한꺼번에"가 아니라 "핵심 흐름 하나부터 끝까지"가 빠르다.
...
이 표는 규칙을 정하는 **설계서**다. 여기서 정한 값을 관측 솔루션([5장](#tech))의 알림 규칙 화면 또는 설정 파일에 그대로 입력하면 실제 감지 규칙이 된다. 전용 관측 솔루션은 "평소 추세" 기준선을 과거 데이터로 자동 학습하므로, 추세 방식 축은 값을 직접 넣기보다 학습된 범위를 확인·조정한다([4.2](#s42) ②단계). 빈 템플릿을 복사해 흐름마다 1건씩 작성한다.

빈 템플릿:

```text
대상 흐름: ____________________ (예: 텔레매틱스 수집 → 피처 테이블)
[최신성]  확인: 마지막 적재 시각 ___분마다   방식: 고정/추세  정상기준:________  심각도:____
[양]      확인: ___시간당 적재 건수          방식: 고정/추세  정상기준:________  심각도:____
[구조]    확인: 컬럼·타입·단위 직전과 대조    방식: 변경감지    정상기준:________  심각도:____
[값 분포] 확인: 결측률 / 범위 이탈 비율       방식: 고정/추세  정상기준:________  심각도:____
```

이 설계서는 종이에서 끝나지 않고 실제 솔루션의 설정으로 옮겨진다. 대표적인 전용 관측 솔루션([Monte Carlo](https://montecarlo.ai)[2]·[Bigeye](https://www.bigeye.com)[4]·[Soda](https://www.soda.io)[5])에서는 아래 흐름으로 들어가며, 종이 항목 하나하나가 화면 설정 또는 설정 파일 한 줄에 대응한다.

1. **원천에 커넥터로 연결.** 데이터 웨어하우스·레이크(Snowflake·Databricks·S3 등)에 읽기 전용 커넥터로 붙는다 — 설계서의 "대상 흐름".
2. **관측할 테이블·컬럼 선택.** 붙은 원천에서 텔레매틱스 피처 테이블을 고른다.
3. **기준선 잡기.** 자동 학습형(Monte Carlo·Bigeye 등)은 과거 데이터로 최신성·양·값 분포의 평소 범위를 스스로 학습해 임계값을 만들고, 사람은 확인·미세조정만 한다. 고정값이 분명한 축(최신성 5분, 음수 금지 등)만 직접 입력한다 — 설계서의 "방식·정상 기준".
4. **알림 채널·심각도 연결.** 축별 심각도를 Slack·Teams·PagerDuty 등 채널에 연결한다(긴급=호출 채널, 경고=로그 채널) — 설계서의 "심각도" + [2.4](#s24).
5. **시범 운영.** 며칠 돌려 오탐을 보고 조정한다([4.2](#s42) ④).

코드형 솔루션([Soda](https://www.soda.io)[5]·dbt·Great Expectations)은 같은 4축을 설정 파일에 직접 적어 배포한다. 아래는 개념 예시이며(실제 문법은 각 도구 문서 확인), 설계서의 네 줄이 그대로 규칙이 되는 모습이다.

```yaml
# telemetry_features 테이블 관측 규칙 (개념 예시)
checks for telemetry_features:
  - freshness(event_time) < 5m           # 최신성: 5분 넘게 새 데이터 없으면 경보
  - row_count > 0                        # 양: 건수 0/급감 감지 (추세는 자동 학습)
  - missing_percent(hydraulic_kpa) < 5   # 값 분포: 결측 5% 초과 시 경보
  - schema:                              # 구조: 컬럼·타입·단위 변경 감지
      warn: when schema changes
```

자동 학습형은 "화면에서 켜고 학습 결과를 확인", 코드형은 "파일에 적어 배포"라는 차이일 뿐, 정하는 내용(4축·기준·심각도)은 위 설계서와 같다.
...
| 관리 항목 | 관리 기준 | 관리 목적 |
|-----------|-----------|-----------|
| 관측 대상 | AI 활용 핵심 데이터 흐름은 관측 대상으로 등록한다. | 이상 사전 감지 범위 확보 |
| 관측 4축 | 최신성·양·구조·값 분포를 함께 관측한다. | 이상 유형 누락 방지 |
| 이상 판단 기준 | 고정 한계값과 평소 추세 기준을 함께 정의한다. | 감지 정확도 확보 |
| 알림·에스컬레이션 | 심각도 등급과 등급별 경로를 정의하여 운영한다. | 대응 속도 확보 |
| 운영 점검 | 관측 규칙과 알림 운영 현황을 정기적으로 점검한다. | 지속적 관측 품질 개선 |
| 데이터 오너십 | 관측 대상마다 이상 대응을 책임지는 데이터 오너를 지정한다. | 대응 책임 소재 명확화 |
| 역할·책임(R&R) | 이상 감지·알림·복구의 역할과 책임을 구분하여 운영한다. | 운영 책임 체계 확보 |
| 생애주기(Lifecycle) | 데이터 생성부터 폐기까지 단계별 관측 기준을 정의한다. | 전 생애주기 관측 유지 |
...
상시 점검하는 핵심 지표는 다섯 개다. 각 구간(구축·운영·운영 품질)에서 하나씩 잡아, 관측을 얼마나 깔았고·얼마나 빨리 잡으며·얼마나 믿을 만한지를 한 화면으로 본다. 절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세우고 목표 대비로 관리한다. 초기에는 이 다섯 개로 시작하고, 운영이 성숙하면 [별첨 전체 KPI 목록](#kpi-all)까지 넓힌다.

| 구분 | KPI | 정의 | 산식 | 목표 방향 |
|------|------|------|------|-----------|
| 구축 수준 | 관측 커버리지 | 핵심 데이터 흐름 중 관측이 적용된 비율 | 관측 등록 파이프라인 / 전체 핵심 파이프라인 | 높을수록 |
| 구축 수준 | 운영 기준 정의율 | 관측 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관측 대상 | 높을수록 |
| 운영 수준 | 이상 감지 시간(MTTD) | 이상 발생부터 알림까지 평균 소요 시간 | 평균(알림 발송 시각 − 이상 발생 시각) | 짧을수록 |
| 운영 수준 | 놓친 이상 건수 | 관측이 잡지 못하고 다른 경로로 발견된 이상 건수 | 기간 내 외부 경로 발견 이상 건수 | 적을수록 |
| 운영 품질 | 오탐률 | 전체 알림 중 실제 이상이 아닌 알림 비율 | (전체 알림 건수 − 실제 이상 확인 건수) / 전체 알림 건수 | 낮을수록 |

오탐률을 무리하게 낮추면 놓친 이상 건수가 늘 수 있으므로 두 지표는 함께 본다.
