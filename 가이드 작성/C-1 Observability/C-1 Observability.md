...
| 핵심 질문 | 한 줄 답 | 본문 |
|---|---|---|
| 어떤 데이터 이상을 관찰해야 하는가? | 최신성·양·구조·값 분포 네 항목을 정해 두고 각 항목마다 지표를 설정한다 | [2.2](#kq1) |
...
| Observability 운영 수준을 어떻게 측정할 것인가? | 관측 커버리지·이상 감지 시간(MTTD)·오탐률로 점검한다 | [7](#kq5) |
...
이 장은 데이터 관측이 무엇이고 무엇으로 이루어지는지를 정의한다. 구축 절차와 운영 방법은 [4장](#how)에서 다루고, 여기서는 네 가지 구성 요소(관측 4 항목·판단 기준·알림 체계·원인 구분 기록)를 정의한다.
...
데이터 관측은 데이터를 신뢰할 수 있게(Trustworthy) 만드는 단계에서 "지금 이 순간 흐름이 정상인가"를 실시간 신호로 살피는 역할을 맡는다.
...
관측은 단순히 "데이터를 본다"는 의미가 아니라, 데이터 흐름이 훼손되는 **네 가지 주요 항목**을 정의하고 각 지점에 대응하는 지표를 설정하는 것이다. 본 가이드는 이 네 가지를 관측의 정본 모델로 삼아 끝까지 일관되게 쓴다.

| 관측 항목 | 내용 | 이상 신호 (텔레매틱스 예시) |
|---|---|---|
| **① 최신성 (Freshness)** | 데이터가 제때 들어오는가 | 1분 주기로 와야 할 장비 데이터가 15분째 새 레코드 없음 — 수집 중단의 첫 신호 |
| **② 양 (Volume)** | 평소만큼의 건수가 들어오는가 | 하루 5만 건 적재되던 가동 로그가 200건만 적재됨 — 수집 어댑터 장애·지역 단절 |
...
이 네 가지 항목은 데이터 관측 업계에서 흔히 쓰는 다섯 기둥(5 Pillars) 가운데 네 가지다[1]. 다섯 번째 기둥인 계통은(Lineage) [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md)가 별도로 맡으므로 관측 지표에서는 제외하되, C-1은 이상이 발생했을 때 그 데이터가 어디로 흘러가 어떤 영향을 주는지 파악할 때 계보를 활용한다([2.4](#s24)).

네 항목이 흐르는 데이터 위에서 동시에 켜져 있는 모습은 관제 화면 한 장으로 표현할 수 있다.

![관측 4항목 대시보드 모형 — 최신성·양·구조·값 분포 네 패널이 한 화면에 있고, 양(Volume) 은 정상, 최신성은 수집 지연으로 경보가 켜진 가상 관제 화면](이미지/관측-대시보드-모형.svg)
...
고정 한계값과 평소 추세는 서로 배타적인 관계가 아니다. 물리적 한계처럼 명확한 구간은 고정 한계값으로 엄격히 제한하고 변동성이 큰 흐름은 평소 추세를 기준으로 모니터링하는 방식으로 함께 활용한다.
...
기본 대상에 관측을 적용한 이후에는 업무 영향도와 변동 수준을 고려하여 관측 수준을 단계적으로 높인다. 두 가지 기준으로 우선순위를 매긴다. 하나는 그 흐름이 틀어졌을 때의 **업무 영향도**(AI 서비스·안전·품질 판정에 직접 닿는가), 다른 하나는 그 흐름의 **변동·고장 빈도**(원천이 자주 바뀌거나 수집이 자주 끊기는가)다.
...
| 구분 | 적용 전 | 적용 후 |
|---|---|---|
| 수집 중단 감지 | 아침 출근 후 대시보드가 비어 있는 것을 보고 약 7시간 뒤에 인지 | 새 레코드 유입 중단 후 즉시 최신성 경보 발생 |
| AI 분석 | 전날의 정상 데이터를 기준으로 분석을 계속해 점검 시점을 놓침 | 최신성 이상을 감지하면 해당 AI 분석 결과를 자동 보류하고 경고 제공 |
| 원인 파악 | 데이터·모델·통신 중 어디가 문제인지 며칠간 추적 | 최신성 이상, 값 분포는 정상 → 데이터 수집 단계 문제로 즉시 범위 축소 |
...
flowchart LR
    ...
    S3["③ 알림 규칙 설정<br/>항목별 지표·등급·경로"]:::gate
...
| **② 정상 기준선 잡기** | 최근 수 주~수개월 과거 데이터로 "평소" 범위를 잡는다. 고정 한계값이 분명한 항목(최신성 5분·음수 금지)은 값을 직접 넣고, 변동이 큰 항목(양·값 분포)은 학습된 범위를 확인·조정한다([2.3](#s23)). 항목별 세부는 [4.3](#s43). | 항목별 정상 기준선 |
| **③ 알림 규칙 설정** | 4항목마다 지표·정상 기준에 더해 심각도 등급과 수신 경로(누구에게·어느 채널로)를 연결한다([2.4](#s24)). | 항목별 알림 규칙 |
...
> **권장 — 한 흐름으로 끝까지 먼저.** 처음부터 모든 흐름의 규칙을 채우려 하지 않는다. [3장](#when)에서 고른 1순위 흐름 하나에 위 네 단계(연결→기준→알림→시범 운영)를 4항목 모두 걸어 끝까지 돌려 본다. 그 한 흐름에서 오탐을 잡고 경보가 실제 경로로 도달하는지 확인한 뒤, 같은 방식([4.3](#s43))을 다음 흐름으로 넓힌다. 관측은 "전 데이터에 한꺼번에"가 아니라 "핵심 흐름 하나부터 끝까지"가 빠르다.

<a id="s43"></a>

### 4.3 관측 지표·판단 기준 설정

네 항목의 감지·알림은 원리가 동일하다. 관측 솔루션이 각 항목의 지표를 **주기적으로 계산**해 **정상 기준과 비교**하고, 기준을 벗어나면 해당 항목에 지정한 **심각도로 경보**를 올려 [2.4](#s24)의 경로로 발송한다. 항목마다 다른 점은은 "무엇을·언제 확인해 무엇과 비교하는가"뿐이다. 흐름 하나를 등록할 때 4항목을 아래와 같인 정의한다.

| 관측 항목 | 감지 항목 및 시점 | 판단 방식 | 정상 기준(예시값) | 이상 시 심각도 |
|---|---|---|---|---|
...
위의 4항목은 관측 솔루션([5장](#tech))의 설정으로 옮겨야 실제 감지 규칙이 된다. 솔루션이 설정 파일로 관리하는 코드형이냐 화면으로 관리하는 화면형이냐에 따라 옮기는 방식만 다르다.

코드형 솔루션([Soda](https://www.soda.io)[5]·dbt·Great Expectations)은 4항목을 설정 파일에 직접 적어 배포한다. 위 표에서 정한 네 줄은 예시적으로 아래 설정이 된다(실제 문법은 각 도구 문서 확인).
...
화면형 솔루션([Monte Carlo](https://montecarlo.ai)[2]·[Bigeye](https://www.bigeye.com)[4] 등)은 같은 네 가지 항목을 알림 규칙 화면의 항목으로 입력한다. 원천에 읽기 전용 커넥터로 연결해 관측할 테이블을 고른 뒤, 양·값 분포처럼 변동이 큰 항목은 과거 데이터로 학습된 평소 범위를 확인·조정하고, 최신성 5분·음수 금지처럼 값이 분명한 건만 직접 입력한다. 끝으로 각 항목의 심각도를 채널(긴급=호출, 경고=로그)에 연결한다([2.4](#s24)).

두 형태 모두 정하는 내용은 위 표와 같다. 코드형은 파일에 적어 배포하고, 화면형은 화면에서 켜 학습 결과를 확인·조정한다는 차이뿐이다.

<a id="s44"></a>

### 4.4 알림·에스컬레이션 운영

관측 운영의 성패는 **알림 피로를 줄이는 것**에 달려 있다. 사소한 알림이 과도하게 발생하면 팀이 알림으로부터 뮤뎌지게 되고 정작 실제 이상 징후를 놓치게 된다.

- **등급 조정.** 모든 알림을 같은 경로로 보내지 않는다. 긴급만 호출로, 경고는 메신저·로그로 분리한다([2.4](#s24)).
- **중복 묶기(Grouping).** 같은 원인에서 연달아 나는 알림은 하나의 사건으로 묶어 한 번만 알린다.
- **억제 윈도우(Suppression Window).** 계획된 정비·배치 작업 중에는 알림을 잠시 멈춘다. 예컨대 수집 복구 후 누락분을 한꺼번에 재적재하면 양 항목이 급증하는데, 이는 정상이므로 억제한다.
...
| 유형 | 특징 | 대표 솔루션 |
|---|---|---|
| **전용 데이터 관측 솔루션** | 4항목을 자동으로 학습·감지, 계보·알림 연동까지 갖춘 통합 플랫폼 | [Monte Carlo](https://montecarlo.ai)[2] · [Anomalo](https://www.anomalo.com)[3] · [Bigeye](https://www.bigeye.com)[4] · [Soda](https://www.soda.io)[5] · [Metaplane](https://www.metaplane.dev)[6] · [Acceldata](https://www.acceldata.io)[7] · [Sifflet](https://www.siffletdata.com)[8] |
...
전용 솔루션은 임계값을 일일이 설정하지 않아도 평소 추세를 자동 학습해 초기 운영 부담이 낮다. 플랫폼 내장형은 이미 그 플랫폼을 사용한다면 추가 도입 필요 없으나 해당 환경으로만 제한을 받는다. 오픈소스는 비용이 낮지만 알림·운영 화면을 직접 설정해 한다. 분포 드리프트나 예측 품질 관측이 핵심이면 Evidently[17] 같은 도구를 보강으로 활용한다.

<a id="s52"></a>

### 5.2 선정 기준

솔루션은 다음 기준으로 비교한다. 가격·세부 기능은 빈번하게 바뀌므로 단정하지 않고 공식 문서와 PoC(개념 검증)로 확인한다.

| 선정 기준 | 확인 포인트 |
|---|---|
| 이상탐지 방식 | 평소 추세를 자동 학습하는가 vs 규칙을 직접 정의해야 하는가 |
| 계보 연계 | 이상 발생 시 상위 원인·하위 영향 범위를 자동으로 추적하는가([C-3](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md) 연계) |
...
> **권장 — 사외 반출 제약을 우선적으로 확인.** 제조 현장 데이터는 사외 반출이 제한되는 경우가 많다. 클라우드 SaaS를 쓰기 어려우면 온프렘·VPC 배포가 되는 전용 솔루션이나 오픈소스(Elementary·Great Expectations·Deequ) 자기호스팅을 우선 검토한다.
...
| 데이터 관측(C-1)이 하는 것 | 인접 주제 | 인접 주제가 하는 것 | 연계 포인트 |
|---|---|---|---|
| 흐름이 정상인지 실시간 신호로 띄움 | [C-2 데이터 품질 관리](../C-2%20데이터%20품질%20관리/C-2%20데이터%20품질%20관리.md) | 데이터를 AI에 쓸 수 있는지 판정 | C-1 신호가 C-2 판정의 근거가 됨 |
| 현재 이상을 실시간 감지 | [C-3 데이터 계통 Lineage](../C-3%20데이터%20계통%20Lineage/C-3%20데이터%20계통%20Lineage.md) | 출처·이동·변환 이력을 사후 추적 | 이상의 영향 범위 파악에 C-3 계보 활용 |
| 이상을 감지·알림 | [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md) | 파이프라인 실행·복구·재처리 | C-1이 알리면 F-1이 복구를 실행 |
| 전처리 산출물이 운영 중에도 유효한지 관측 | [B-1 데이터 전처리](../B-1%20데이터%20전처리/B-1%20데이터%20전처리.md) | 비정형 데이터를 AI가 읽을 구조로 변환 | 양식 변경으로 전처리가 훼손되면 C-1 구조·분포 항목이 감지 |
| A-2 기준으로 이상 판단 | [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md) | 필드·단위·갱신 주기 등 속성 정의 | A-2가 정한 단위·주기·허용값이 이상 판단 기준 |

신뢰(Trustworthy) 세 주제의 관계는 다음과 같다. 관측(C-1)은 운영 중 실시간 신호를 띄우고 계통(C-3)은 사후 이력 근거를 대며, 품질 게이트(C-2)가 그 둘을 근거로 AI 투입 가부를 판정한다.
...
이 절은 데이터 관측이 실제로 제대로 작동하고 있는지를 수치로 확인하기 위한 KPI를 정의한다. 지표의 오너십·역할과 책임은 4장(운영)과 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)에서, 데이터 보존·폐기 시간축은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','clusterBkg':'#EEF4FB','clusterBorder':'#6B9AD1','fontSize':'13px'}}}%%
flowchart LR
    STD["운영·책임 체계<br/>(4장 · F-1 · F-2)"]:::std
    K["KPI로 점검<br/>관측 커버리지·MTTD·오탐률"]:::mid
...
|-----|-------------|--------------------------|-----------|
| 관측 커버리지 | AI가 쓰는 핵심 데이터를 실제로 지켜보는지 확인한다. 사각지대가 있으면 이상을 놓친다 | 관측 등록 파이프라인 / 전체 핵심 파이프라인 · 월 1회 | 핵심 파이프라인부터 100%를 목표로 단계 확대 |
| 이상 감지 시간(MTTD) | 감지가 지연되면 이상 데이터가 유입될 수 있으므로, 문제를 얼마나 신속하게 감지하는지를 확인한다 | 평균(알림 시각 − 이상 발생 시각) · 월 집계 | 기준선 대비 단축, 심각도별 SLA 합의 |
| 오탐률 | 오탐이 잦으면 알림으로 인한 경각심이 없어지므로, 알림의 신뢰성을 검토한다 | (전체 알림 − 실제 이상) / 전체 알림 · 월 집계 | 담당자가 관리 가능한 수준으로 SLA 합의 |

오탐률은 감지 실패한 이상(관측이 못 잡고 다른 경로로 발견된 건)과 함께 본다 — 한쪽을 무리하게 낮추면 다른 쪽이 늘기 때문이다. 운영이 성숙하면 해결 시간(MTTR)·관측 자동화율 등으로 넓힌다.
...
| 2026-07-01 | 1.4 | 2.1 "체계 내 위치" 중복 제거 (고객) — C-1·C-2·C-3 공통 | C-1·C-2·C-3의 2.1이 같은 신뢰(Trustworthy) 삼각 분업 설명·다이어그램을 세 파일에 반복하고 6장 Where와도 겹쳐 읽기 불편 → 2.1을 "정의와 역할"로 개칭하고 정의 + 한 줄 위치 포인터만 남김. 삼각 분업 문단은 삭제, 삼각관계 mermaid는 6장 Where로 이동(설명 문장 동반). 다른 주제와의 경계를 6장으로 일원화. What 도입문·목차 앵커 문구 정합 | 2장·6장·목차 |
| 2026-07-01 | 1.5 | KPI 섹션 재설계 (고객) — C-1·C-2·C-3 공통 | "관리 기준 vs KPI 층이 다르다" 도입문과 관리 기준 8행 표가 혼란(거버넌스 3행이 모두 "운영 기준 정의율" 하나로 수렴, KPI 없는 행 다수)해 처음부터 재설계. 관리 기준 프레임과 보일러플레이트 목표 문장을 걷어내고, 구축→운영→운영 품질 3층으로 그 주제 고유 핵심 지표 4개만 남긴 표(구분·지표·무엇을 재나·산식·목표 방향)로 교체. 별첨 "전체 KPI 목록"은 흡수(산식을 본문으로), 추가 지표는 한 줄로 나열. 오너십·R&R·생애주기 운영 기준은 F-1·F-2 포인터로. 제목 `# 7. KPI`→`## 7. KPI — 성과 지표`, 목차 별첨 링크 제거 | 7장·목차 |
| 2026-07-01 | 1.6 | KPI 섹션 운영 기준 복원·2소절화 (고객) — C-1·C-2·C-3 공통 | "운영 기준과 KPI가 잘 적용되는지 파악"하도록 재구성. 도입문에 '품질·출처·변경 이력·계보 관리 체계로 신뢰성 저하·Hallucination 방지' 목적 복원. 7.1 운영 기준(관측 대상·이상 판단/알림 + 데이터 오너십·R&R·생애주기)과 7.2 핵심 KPI(구축·운영·운영 품질, 산식 포함)로 분리. 운영 기준 적용 여부는 KPI '운영 기준 정의율' 하나로만 측정해 옛 '3행→1KPI' 중복 없이 반영. 목차 7.1·7.2 갱신 | 7장·목차 |
| 2026-07-01 | 1.7 | KPI 섹션 다이어그램 추가 (고객) — C-1·C-2·C-3 공통 | 7장이 표·글만 있어 잘 안 와닿는다는 피드백 → 도입부에 '운영 기준 정의 → 구축·운영·운영 품질 단계별 KPI 점검 → 목표(SLA) 대비 미달 시 개선' 순환 mermaid 추가(주제별 KPI 라벨, 02 표준 컬러). 한 줄 설명문 동반 | 7장 |
| 2026-07-01 | 1.8 | 7.1 운영 기준 실질 정의 (고객) — C-1·C-2·C-3 공통 | "지정한다/구분한다/정의한다"만 있고 실제 내용이 없던 순환 표를 걷어내고, 데이터 오너십·역할과 책임(R&R)·생애주기(Lifecycle)를 구체 내용으로 정의. 오너십=오너 1명 지정·정상 기준 승인·이상 시 최종 판단, R&R=데이터 오너/관측 담당/운영 담당 역할·책임 표, 생애주기=수집→적재→활용→보관/폐기 중 관측 유지·기준선 학습 기간 | 7.1 |
| 2026-07-01 | 1.9 | 운영 기준 공통 분리·KPI 정비 (고객) — C-1·C-2·C-3 공통 | ① C-1·C-2·C-3에 중복되던 운영 기준(오너십·R&R·생애주기)을 신설 [별첨 C. 운영 기준서](../별첨%20C%20운영%20기준서/별첨%20C.%20운영%20기준서.md)로 분리, 7.1 삭제하고 7장 도입에서 링크. ② 7장을 이 주제 KPI만 남기고 표를 'KPI·왜 필요한가·측정 방법(산식·주기)·목표 설정'으로 재구성 — '높을수록/낮을수록' 방향 열 제거, 컨설팅식(기준선 측정→오너와 SLA 합의) 목표 설정으로 교체. ③ KPI를 꼭 필요한 3개로 축소(관측 커버리지·MTTD·오탐률), 운영 기준 정의율은 별첨 C로 이관. ④ 순환 다이어그램을 '운영 기준(별첨 C)→KPI 점검→목표 대비→개선'으로 단순화, 목차 7.1/7.2·질문 표 갱신 | 7장·목차·별첨 C |
| 2026-07-01 | 2.0 | 별첨 C 폐지·F로 연결 (고객) | 운영 기준(오너십·R&R·생애주기)은 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)(7.1 관리 기준)·[F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)가 이미 다루고 이 가이드도 4장(운영)에 자기 오너·역할이 있어, 별첨 C 운영 기준서는 중복 → **삭제**. 7장 KPI 섹션을 자기 KPI로 완결시키고 오너십·역할은 4장·F-1, 보존·폐기는 F-2로 연결. 도입문·다이어그램(운영·책임 체계=4장·F-1·F-2)·질문 표의 별첨 C 참조 제거 | 7장·질문 표 |
| 2026-07-01 | 2.1 | 2.1 경계 문장 제거 (고객) | 2.1 정의와 역할에서 "실시간 이상 감지(C-1)·사후 이력 추적(C-3)…과의 경계는 6장에서 정리한다" 문장 삭제 — 다른 주제와의 관계는 6장 Where에서만 다루고 본문 정의에는 두지 않음. 정의와 역할만 남김 | 2.1 |
| 2026-07-01 | 2.2 | 6장 Where 표 컬럼 순서 통일 (고객) | Where 표를 B-1 형식('[주제]가 하는 것·인접 주제·인접 주제가 하는 것·연계 포인트' 4열)에 맞춤. 기존 3열('인접 주제·그 주제의 역할·C-1과의 경계')을 4열로 전환 — 경계 서술을 'C-1이 하는 것'과 '연계 포인트'로 분리 | 6장 |
| 2026-07-01 | 2.2 | 2.4 표 머리글 정리 (고객) | 심각도 등급 표의 "경로·시점" 열 머리글이 어색하다는 피드백 → "알림 방식 (예시)"로 변경(셀 내용이 호출·메신저·로그 등 알림 방식 예시임) | 2.4 |
