...
상시 점검하는 핵심 지표는 다섯 개다. 각 구간(구축·운영·운영 품질)에서 하나씩 잡아, 계통을 얼마나 이었고·얼마나 되짚으며·얼마나 최신인지를 한 화면으로 본다. 절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세우고 목표 대비로 관리한다. 초기에는 이 다섯 개로 시작하고, 운영이 성숙하면 [별첨 전체 KPI 목록](#kpi-all)까지 넓힌다.

| 구분 | KPI | 정의 | 산식 | 목표 방향 |
|------|------|------|------|-----------|
| 구축 수준 | Lineage Coverage | 관리 대상 데이터 중 Lineage가 구축된 비율 | Lineage 연결 완료 자산 / 구축 대상 자산 | 높을수록 |
| 구축 수준 | 운영 기준 정의율 | 계통 관리 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관리 대상 | 높을수록 |
| 운영 수준 | 근거 추적 성공률 | AI 답변을 원천 데이터까지 역추적 가능한 비율 | 성공 건수 / 추적 대상 건수 | 높을수록 |
| 운영 수준 | 영향도 분석 수행률 | 변경 작업 전 영향도 분석을 수행한 비율 | 영향도 분석 수행 건수 / 변경 작업 건수 | 높을수록 |
| 운영 품질 | 계통 최신성 | 실제 파이프라인 변경 후 Lineage 반영 수준 | SLA 내 갱신 건수 / 전체 변경 건수 | 높을수록 |
