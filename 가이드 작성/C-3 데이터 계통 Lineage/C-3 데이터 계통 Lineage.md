...
> 정의: 데이터의 출처·이동·변환·활용 이력을 기록해, AI 답변·보고서가 어떤 데이터에서 나왔는지 추적하는 체계.
...
계통 관리 수준은 다음 지표로 본다. 부풀리지 않고 운영 목표와 연결되는 3~4개로 둔다.[\[19\]](#ref19) 목표 수준은 환경마다 다르므로 PoC에서 기준선을 잡고 합의한다.

| 지표 | 쉬운 의미 | 측정 목적 | 산식 (산정 가능 시) | 방향 |
|---|---|---|---|---|
| 계통 연결 범위 | AI 활용 핵심 흐름 중 계통이 끊김 없이 이어진 비율 | 근거를 댈 수 있는 범위 | 계통 연결 완료 흐름 수 ÷ AI 핵심 흐름 수 | 높을수록 좋음 (↑) |
| 근거 추적 성공률 | AI 답변·리포트 중 근거를 원천까지 역추적한 비율 | 답의 검증·신뢰 정도 | 역추적 성공 건수 ÷ 표본 답변 건수 | 높을수록 좋음 (↑) |
| 영향분석 소요시간 | 원천·로직 변경 시 영향 범위 파악에 걸린 시간 | 변경 사고 사전 차단 | 평균(영향 범위 확정 시각 − 변경 검토 시작 시각) | 낮을수록 좋음 (↓) |
| 계통 노후화율 | 실제 파이프라인과 어긋난 계통 기록의 비율 | 계통이 가리키는 근거의 신뢰 유지 | 실제와 불일치한 계통 레코드 수 ÷ 점검 표본 수 | 낮을수록 좋음 (↓) |

측정값 자체보다, AI 답변의 근거를 제시하고 변경 영향을 사전에 파악하는 상태에 도달했는지를 본다.
