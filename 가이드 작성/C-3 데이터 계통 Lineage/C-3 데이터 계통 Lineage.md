...
계통은 AI 결과를 신뢰하고 활용하게 만드는 신뢰(Trustworthy) 묶음에서, 데이터가 과거에 어떤 경로로 왔는지를 사후에 추적해 판정과 이상 감지에 근거를 대는 역할을 맡는다. 계통 자체는 판정하지 않는다. 실시간 이상 감지([C-1 Observability](../C-1%20Observability/C-1%20Observability.md))·사용 가능 여부 판정([C-2 데이터 품질 관리](../C-2%20데이터%20품질%20관리/C-2%20데이터%20품질%20관리.md))·보존과 폐기 기간([F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md))과의 경계는 [6장](#where)에서 정리한다.
