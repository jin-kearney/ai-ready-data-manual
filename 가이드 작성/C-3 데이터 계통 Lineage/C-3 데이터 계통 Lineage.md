...
> 🔗 **2층 연결:** 솔루션을 주제 가로질러 묶어 평가·선정하려면 → [Tech Stack 비교 정본](../../전체%20목차/01%20Tech%20Stack%20비교%20(솔루션×주제).md). 계통(C-3)은 카탈로그·거버넌스 플랫폼 한 제품이 A-1·A-2·A-3과 함께 커버하는 것이 시장 표준이며, 변별점은 컬럼 단위 계통이다. 이 절은 그 가운데 계통 기능만 떼어 비교한다.
...
> 솔루션을 주제 전반에 걸쳐 묶어 비교·선정하는 일은 2층 정본 [Tech Stack 비교 (솔루션×주제)](../../전체%20목차/01%20Tech%20Stack%20비교%20(솔루션×주제).md)가 전담한다.
