...
<!-- KQ→섹션 매핑(자가 점검): KQ1 대상 흐름→2.2(s22)+3(when) / KQ2 변환 기록→2.3(s23)+4.4(s44) / KQ3 근거 역추적→2.3(s23)+4.1(s41)+4.4(s44) / KQ4 변경 영향→2.4(s24)+4.3(s43) / KQ5 감사 증빙→2.4(s24)+3.2(s32) / KQ6 운영 기준 및 KPI→7.1(s71)+7.2(s72). -->
...
데이터 품질·출처·변경 이력·계보(Lineage) 관리 체계를 통해 AI 활용 시 데이터 신뢰성 저하와 Hallucination을 방지하려면, 계통이 무엇을 지켜야 하는지(운영 기준)와 그것이 잘 적용되는지(KPI)를 함께 정한다.
...
flowchart LR
    STD["운영 기준 정의<br/>추적 대상·범위·변경/근거 관리<br/>+ 오너십·R&R·생애주기"]:::std
    T1["구축<br/>계통 커버리지<br/>운영 기준 정의율"]:::t1
    T2["운영<br/>근거 추적 성공률<br/>영향도 분석 수행률"]:::t2
    T3["운영 품질<br/>계통 최신성"]:::t3
...
운영 기준은 계통을 누가·무엇을·어느 단계까지 책임지고 관리하는지 정한 약속이다. 아래 세 가지를 정의한다.

**데이터 오너십** — 계통 관리 대상 데이터마다 데이터 오너 1명을 지정한다. 오너는 그 데이터의 계통 정보(출처·변환·활용 경로)가 정확한지 책임지고, 계통 단절·오류가 확인되면 수정 우선순위를 판단한다.

**역할·책임(R&R)** — 계통 수집부터 검증까지의 책임을 아래와 같이 나눈다.

| 역할 | 책임 |
|------|------|
| 데이터 오너 (현업 부서) | 계통 정확성 책임, 단절·오류 수정 우선순위 판단 |
| 계통 담당 (데이터·플랫폼팀) | 계통 수집·갱신, AI 답변 근거(Citation) 기록 운영 |
| 검증 담당 | 계통 정합성·근거 추적 가능 여부 점검 |

**생애주기(Lifecycle)** — 데이터가 수집→적재→활용→보관/폐기를 거치는 동안 계통 기록을 유지한다. 근거·이력의 보존 기간은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)의 정책을 따른다.

<a id="s72"></a>

### 7.2 핵심 KPI

운영 기준이 잘 적용되는지 아래 지표로 점검한다. 구축(얼마나 넓게 이었나)→운영(얼마나 잘 되짚나)→운영 품질(얼마나 최신으로 유지되나) 순으로 본다.

| 구분 | 지표 | 무엇을 재나 | 산식 | 목표 방향 |
|------|------|-------------|------|-----------|
| 구축 | 계통 커버리지 | 핵심 자산 중 계통이 구축된 비율 | 계통 연결 완료 자산 / 구축 대상 자산 | 높을수록 |
| 구축 | 운영 기준 정의율 | 계통 관리 대상 중 오너십·R&R·생애주기 기준이 정의된 비율 | 운영 기준 정의 대상 / 관리 대상 | 높을수록 |
| 운영 | 근거 추적 성공률 | AI 답변을 원천 데이터까지 되짚을 수 있는 비율 | 성공 건수 / 추적 대상 건수 | 높을수록 |
| 운영 | 영향도 분석 수행률 | 변경 작업 전 영향도 분석을 수행한 비율 | 영향도 분석 수행 건수 / 변경 작업 건수 | 높을수록 |
| 운영 품질 | 계통 최신성 | 파이프라인 변경 후 계통에 제때 반영된 수준 | SLA 내 갱신 건수 / 전체 변경 건수 | 높을수록 |

절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다.
