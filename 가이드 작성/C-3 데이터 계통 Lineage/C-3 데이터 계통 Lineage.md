...
계통을 지속적으로 운영하기 위해 아래를 정의한다.

| 운영 기준 | 무엇을 지키나 |
|------|-------------|
| 추적 대상·범위 | 원천부터 AI 답변까지, 출처·변환 이력·활용 이력을 계통으로 관리한다 |
| 변경·근거 관리 | 구조·변환 로직 변경 시 계통을 함께 갱신하고, AI 답변에 참조 근거(Citation)를 기록한다 |
| 데이터 오너십 | 계통 관리 대상마다 계통 정보를 책임지는 데이터 오너를 지정한다 |
| 역할·책임(R&R) | 계통 수집·갱신·검증의 역할과 책임을 구분한다 |
| 생애주기(Lifecycle) | 데이터 생성부터 폐기까지 계통 기록을 유지·관리한다 |
