...
관리 기준과 KPI는 층이 다르다. **관리 기준**은 "이렇게 운영하겠다"는 약속으로, 아래 여덟 가지를 모두 갖춰 운영하는 것이 목표다. **KPI**는 그 약속이 잘 지켜지는지 숫자로 재는 계기판으로, 여덟 가지를 다 수치화하지 않고 상태를 대표하는 다섯 개만 상시로 본다. 아래 표는 각 관리 기준을 어떤 핵심 KPI로 재는지 한 줄로 잇는다.
...
|-----------|--------------------------|--------------------------|-----------|
| 추적 대상 | 원천부터 AI 답변까지 Lineage 대상으로 관리한다 | Lineage Coverage | 높을수록 |
| 추적 범위 | 출처·변환 이력·활용 이력을 함께 관리한다 | (Lineage Coverage에 포함) | — |
| 변경 관리 | 데이터 구조·변환 로직 변경 시 Lineage를 함께 갱신한다 | 영향도 분석 수행률 | 높을수록 |
| 근거 관리 | AI 답변은 참조한 데이터·문서·Citation을 함께 기록한다 | 근거 추적 성공률 | 높을수록 |
| 최신성 관리 | 운영 중인 Lineage를 실제 데이터 흐름과 일치하게 유지한다 | 계통 최신성 | 높을수록 |
| 데이터 오너십 | 계통 관리 대상마다 계통 정보를 책임지는 데이터 오너를 지정한다 | 운영 기준 정의율 | 높을수록 |
| 역할·책임(R&R) | 계통 수집·갱신·검증의 역할과 책임을 구분하여 운영한다 | 운영 기준 정의율 | 높을수록 |
| 생애주기(Lifecycle) | 데이터 생성부터 폐기까지 계통 기록을 유지·관리한다 | 운영 기준 정의율 | 높을수록 |

핵심 KPI 다섯 개(Lineage Coverage·운영 기준 정의율·근거 추적 성공률·영향도 분석 수행률·계통 최신성)의 산식과 나머지 계통 지표는 [별첨 전체 KPI 목록](#kpi-all)에 둔다. 절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다.
...
[7.2 핵심 운영 KPI](#s72)의 다섯 개를 포함한 계통 KPI 전체다. 처음부터 전부 재지 말고 핵심 다섯 개로 시작해, 운영이 자리 잡으면 아래 지표까지 단계적으로 넓힌다.

| 구분 | KPI | 정의 | 산식 | 활용 목적 |
|------|------|------|------|-----------|
| 구축 수준 | Lineage Coverage | 관리 대상 데이터 중 Lineage가 구축된 비율 | Lineage 연결 완료 자산 / 구축 대상 자산 | 구축 범위 관리 |
...
| 운영 수준 | 근거 추적 성공률 | AI 답변을 원천 데이터까지 역추적 가능한 비율 | 성공 건수 / 추적 대상 건수 | 추적 가능성 확보 |
|  | 영향도 분석 수행률 | 변경 작업 전 영향도 분석을 수행한 비율 | 영향도 분석 수행 건수 / 변경 작업 건수 | 변경 관리 수준 |
| 운영 품질 | 계통 최신성 | 실제 파이프라인 변경 후 Lineage 반영 수준 | SLA 내 갱신 건수 / 전체 변경 건수 | 최신성 관리 |
|  | Broken Lineage 비율 | 단절된 Lineage 비율 | 단절 Edge / 전체 Edge | 계통 품질 관리 |
