...
> 정의: 데이터의 출처·이동·변환·활용 이력을 기록해 AI 답변·보고서가 어떤 데이터에서 나왔는지 추적할 수 있게 하는 체계.
...
2. [What — 무엇인가·무엇을 갖추나](#what)
    - [2.1 데이터 계통이란 + 범위·경계](#s21)
    - [2.2 추적 대상 흐름과 입도](#s22)
    - [2.3 계통이 기록하는 세 가지](#s23)
    - [2.4 계통으로 답하는 두 가지 질문 — 영향도와 감사](#s24)
3. [When — 어디부터 추적하나 (대상 우선순위)](#when)
    - [3.1 기본 필수](#s31)
    - [3.2 더 깊게](#s32)
4. [예시 시나리오 — 한눈에](#example)
5. [Tech Stack — 계통 솔루션 검토](#tech)
    - [5.1 솔루션 유형](#s51)
    - [5.2 선정 기준](#s52)
6. [How — 어떻게 준비·운영하나](#how)
    - [6.1 구축 절차](#s61)
    - [6.2 수집 방식 고르기](#s62)
    - [6.3 계통 레코드에 무엇을 남기나](#s63)
    - [6.4 운영과 역할](#s64)
7. [Where — 다른 주제와의 관계](#where)
8. [KPI — 성과 지표](#kpi)

- [별첨 (Appendix)](#별첨-appendix)
    - [주요 용어](#a1) · [계통 레코드 항목 사전(전체)](#a2) · [수집 방식 비교](#a3) · [감사 대응 점검표](#a4)
- [참고자료 (References)](#참고자료-references) · [핵심 질문 점검](#이-가이드가-답하는-5가지-핵심-질문) · [변경 이력 / 피드백 반영](#변경-이력--피드백-반영)

---

> **예시 표기 안내:** 본 가이드의 다이어그램·표·그림에 나오는 구체 값(설비·라인 번호, 센서값, 문서번호, 날짜, 건수 등)은 이해를 돕기 위한 가상 예시이며 실제 데이터가 아니다. 실제 값은 PoC·프로젝트에서 확정한다. 계열사명도 적용 맥락 설명용이다.
...
이 가이드는 데이터 계통이 왜 필요한지(1장), 무엇을 기록하는지(2장), 어떤 흐름부터 추적할지(3장), 적용하면 어떻게 달라지는지(4장), 어떤 솔루션으로(5장) 어떻게 구축·운영하는지(6장)를 다룬다. 끝까지 강조하는 메시지는 하나다. AI가 내놓은 답과 보고서는 그 근거가 어느 데이터에서 왔는지 짚을 수 있어야 현업이 믿고 쓴다. 계통은 그 근거의 경로를 데이터로 남긴다.
...
AI가 "1호기 베어링에 이상이 있다"고 답해도, 그 판단이 어느 센서값·어느 기준서에서 나왔는지 짚지 못하면 현장은 그 답을 받아들이지 않는다. 데이터 계통은 답과 데이터를 잇는 경로를 기록해, AI 결과를 검증 가능한 것으로 바꾼다.

<a id="s11"></a>

### 1.1 현업에서 막히는 지점

계통 기록 없이 AI 과제를 운영할 때 반복적으로 부딪히는 문제는 다음과 같다.

| 막히는 지점 | 현장에서 벌어지는 일 |
|---|---|
| 답의 근거를 못 짚는다 | AI 점검 리포트가 결론만 내놓고 어느 센서 로그·성적서·기준서를 근거로 했는지 보여주지 못해, 현업 엔지니어가 결과를 신뢰하지 않는다 |
| 바꾸면 무엇이 깨지는지 모른다 | 원천 테이블 스키마나 전처리 로직을 바꾼 뒤에야 어떤 리포트·모델·AI 답변이 틀어졌는지 드러난다. 사전에 영향 범위를 알 수 없다 |
| 감사에 답하지 못한다 | "이 불량 판정 AI가 쓴 데이터의 출처와 변환 과정은 무엇인가"라는 내부·고객 감사 질문에 즉시 증빙하지 못해 수일이 걸린다 |
| 팀마다 출처 이해가 다르다 | 같은 데이터의 출처와 가공 이력을 생산팀·품질팀·IT가 제각각 이해해, 분석 결과가 어긋날 때 원인을 찾는 데 시간이 든다 |

공통점은 데이터가 없는 것이 아니라, 데이터가 어디서 와서 어떻게 변해 그 답이 됐는지의 경로가 남아 있지 않다는 데 있다.
...
- 근거를 즉시 역추적한다. AI 답변·리포트가 참조한 데이터셋·문서·청크를 답과 함께 남기면, 현업이 클릭 한 번으로 원천 센서값·성적서 페이지까지 거슬러 확인한다. 전처리가 붙여 둔 원본 위치 정보([B-1 데이터 전처리](../B-1%20데이터%20전처리/B-1%20데이터%20전처리.md))가 이 역추적의 출발점이 된다.
- 변경 영향을 미리 본다. 원천이나 전처리 로직을 바꾸기 전에 그 변경이 어떤 리포트·모델·AI 서비스로 번지는지 확인한다. 배포한 뒤 장애로 알게 되는 대신, 배포 전에 영향받는 자산 목록을 본다.
- 감사·규제에 바로 답한다. 출처·변환 이력·사용 목적·답변 근거를 한 번에 꺼내는 증빙 체계를 갖추면, 감사 질의에 수일이 아니라 당일에 대응한다. AI의 투명성·추적성을 요구하는 규제 흐름[\[15\]](#ref15)에도 같은 기록이 쓰인다.
...
이 장은 데이터 계통이 무엇이고 무엇을 기록하는지를 정의한다. 구축 절차와 운영은 [6장](#how)에서 다루고, 여기서는 계통의 정의·경계, 추적 대상 흐름, 기록하는 요소, 그것으로 답하는 질문을 정한다.
...
계통의 책임 범위는 연결성·추적성까지다. 실시간 이상 탐지는 [C-1](../C-1%20Observability/C-1%20Observability.md), 사용 가능 여부 판정은 [C-2](../C-2%20데이터%20품질%20관리/C-2%20데이터%20품질%20관리.md), 데이터의 보존·폐기 기간은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)가 맡는다. 이 경계는 [7장](#where)에서 정리한다.
...
- 테이블 단위: 어느 자산이 어느 자산에서 왔는지만 기록한다. 구현이 단순하고 대부분 솔루션이 기본 제공하지만, 안의 어느 값이 영향인지는 모른다.
...
테이블 단위는 "이 테이블이 저 테이블에 영향을 준다"까지만 알려주지만, 컬럼 단위는 "단가 컬럼이 바뀌면 총원가 컬럼이 영향받는다"를 짚어 준다. 그래서 스키마를 바꾸기 전에 실제로 영향받는 리포트·모델을 정확히 추릴 수 있다.
...
세 번째가 AI-ready 계통을 일반 계통과 구분 짓는 지점이다. 검색 증강 생성(RAG, AI가 답할 때 우리 문서를 찾아 근거로 함께 쓰는 방식)에서는 답이 참조한 청크와 그 원본 위치를 함께 남겨야 답을 근거까지 되짚을 수 있다.

![AI 답변에 근거를 함께 남기는 모형 — 답변 카드 아래 근거 칩(센서 로그·기준서 p.5·유사 사례)이 각각 원본 문서로 이어진다](이미지/AI답변-근거카드-모형.svg)

> **용어 풀이 — 근거(Citation)·출처 정보(Provenance).** 근거는 한 답변이 참조한 데이터 조각의 목록이다. 출처 정보는 각 조각이 어느 문서·페이지·좌표에서 왔는지의 꼬리표로, 전처리가 청크에 붙여 둔다. 계통은 이 꼬리표들을 흐름으로 이어, 답 → 청크 → 원본 문서 → 원천까지의 경로를 만든다.
...
예를 들어 성적서 전처리에서 두께 단위를 바꾸면, 하류로는 그 컬럼을 쓰는 품질 데이터셋·불량 분석 리포트·AI 점검 답변이 영향받음을 배포 전에 확인한다. 거꾸로 불량 분석 결과가 이상하면 상류로 거슬러 어느 전처리 단계·원천 성적서에서 비롯됐는지 찾는다. 영향도 분석을 신뢰할 수준으로 하려면 [2.2](#s22)의 컬럼 단위 계통이 필요하다.
...
- AI 답변·자동 리포트로 바로 이어지는 흐름 — 답의 근거를 못 대면 현업이 쓰지 못하므로 가장 먼저 잇는다.
- 여러 팀·계열사가 함께 쓰는 핵심 데이터셋 — 한 곳이 바뀌면 여러 곳이 영향받으므로 경로를 알아야 한다.
...
quadrantChart
    title 계통 추적 대상 우선순위
    x-axis 자동 수집 쉬움 --> 자동 수집 어려움
    y-axis 신뢰·감사 중요도 낮음 --> 신뢰·감사 중요도 높음
    quadrant-1 "전략 과제 (도구 갖춘 뒤)"
    quadrant-2 "최우선 착수"
    quadrant-3 "후순위"
    quadrant-4 "보류/수기 보완"
    "AI 답변 근거 흐름": [0.4, 0.92]
    "품질·안전 판정 리포트": [0.3, 0.8]
    "레거시 ETL 경로": [0.82, 0.62]
    "내부 임시 분석": [0.55, 0.25]
```

가치가 높고 자동 수집이 쉬운 왼쪽 위(최우선)부터 잇고, 중요하지만 레거시라 자동 수집이 어려운 오른쪽 위는 솔루션을 갖춘 뒤 단계적으로 옮겨 간다. 자동화가 어려운 구간은 [6.2](#s62)의 수기 선언으로 공백을 메운다.
...
발전 설비 진동 점검 AI를 운영하는 경우를 예로 든다(가상). 적용 전에는 AI가 "1호기 베어링 이상 징후"라고 답해도 근거를 보여주지 못해, 현장은 직접 데이터를 다시 뒤져 확인한 뒤에야 조치한다. 계통과 근거 기록을 붙이면 답이 참조한 센서 로그·기준서·과거 사례를 클릭해 원천까지 거슬러 확인하고, 그 자리에서 조치로 넘어간다.

| 적용 전 | 적용 후 |
|---|---|
| AI 점검 리포트가 결론만 제시, 근거 불명 | 답변에 근거(센서 로그·기준서 p.5·유사 사례)를 함께 표시 |
| 현장이 데이터를 다시 뒤져 검증 후 조치 | 근거를 클릭해 원천까지 역추적, 바로 조치 |
| 원천 변경 시 장애가 난 뒤 대응 | 변경 전 영향받는 리포트·모델을 미리 확인 |
| 감사 때 출처 수집에 수일 | 출처·변환·근거를 당일 증빙 |

흐름을 미리 보면 다음과 같다. 추적할 흐름을 고르고([3장](#when)) → 출처·변환을 자동 수집으로 잇고([6.1](#s61)) → AI 답변에 근거를 로깅하고([6.3](#s63)) → 역추적·영향분석으로 활용한다([2.4](#s24)).
...
굵은 파란 경로가 이 답변이 실제로 근거로 삼은 흐름이다. 답변의 근거 링크를 누르면 이 경로를 거꾸로 따라가 진동 센서 로그와 정비 기준서 원본까지 도달한다. 같은 데이터셋에 모이지만 이 답과 무관한 흐름(회색)은 근거에서 빠진다. 이렇게 답마다 근거 경로가 분리돼 남는 것이 AI-ready 계통의 목표 모습이다.
...
> 🔗 **2층 연결:** 솔루션을 주제 가로질러 묶어 평가·선정하려면 → [Tech Stack 비교 정본](../../Tech%20Player/01%20Tech%20Stack%20비교%20(솔루션×주제).md). 계통(C-3)은 카탈로그·거버넌스 플랫폼 한 제품이 A-1·A-2·A-3과 함께 커버하는 것이 시장 표준이며, 변별점은 컬럼 단위 계통이다. 이 절은 그 가운데 계통 기능만 떼어 비교한다.
...
**(1) 계통 수집 오픈 표준·오픈소스.** 도구를 바꿔도 계통 데이터를 재활용할 수 있게 표준 위에서 모은다.
...
- 수집 방식 적합성 — 레거시 코드가 많으면 정적 파싱, 실행 경로가 중요하면 런타임 이벤트([6.2](#s62)).
- AI 답변 근거 연결 — 답이 참조한 청크·데이터셋을 계통과 이을 수 있는지.
...
계통은 다음 순서로 구축·운영한다 — 대상 흐름을 정의하고([6.1](#s61)), 출처·변환을 수집 방식에 맞게 잇고([6.2](#s62)), 계통 레코드와 AI 답변 근거를 남기며([6.3](#s63)), 흐름이 바뀌면 갱신하는 운영으로 마무리한다([6.4](#s64)).
...
대상 흐름을 정의할 때는 [3장](#when)의 우선순위로 AI 답변·핵심 리포트로 이어지는 흐름부터 잡는다. 출처·변환 연결은 자동 수집을 기본으로 하고([6.2](#s62)), AI 답변 근거 로깅은 답이 참조한 청크·데이터셋·Tool 결과를 답과 함께 남긴다. 검증은 그려진 계통이 실제 흐름과 맞는지 대조하는 단계로, 누락·끊김을 잡는다.
...
| 런타임 이벤트 | 실제 실행 시 엔진·오케스트레이터가 계통 이벤트 방출(OpenLineage 등) | 실제 실행 경로만 정확히, 동적 경로 추적 | 에이전트·플러그인 설치 필요, 실행 안 된 경로는 누락 |
| 수기 선언 | 사람이 흐름을 직접 입력·선언 | 자동화 안 되는 레거시·엑셀·공유드라이브 공백 메우기 | 변경 시 수동 갱신, 실제와 어긋날 위험 |

현대 환경은 정적 파싱과 런타임 이벤트를 함께 써 설계 의도와 실제 실행을 모두 담고, 수기 선언으로 자동화가 닿지 않는 공백만 보완한다. 전수를 수기로 그리려 하지 않는다 — 관리 비용이 커지고 금세 실제와 어긋난다.
...
| 실행 시각 | 언제 돌았나 | 2026-06-14 03:00 | 필수 | 자동(런타임) |
| 입도 | 테이블/컬럼/청크 | 컬럼 | 권장 | 자동/엔지니어 |
| 근거 청크 ID | AI 답변이 참조한 원본 조각 | QR-2026-0481_p3_r2 | 답변 흐름 시 필수 | 자동(로깅) |

남기는 내용은 막연하면 쓸모가 없다. 무엇을·어떤 로직으로·어느 버전으로 바꿨는지가 구체적이어야 나중에 되짚을 수 있다.
...
가장 헷갈리는 경계는 C-1과 C-3이다. 둘 다 신뢰를 떠받치지만, C-1은 지금 이 순간의 이상을 감지하는 실시간 관측이고, C-3은 과거에 어떤 경로로 왔는지를 사후에 추적하는 이력이다. C-1이 "지금 신호가 이상한가"라면 C-3은 "이 결과가 어디서 왔는가"다.
...
계통 관리 수준은 다음 지표로 본다. 부풀리지 않고 운영 목표와 연결되는 3~4개로 둔다.[\[19\]](#ref19)

| 지표 | 쉬운 의미 | 방향 | 기대효과 |
|---|---|---|---|
| 계통 연결 범위 | AI 활용 핵심 흐름 중 계통이 끊김 없이 이어진 비율 | ↑ | 근거를 댈 수 있는 흐름이 넓어짐 |
| 근거 추적 성공률 | AI 답변·리포트 중 근거를 원천까지 역추적할 수 있는 비율 | ↑ | 답을 검증·신뢰할 수 있음 |
| 영향분석 소요시간 | 원천·로직 변경 시 영향 범위를 파악하는 데 걸리는 시간 | ↓ | 변경 사고를 사전에 차단 |
| 계통 노후화율 | 실제 파이프라인과 어긋난 계통 기록의 비율 | ↓ | 계통이 가리키는 근거를 믿을 수 있음 |

지표값의 목표 수준은 환경마다 다르므로 PoC에서 기준선을 잡고 합의한다. 측정 자체보다, AI 답변의 근거를 댈 수 있고 변경 영향을 미리 보는 상태에 도달했는지를 본다.
...
본문 [6.3](#s63)의 대표 항목을 포함한 전체 목록. 실제 항목은 솔루션·PoC에서 확정한다.
...
<a id="이-가이드가-답하는-5가지-핵심-질문"></a>

## 이 가이드가 답하는 5가지 핵심 질문

| 핵심 질문 | 한 줄 답 | 다루는 곳 |
|---|---|---|
| 어떤 데이터 흐름을 계통 대상으로 관리할 것인가? | 원천부터 AI 답변까지 전 구간 — AI 답변·핵심 리포트로 이어지는 흐름을 우선으로 잡는다 | [§2.2 추적 대상 흐름과 입도](#s22) · [§3 When](#when) |
| 데이터가 어떤 변환과 조합을 거쳤는지 어떻게 기록할 것인가? | 연결·변환 이력·AI 답변 근거 세 가지를 계통 레코드에 항목별로 남긴다 | [§2.3 계통이 기록하는 세 가지](#s23) · [§6.3 계통 레코드에 무엇을 남기나](#s63) |
| AI 답변이나 보고서의 근거 데이터를 어떻게 역추적할 것인가? | 답변 로깅 시 참조 청크·데이터셋·Tool 결과를 함께 남겨, 답 → 청크 → 원본까지 역추적한다 | [§2.3 계통이 기록하는 세 가지](#s23) · [§6.3 계통 레코드에 무엇을 남기나](#s63) |
| 데이터 변경의 영향 범위를 어떻게 분석할 것인가? | 하류 추적(Downstream)으로 변경 전에 영향받는 리포트·모델·AI 서비스 목록을 확인한다 | [§2.4 영향도와 감사](#s24) · [§6.2 수집 방식 고르기](#s62) |
| 감사나 규제 대응 시 무엇을 증빙할 것인가? | 출처·변환 이력·접근 이력·AI 답변 근거를 한 번에 꺼낼 수 있는 증빙 체계를 계통으로 갖춘다 | [§2.4 영향도와 감사](#s24) · [§3.2 더 깊게](#s32) · [감사 대응 점검표](#a4) |
