...
<!-- KQ→섹션 매핑(자가 점검): KQ1 대상 흐름→2.2(s22)+3(when) / KQ2 변환 기록→2.3(s23)+4.4(s44) / KQ3 근거 역추적→2.3(s23)+4.1(s41)+4.4(s44) / KQ4 변경 영향→2.4(s24)+4.3(s43) / KQ5 감사 증빙→2.4(s24)+3.2(s32) / KQ6 성과 지표→7.1(s71). -->
...
데이터 계통이 실제로 작동하는지 숫자로 확인하는 계기판이다. 구축(얼마나 넓게 이었나)·운영(얼마나 잘 되짚나)·운영 품질(얼마나 최신으로 유지되나) 세 층으로 나눠 본다. 처음부터 다 재지 말고 아래 핵심 지표로 시작해, 운영이 자리 잡으면 뒤에 적은 지표까지 단계적으로 넓힌다.
...
절대 기준값을 정하기보다 지표마다 자체 목표(SLA)를 세워 목표 대비로 관리한다. 운영이 성숙하면 컬럼 단위 커버리지·AI 근거 연결률·단절 계통 비율·계통 수정 Lead Time·계통 활용도까지 넓힌다.

오너십·역할과 책임(R&R)·생애주기 같은 운영 기준은 별도 지표로 재기보다 계통 관리 대상마다 정해 두고 정기 점검한다([F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)·[F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다).
