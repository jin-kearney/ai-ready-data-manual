...
<!-- KQ→섹션 매핑(자가 점검): KQ1 대상 흐름→2.2(s22)+3(when) / KQ2 변환 기록→2.3(s23)+4.4(s44) / KQ3 근거 역추적→2.3(s23)+4.1(s41)+4.4(s44) / KQ4 변경 영향→2.4(s24)+4.3(s43) / KQ5 감사 증빙→2.4(s24)+3.2(s32)+별첨(a4). -->
...
데이터 계통은 구축 자체보다 지속적인 운영과 최신성 유지가 중요하다.

운영 과정에서는 계통 연결 범위, 근거 추적 가능 여부, 영향도 분석 활용 수준을 지속적으로 점검하고 개선한다.

운영 수준을 평가하기 위한 상세 KPI와 산정 기준은 **7장 KPI**에서 정의한다.
...
> **2층 연결:** 솔루션을 주제 가로질러 묶어 평가·선정하려면 → [Tech Stack 비교 정본](../../Tech%20Player/01%20Tech%20Stack%20비교%20(솔루션×주제).md). 계통(C-3)은 카탈로그·거버넌스 플랫폼 한 제품이 A-1·A-2·A-3과 함께 커버하는 것이 시장 표준이며, 변별점은 컬럼 단위 계통이다. 본 절은 그 가운데 계통 기능만 떼어 비교한다.
...
데이터 계통(Lineage)은 구축 여부보다 실제 운영 과정에서 얼마나 지속적으로 활용되고 최신 상태를 유지하는지가 중요하다.

따라서 KPI는 단순 구축 완료 여부가 아니라 **계통의 연결성, 추적성, 운영성**을 지속적으로 관리하기 위한 기준으로 활용한다.

| 구분 | KPI | 정의 | 산식 | 활용 목적 |
|------|------|------|------|-----------|
| 구축 수준 | Lineage Coverage | 관리 대상 데이터 중 Lineage가 구축된 비율 | 구축 대상 자산 중 Lineage 연결 완료 비율 | 구축 범위 관리 |
| 구축 수준 | Column-level Coverage | 컬럼 단위까지 추적 가능한 데이터 비율 | Column Lineage 구축 데이터 / 전체 구축 데이터 | 영향도 분석 수준 관리 |
| 구축 수준 | AI Citation Coverage | AI 답변에 근거(Citation)가 연결된 비율 | Citation 포함 답변 / 전체 AI 답변 | AI 근거 추적 수준 관리 |
...
| 운영 수준 | 계통 최신성 | 실제 파이프라인 변경 후 Lineage 반영 수준 | SLA 내 갱신 건수 / 전체 변경 건수 | 최신성 관리 |
| 운영 품질 | Broken Lineage 비율 | 단절된 계통 비율 | 단절 Edge / 전체 Edge | 계통 품질 관리 |
| 운영 품질 | Lineage 수정 Lead Time | 오류 발견 후 수정 완료까지 소요시간 | 평균 수정시간 | 운영 효율성 |
| 운영 품질 | 계통 활용률 | 영향도 분석·감사·AI 근거 조회에 활용된 횟수 | 조회 건수 | 실제 활용도 관리 |
...
| Level | 기준 |
|---------|------|
| Level 1 | 주요 데이터만 수동 관리 |
| Level 2 | 핵심 데이터 자동 Lineage 구축 |
| Level 3 | 컬럼 단위 Lineage 구축 |
| Level 4 | AI Citation과 통합 관리 |
| Level 5 | 영향도 분석과 변경 관리가 자동 수행되는 수준 |

데이터 계통의 목표는 모든 데이터를 추적하는 것이 아니라, AI 활용 과정에서 필요한 데이터 흐름과 근거를 신뢰성 있게 관리하는 수준(Level 5)에 도달하는 것이다.
