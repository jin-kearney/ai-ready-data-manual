...
이 장은 데이터 계통이 무엇이고 무엇을 기록하는지를 정의한다. 구축 절차와 운영은 [4장](#how)에서 다루고, 여기서는 계통의 정의, 추적 대상 흐름, 기록하는 요소, 그것으로 답하는 질문을 정한다.
...
계통은 AI 결과를 신뢰하고 활용하게 만드는 신뢰(Trustworthy) 묶음에서, 데이터가 과거에 어떤 경로로 왔는지를 사후에 추적해 판정과 이상 감지에 근거를 대는 역할을 맡는다. 계통 자체는 판정하지 않는다.
...
정적 파싱은 실행 전에 코드를 분석하므로 배포 전 사전 영향분석에 강하고, 런타임 이벤트는 실제 실행 경로를 정확히 담는다. 현대 환경은 두 방식을 함께 써 설계 의도와 실제 실행을 모두 담고, 수기 선언으로 자동화가 닿지 않는 공백만 보완한다. 전수를 수기로 그리지 않는다. 관리 비용이 커지고 금세 실제와 어긋난다.
...
이 절은 계통이 실제로 잘 관리되고 있는지 숫자로 확인하는 KPI를 정의한다. 지표의 오너십·역할과 책임은 4장(운영)과 [F-1 데이터 운영관리](../F-1%20데이터%20운영관리/F-1%20데이터%20운영관리.md)에서, 데이터 보존·폐기 시간축은 [F-2 데이터 생애주기 관리](../F-2%20데이터%20생애주기%20관리/F-2%20데이터%20생애주기%20관리.md)에서 다룬다.
