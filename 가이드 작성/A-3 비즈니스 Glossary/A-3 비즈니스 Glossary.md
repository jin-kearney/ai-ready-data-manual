...
두산전자 품질보증팀이 **AI 결함 분석**을 도입했다. 그런데 "스크래치 불량 추이"를 물으면 데이터의 절반만 나온다. 원인은 현장이 같은 결함을 제각각 부른 것 — `긁힘·기스·scratch`, `빵꾸·홀·핀홀`, `휨·컬`. 문제의 원인은 데이터 부족이 아니라 동일한 결함을 서로 다른 용어로 기록하고 있다는 점이었다.
