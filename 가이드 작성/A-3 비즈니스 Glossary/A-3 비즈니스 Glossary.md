...
2. [What — 무엇을 갖추나 (용어 카드·동의어·유형)](#2-what--무엇을-갖추나-용어-카드동의어유형)
   - [2.1 비즈니스 Glossary란 + AI-ready Data 체계 내 위치](#21-비즈니스-glossary란--ai-ready-data-체계-내-위치)
   - [2.2 표준 용어 카드 — 표준 정의 항목](#22-표준-용어-카드--표준-정의-항목)
   - [2.3 동의어·금지어](#23-동의어금지어-현장-표현과-표준어-연결)
   - [2.4 연계 정보](#24-연계-정보-데이터-필드책임-부서)
   - [2.5 주요 관리 유형 (비즈니스 용어 / 지표 기준)](#25-주요-관리-유형-비즈니스-용어--지표-기준)

3. [When — 어디부터 표준화하나 (우선순위)](#3-when--어디부터-표준화하나-우선순위)
   - [3.1 표준화 대상 선정 기준](#31-표준화-대상-선정-기준)
   - [3.2 우선순위](#32-우선순위)

4. [예시 시나리오 — 결함명 표준화](#4-예시-시나리오--결함명-표준화-두산전자-품질)
   - [4.1 적용 배경](#41-적용-배경)
   - [4.2 적용 전 / 후](#42-적용-전--후)
   - [4.3 표준화 수행 과정](#43-표준화-수행-과정)

5. [Tech Stack — 솔루션 검토](#5-tech-stack--솔루션-검토)
   - [5.1 도구 유형](#51-도구-유형)
   - [5.2 선정 기준](#52-선정-기준)

6. [How — 어떻게 준비·운영하나](#6-how--어떻게-준비운영하나)
   - [6.1 표준 용어 정의 절차](#61-표준-용어-정의-절차)
   - [6.2 용어 충돌 조정](#62-용어-충돌-조정)
   - [6.3 표준 용어 저장소와 활용 체계](#63-표준-용어-저장소와-활용-체계)
   - [6.4 동의어 및 약어 관리](#64-동의어-및-약어-관리)
   - [6.5 용어 변경 관리](#65-용어-변경-관리)

7. [Where — 다른 주제와의 관계](#7-where--다른-주제와의-관계)

8. [별첨 (Appendix)](#별첨-appendix)
...
비즈니스 Glossary는 업무에서 쓰는 용어·약어·동의어를 하나의 표준 정의로 통일해, 사람과 AI가 같은 데이터를 같은 의미로 해석하게 하는 용어 사전이다. 이 가이드는 Glossary가 왜 필요한지(1장), 무엇을 갖추는지(2장), 어디부터 표준화할지(3장)를 설명하고, 결함명 표준화 사례(4장)로 전체 흐름을 그린 뒤 솔루션 선택(5장)과 구축·운영 방법(6장), 인접 주제와의 경계(7장)를 다룬다.
...
[A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md)가 데이터 필드와 컬럼의 의미를 설명한다면, 비즈니스 Glossary는 그보다 앞선 단계에서 조직이 사용하는 용어 자체의 의미를 표준화한다. 동일한 데이터를 쓰더라도 용어의 정의와 사용 방식이 조직마다 다르면 검색·분석·보고 과정에서 해석 차이가 생긴다. 제조 현장에서 자주 나타나는 어려움은 다음과 같다.
...
메타데이터가 데이터 자산을 해석할 수 있게 했다면, 비즈니스 Glossary는 조직 전체가 같은 용어로 소통하게 한다. 용어 때문에 못 찾던 데이터를 찾게 하고, 보고서마다 다르던 지표 숫자를 통일하며, 글로벌 현장을 하나의 기준으로 잇는다.
...
비즈니스 Glossary의 활용 가치는 표준명 자체보다 동의어와 금지어를 체계적으로 관리하여 현장에서 실제 사용하는 용어를 표준명과 연결함으로써 검색과 변환의 정확성을 높이는 데 있다.

- **동의어** — 같은 뜻의 다른 표현을 표준명에 묶어, 어떤 말로 물어도 찾을 수 있게 한다(예: 스크래치 ← 긁힘·기스·scratch).
- **금지어** — 혼동을 일으켜 쓰지 말아야 할 표현이다. 예를 들어 "매출액"은 주문·확정·회계 기준이 섞여 모호하므로 "확정 매출"을 쓴다.

### 2.4 연계 정보 (데이터 필드·책임 부서)

용어가 어느 데이터 필드에 쓰이고 누가 책임지는지를 연결해야, Glossary가 단순한 사전을 넘어 실제 데이터와 이어진다.

- **관련 데이터 필드** — 그 용어가 실제로 붙는 컬럼·테이블. [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md)의 비즈니스 메타데이터가 이 표준 용어를 인용한다.
- **책임 부서** — 정의의 정확성을 책임지는 조직으로, 충돌이나 변경이 생길 때 의사결정 주체가 된다.
...
## 4. 예시 시나리오 — 결함명 표준화 (두산전자 품질)

본 장에서는 두산전자 품질 조직의 결함명 표준화 사례를 통해 비즈니스 Glossary를 구축하고 운영하는 과정을 살펴본다. 동일한 결함이 현장과 시스템마다 다른 명칭으로 기록되는 상황을 가정하고, 용어 수집부터 표준화·운영까지의 전체 흐름을 따라간다.

### 4.1 적용 배경

두산전자 품질보증팀은 AI 기반 결함 분석 체계를 도입하는 과정에서 동일한 결함이 여러 명칭으로 기록되고 있음을 확인하였다. 예를 들어 `긁힘`, `기스`, `scratch`는 모두 같은 결함을 가리키지만 서로 다른 표현으로 저장되어 있었고, `빵꾸`, `홀`, `핀홀`, `휨`, `컬` 같은 표현도 혼재되어 있었다. 이로 인해 검색 결과가 누락되거나 분석 기준이 달라지는 문제가 생겨, 결함명을 표준화하는 작업이 필요해졌다.

### 4.2 적용 전 / 후

결함명을 표준화하면 데이터 활용이 다음과 같이 달라진다.
...
결함명 표준화는 용어 수집부터 운영까지 다음 여덟 단계로 진행한다.
...
| 단계 | 주요 활동 | 산출물 | 관련 절 |
|---|---|---|---|
| ① 용어 수집 | QMS 결함 코드, 작업일지, 품질 보고서에서 실제 사용 표현 수집 | 현장 용어 목록 | §6.1 |
| ② 용어 정리 및 합의 | 품질 SME가 동의어와 표준명 정의 | 표준명 후보 및 동의어 그룹 | §6.1 |
| ③ 용어 카드 작성 | 정의, 동의어, 금지어, 책임 부서, 관련 필드 등록 | 용어 카드 | §2.2, Appendix A |
| ④ 충돌 조정 | 의미가 중복되거나 불명확한 용어 정리 | 승인된 표준 정의 | §6.2 |
| ⑤ 등록 및 게시 | 표준 용어를 Glossary에 등록하고 검수 | 표준 용어집 | §6.3 |
| ⑥ 전사 공유 | 카탈로그, QMS, 검색 서비스에 반영 | 공통 용어 체계 적용 | §6.3 |
| ⑦ 검색 연계 | 동의어와 약어를 표준 용어에 연결 | 검색·AI 활용 체계 반영 | §6.4 |
| ⑧ 운영 및 유지보수 | 신규 용어 등록 및 변경 관리 | 최신 Glossary 유지 | §6.5 |
...
합의된 용어는 용어 카드 형식으로 정리하고 검토·승인 절차를 거쳐 표준 용어로 확정한다. 정의는 가능한 한 구체적으로 작성하며, KPI나 지표는 계산식과 적용 기준을 함께 명시한다.

| 항목 | Before | After | 개선 이유 |
|---|---|---|---|
| 수율 | 생산이 잘된 정도 | 양품 수량 ÷ 투입 수량 (재작업 제외, 월 마감 기준) | 계산 기준 명확화 |
| 리드타임 | 걸리는 시간 | 주문 확정일 ~ 납품 완료일 (영업 기준) | 적용 범위 명확화 |
| 부진재고 | 오래된 재고 | 최근 12개월 출고 이력이 없는 재고 | 판단 기준 명확화 |
...
**ii) 등록 및 게시** ① 소량은 UI에서 직접 입력, ② **대량은 엑셀/CSV로 모아 일괄 import**(엑셀로 작성 → 검수 → 업로드), ③ 자동화는 API로 등록. 업로드 후 검수·승인(§6.1 iii)을 거쳐 "게시"된다.
...
| 동의어 추가 | 표준명에 연결 | 검색 변환(§6.4)에 즉시 반영 |
