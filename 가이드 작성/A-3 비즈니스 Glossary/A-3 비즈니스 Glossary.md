...
통합 데이터 플랫폼에 내장된 카탈로그(예: **Databricks Unity Catalog**[\[4\]](#ref4))를 쓰면 카탈로그([A-1](../A-1%20데이터%20카탈로그/A-1%20데이터%20카탈로그.md))·메타데이터([A-2](../A-2%20메타데이터/A-2%20메타데이터.md))와 같은 곳에서 용어집을 함께 관리해 한 제품으로 묶을 수 있다([2층 정본의 통합 거버넌스 묶음](../../Tech%20Player/01%20Tech%20Stack%20비교%20(솔루션×주제).md) 참조). 다만 Unity Catalog의 네이티브 용어집(Glossary)·동의어 기능은 아직 Preview 단계이므로, 용어집·동의어 매핑이 핵심이면 전용 거버넌스(Collibra·Purview·Atlan)를 함께 검토하고 PoC로 확인한다.
...
| 2026-06-29 | v0.19 | §5.1 'Databricks Unity Catalog 1순위 후보' 표현 삭제(고객 요청, A-1과 정합) — 특정 솔루션 랭킹 대신 '통합 플랫폼 내장 카탈로그를 쓰면 한 제품으로 묶을 수 있다'로 중립화. Preview 단계 caveat·전용 거버넌스 병행 안내는 유지. | §5.1 |
