...
| 4 | Glossary를 **AI 활용 체계**와 어떻게 잇나? | 현장 용어를 표준 용어로 변환해 카탈로그·메타·RAG·Agent 검색에 연결 | [§7.3](#sec73) |
| 5 | **용어 변경**을 어떻게 관리하나? | 신규·변경·폐기·동의어 추가를 버전·승인으로 관리하고 변경 영향을 점검 | [§7.4](#sec74) |
...
**👉 한 줄 요약:** 동의어를 표준명에 묶기 전엔 "기스"로 기록된 결함이 검색에서 빠졌지만, 묶은 뒤엔 어느 표현으로 물어도 찾는다.

| | 적용 전 | 적용 후 |
|---|---|---|
| "스크래치 불량" 검색 | "기스"로 기록된 데이터 누락 | 동의어 묶여 전부 검색 |
| "리드타임 평균" | 영업·생산 정의 혼재 → 무의미 | 구분 정의로 정확한 집계 |
| "수율" 보고 | 팀마다 다른 숫자 | 표준 계산식으로 일치 |
| 글로벌 분석 | 각국 용어 장벽 | 표준명 매핑으로 통합 |

### 5.2 흐름 미리보기

**👉 한 줄 요약:** 용어 하나가 표준으로 자리잡기까지의 흐름.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','clusterBkg':'#EEF4FB','clusterBorder':'#6B9AD1','fontSize':'12px'}}}%%
flowchart LR
    C["① 대상 용어 추리기<br/>현장·문서서 실제 표현 수집"]:::src
    D["② 표준명·동의어·금지어 정의<br/>SME 초안"]:::n
    E["③ 충돌 조정·승인<br/>SME·오너·중앙조직"]:::loop
    F["④ AI 검색에 연결<br/>현장어→표준어 변환"]:::cat
    C --> D --> E --> F
    E -.재논의.-> D
...
### 7.1 용어 수집·정의 — 잘 쓴 정의 vs 못 쓴 정의 (현업 실행 키트 ㉡)

**👉 한 줄 요약:** 현장·문서에서 실제 표현을 모아 표준명·동의어·금지어로 정리하되, **정의는 측정 가능·명확하게** 쓴다.
...
**👉 한 줄 요약:** Glossary는 **현장 용어를 표준 용어로 변환**해 카탈로그·메타·RAG·Agent 검색에 연결된다 — 이것이 Glossary가 "사전"을 넘어 AI 인프라가 되는 지점.

- **카탈로그·메타 연계:** [A-1 카탈로그](../A-1%20데이터%20카탈로그/A-1%20데이터%20카탈로그.md) 검색과 [A-2 메타데이터](../A-2%20메타데이터/A-2%20메타데이터.md) 비즈 메타가 Glossary 표준 용어를 인용한다(컬럼 설명에 표준 용어 우선 사용).
- **현장어→표준어 변환:** 사용자가 "기스 불량"으로 물으면, AI가 동의어 사전을 거쳐 "스크래치"로 변환해 검색한다.
- **RAG·Agent 기준어:** AI Agent가 질의·답변에서 Glossary 정의를 "판단 기준"으로 삼아, 같은 용어를 같은 의미로 처리한다.

> **용어 풀이 — RAG:** AI가 답하기 전에 사내 데이터를 먼저 검색해 근거로 쓰는 방식. Glossary가 검색어를 표준화하면 RAG가 더 정확히 데이터를 찾는다.
...
| 동의어 추가 | 표준명에 연결 | 검색 변환에 반영 |

**역할(RACI 요약):** 용어 제안·정의 = 현업 SME **R**, 표준 확정·충돌 조정 = 중앙 스튜워드 **R/A**, 계열사 정책 = 거버넌스 **A**, 검색 연계 = IT **R**.
...
| 일자 | 버전 | 변경 내용 | 반영 위치 |
|------|------|-----------|-----------|
| 2026-06-19 | v0.1 | 초안 작성 — 전체 목차 A-3 9섹션 골격 위에 두산 엑셀 Glossary(3.5.1 개념·2유형, 도메인 용어 예시집) 흡수. 「현업 실행 키트」 5장치 적용(㉠ §3.1 용어 카드, ㉡ §7.1 Before→After, ㉢ [Appendix C], ㉣ [Appendix A] 빈템플릿+완성예시, [Appendix B] 도메인 용어집). A-2 메타데이터·B-3 온톨로지와 경계 명시. 웹 리서치 없이 기존 스토리라인 기반 + 엑셀 보완. KQ 5개 전부 커버(1→§4, 2→§3.1, 3→§7.2, 4→§7.3, 5→§7.4) | 전체 |
