...
1. [개요](#1-개요)
   - [1.1 비즈니스 Glossary란](#11-비즈니스-glossary란)
   - [1.2 적용 범위와 체계 내 위치](#12-적용-범위와-체계-내-위치-용어의-뜻)
   - [1.3 주요 대상 조직](#13-주요-대상-조직)
...
3. [무엇을 갖추나 (What — 용어 카드·동의어·유형)](#3-무엇을-갖추나-what--용어-카드동의어유형)
   - [3.1 표준 용어 카드 — 표준 정의 항목](#31-표준-용어-카드--표준-정의-항목)
   - [3.2 동의어·금지어](#32-동의어금지어-현장-표현을-표준어로-잇기)
   - [3.3 연계 정보](#33-연계-정보-데이터-필드책임-부서)
   - [3.4 두 가지 유형 — 비즈니스 용어 / 지표 기준](#34-두-가지-유형--비즈니스-용어--지표-기준)

4. [어디부터 표준화하나 (우선순위)](#4-어디부터-표준화하나-우선순위)
   - [4.1 표준화 대상 용어](#41-표준화-대상-용어)
   - [4.2 우선순위](#42-우선순위)
   - [4.3 표준화 대상 선정 기준](#43-표준화-대상-선정-기준)

5. [예시 시나리오 — 결함명 표준화](#5-예시-시나리오--결함명-표준화-두산전자-품질)
   - [5.1 적용 배경](#51-적용-배경)
   - [5.2 적용 전 / 후](#52-적용-전--후)
   - [5.3 표준화 수행 과정](#53-표준화-수행-과정)
...
7. [어떻게 준비·운영하나 (How)](#7-어떻게-준비운영하나-how)
   - [7.1 표준 용어 정의 절차](#71-표준-용어-정의-절차)
   - [7.2 용어 충돌 조정](#72-용어-충돌-조정)
   - [7.3 표준 용어 저장소와 활용 체계](#73-표준-용어-저장소와-활용-체계)
   - [7.4 동의어 및 약어 관리](#74-동의어-및-약어-관리)
   - [7.5 용어 변경 관리](#75-용어-변경-관리)

8. [다른 주제와의 관계](#8-다른-주제와의-관계)

9. [성과 지표·고도화 로드맵](#9-성과-지표고도화-로드맵)
   - [9.1 성과 지표 (KPI)](#91-성과-지표-kpi)
   - [9.2 고도화 로드맵](#92-고도화-로드맵--우리-수준과-다음-한-걸음-성숙도-단계)
   - [9.3 미래 AI 자동화 전망](#93-미래-ai-자동화-전망--어디는-ai가-어디는-사람이)

10. [별첨 (Appendix)](#별첨-appendix)
...
AI Agent가 질의·답변에서 동일 용어를 동일 의미로 쓴다. Glossary는 AI와 사용자가 동일한 기준으로 용어를 해석할 수 있도록 지원한다.
...
비즈니스 Glossary는 새로운 용어를 만드는 작업이 아니라, 현업에서 이미 사용하고 있는 용어를 수집하고 표준 기준으로 정리하는 과정이다.

따라서 용어 표준화는 현장 표현을 수집하고, 업무 전문가가 의미를 검토한 뒤, 조직 차원의 표준 용어로 확정하는 순서로 진행한다.

#### i) 용어 수집

표준 용어는 현업에서 실제 사용하는 표현을 기반으로 정의한다.

MES, QMS와 같은 업무 시스템의 코드값뿐 아니라 작업일지, 품질 보고서, 대시보드, 회의 자료 등에서 사용되는 용어를 함께 수집한다.
...
수집된 용어는 도메인별 SME와 함께 검토한다.

이 과정에서 동일한 의미를 가진 용어는 하나의 표준명으로 통합하고, 동일한 용어가 여러 의미로 사용되는 경우에는 구분 기준을 정의한다.

합의 결과는 표준명, 동의어, 정의 초안 형태로 정리한다.

#### iii) 표준 정의 확정

합의된 용어는 용어 카드 형식으로 정리하고 검토·승인 절차를 거쳐 표준 용어로 확정한다.

정의는 가능한 한 구체적으로 작성하며, KPI나 지표의 경우 계산식과 적용 기준을 함께 명시한다.
...
**i) 표준 용어 저장소** 표준 용어는 단일 저장소를 기준으로 관리한다. 부서마다 엑셀을 따로 두면 표준화의 의미가 없다.
...
**ii) 등록 및 게시** ① 소량은 UI에서 직접 입력, ② **대량은 엑셀/CSV로 모아 일괄 import**(현업이 익숙한 방식 — 엑셀로 작성 → 검수 → 업로드), ③ 자동화는 API로 등록. 업로드 후 검수·승인(§7.1 iii)을 거쳐 "게시"된다.

**iii) 활용 채널 연계** 정본 하나를 등록하면 아래 채널에 자동 노출되게 연결한다.
...
현업에서는 동일한 개념을 다양한 표현으로 사용하는 경우가 많다.
비즈니스 Glossary는 이러한 표현을 표준 용어와 연결하여 검색, 분석, AI 활용 과정에서 동일한 의미로 해석할 수 있도록 지원한다.
...
| 단계 | 설명 |
|---|---|
| 동의어 등록 | 현장에서 사용하는 표현을 표준 용어에 연결 |
| 검색어 변환 | 입력한 표현을 표준 용어 기준으로 변환 |
| 데이터 검색 | 표준 용어를 기준으로 관련 데이터 조회 |
| 결과 제공 | 동일 기준으로 검색 결과 제공 |
