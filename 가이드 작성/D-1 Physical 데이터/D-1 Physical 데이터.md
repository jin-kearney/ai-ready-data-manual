...
### 2.5 4D 데이터 — 공간과 시간을 함께 담기

앞의 설명은 대부분 한 지점의 값이 시간에 따라 쌓이는 스칼라 시계열을 전제한다(온도 85℃, 진동 2.1mm/s). 그러나 AI가 물리세계를 인지하는 범위가 넓어지면서, 물리 데이터는 3차원 공간과 시간을 함께 담는 4D 데이터로 확장된다. 대상이 공간에서 어떻게 놓여 있고 시간에 따라 어떻게 움직이는지를 담아야, AI가 값뿐 아니라 형상·위치·동작까지 이해할 수 있다.

> 4D 데이터: 3차원 공간(3D)에 시간(1D)을 더한 것. 한 점의 값이 아니라, 공간 위의 형상·위치가 시간에 따라 변하는 것을 담는다.

제조 현장에서 4D 데이터에 해당하는 예는 다음과 같다.

| 유형 | 예시 | 스칼라 시계열과 다른 점 |
|---|---|---|
| 3D 측정·스캔 | 3D 치수 검사, 3D 스캐너·LiDAR 점군(point cloud) | 한 값이 아니라 형상 전체를 좌표로 |
| 영상·depth | 외관 검사 카메라, depth 센서 | 값이 공간(픽셀·깊이)에 퍼져 있음 |
| 설비·로봇 동작 | 로봇 팔 궤적, 가공 경로 | 3D 위치·자세가 시간에 따라 이동 |
| 공간 매핑 신호 | 설비 3D 형상에 얹은 온도·진동 분포 | 값이 "어느 지점의 것"인지까지 |

4D 데이터를 AI 입력으로 준비하는 원칙은 스칼라 시계열과 같되, 공간 차원이 하나 더해진다. [2.3](#kq3)의 표준 3종(시간·단위·설비ID)에 3D 좌표계와 기준점(어느 원점 기준의 좌표인가)을 더해 통일하고, 스칼라 센서값을 그 값이 발생한 3D 위치·시간에 연결한다. 저장도 스칼라 시계열 저장소만으로는 부족해, 점군·영상 같은 대용량 공간 데이터를 함께 다룰 수 있는 형태를 검토한다([6.2](#s52)).

> 모든 설비에 4D 데이터가 필요하지는 않다. 온도·진동 감시처럼 한 지점 값으로 충분하면 스칼라 시계열로 두고, 형상·위치·동작이 중요한 경우(3D 치수 검사, 로봇 동작, 외관 검사 영상)에만 4D로 넓힌다. 넓히더라도 실데이터를 표준화·정제해 AI가 읽을 수 있게 준비한다는 원칙은 그대로다.
