...
2. [What — 무엇인가·무엇을 갖추나](#what)
    ...
    - [2.6 여러 센서를 융합하는 경우 — 자율주행·자율 이동장비](#s26)
...
AI는 소프트웨어를 넘어 물리세계로 들어오고 있다. 센서·카메라로 현장을 인지하고 스스로 판단해 행동하는 이 흐름을 Physical AI라 부르며, 제조 현장에서는 정해진 규칙만 따르던 로봇을 넘어 상황을 스스로 판단해 공정에 투입되는 로봇·휴머노이드로 나타난다. 이때 AI가 세상을 인지하려면 센서 신호를 기계가 읽을 수 있는(machine-readable) 형태로 먼저 만들어야 한다.

이 전제는 활용 유형과 무관하게 같다. 본 가이드가 다루는 이상 감지·예지보전·품질 분석형 AI든, 물리세계에서 인지·행동하는 Physical AI든, 출발선은 현장 신호를 시간·단위·설비ID로 표준화하고 1차 정제해 AI가 바로 읽을 수 있게 준비하는 데 있다. D-1은 그 데이터 토대를 다룬다. 로봇·모델 자체를 만드는 일은 범위 밖이며, 여기서는 그 AI가 쓸 물리 데이터를 준비한다.

> 자율주행이 희소한 주행 상황을 합성·시뮬레이션 데이터로 대량 학습하듯, 제조에서도 고장·이상처럼 드물어 실데이터가 부족한 상황은 합성으로 보완한다(→ [E-2 합성데이터](../E-2%20합성데이터/E-2%20합성데이터.md)). 실데이터 확보가 먼저이고, 합성은 그 빈 곳을 채운다.
...
이 밖에 같은 시각에 값이 두 번 찍히는 중복, 나중 값이 더 이른 시각을 갖는 시간 역전도 정렬 전에 정리한다. Historian이 저장 공간을 아끼려 변화가 있을 때만 기록하는 압축·데드밴드 때문에 시각 간격이 불규칙해지는 문제는 [5.2](#s62)에서 다룬다. 다만 여기까지는 신호를 다룰 수 있게 만드는 기초 정제이며, 이 배치를 최종적으로 써도 되는가의 판정은 [C-2 데이터 품질 관리](../C-2%20데이터%20품질%20관리/C-2%20데이터%20품질%20관리.md)가 맡는다.

<a id="s25"></a>

### 2.5 3D·4D 데이터 — 형상과 물리 거동까지 담기

앞의 설명은 대부분 한 지점의 값이 시간에 따라 쌓이는 스칼라 시계열을 전제한다(온도 85℃, 진동 2.1mm/s). 그러나 AI가 물리세계를 인지하고 모사하는 범위가 넓어지면서, 물리 데이터는 형상과 물리 거동까지 담는 3D·4D 데이터로 확장된다.

- 3D 데이터는 설비·제품의 형상과 배치를 3차원으로 담는다. 센서값을 3D 형상 위에 얹은 디지털 트윈(Digital Twin), 3D 치수 검사, 점군(point cloud)·영상이 여기 해당한다. 어디에 무엇이 있는지는 담기지만, 그 자체로는 정적이다.
...
이런 시뮬레이션·모델 자체(물리 현상을 미리 학습한 생성형 시뮬레이션 등)를 만드는 일은 D-1의 범위가 아니다. D-1은 그 모델이 학습하고 검증에 쓸 물리 데이터를 준비한다. 준비 관점에서 스칼라 시계열과 달라지는 점은 다음과 같다.

| 데이터 | 담는 것 | 예시 |
|---|---|---|
| 스칼라 시계열 | 한 지점의 값 × 시간 | 온도·진동·압력 |
| 3D | 형상·배치·센서 위치 | 디지털 트윈, 3D 치수·점군, 외관 검사 영상 |
| 4D | 3D + 물리적 특성·시간 거동 | 하중·힘, 재료 물성, 시간에 따른 변형·진동·궤적 |

4D 데이터를 AI 입력으로 준비하는 원칙은 스칼라 시계열과 같되, 공간·물리 차원이 더해진다. [2.3](#kq3)의 표준 3종(시간·단위·설비ID)에 3D 좌표계와 기준점(어느 원점 기준의 좌표인가)을 더해 통일하고, 센서값·힘·물성을 그 값이 발생한 3D 위치·시간에 연결한다. 저장도 점군·영상·시뮬레이션 결과 같은 대용량 공간 데이터를 함께 다룰 형태를 검토한다([6.2](#s52)).

> 모든 설비에 3D·4D 데이터가 필요하지는 않다. 온도·진동 감시처럼 한 지점 값으로 충분하면 스칼라 시계열로 두고, 형상·힘·거동이 중요한 경우(3D 치수 검사, 디지털 트윈, 물리 기반 예측)에만 넓힌다. 넓히더라도 실데이터를 표준화·정제해 AI가 읽을 수 있게 준비한다는 원칙은 그대로다.

<a id="s26"></a>

### 2.6 여러 센서를 융합하는 경우 — 자율주행·자율 이동장비

한 대상을 여러 종류의 센서로 동시에 보고, 센서가 실린 플랫폼 자체가 움직이는 경우가 있다. 자율주행 차량이 대표적이고, 제조 현장에서는 자율주행 건설장비·물류 로봇(AGV)·자율 지게차가 같은 형태다. 이때 물리 데이터 준비의 어려움은 센서 하나가 아니라 여러 센서를 하나의 장면으로 합치는 데 있다.

LiDAR(레이저로 주변을 3D로 스캔하는 센서)는 3D 점군, 카메라는 영상, radar는 거리·속도, GPS·IMU(위치·자세 센서)는 위치·자세를 각각 다른 주기로 쏟아낸다. 이를 AI가 쓸 하나의 장면으로 합치려면 세 가지를 맞춰야 한다.

- 시간 동기화: 여러 센서 값을 같은 순간 기준으로 맞춰야 한 장면으로 합쳐진다. 수십 ms만 어긋나도 물체 위치가 틀어진다([2.3](#kq3)의 시간 표준).
...
이렇게 정합한 결과가 [2.5](#s25)의 4D 장면 — 3D가 시간에 따라 이어진 것 — 이 되어 인식·판단의 입력이 된다. 실제로 겪기 드문 위험 상황(급끼어들기·악천후·돌발 장애물)은 실데이터가 부족하므로 시뮬레이션·합성으로 보강한다([7장](#where)의 E-2).

여기서도 D-1의 몫은 자율주행 인식·판단 모델을 만드는 것이 아니라, 그 모델이 학습·검증에 쓸 다중 센서 데이터를 시간·공간으로 정합하고 정제해 준비하는 데까지다.
...
| [E-2 합성데이터](../E-2%20합성데이터/E-2%20합성데이터.md) | 고장·이상처럼 드물어 실데이터가 부족한 상황은 E-2가 합성·시뮬레이션으로 보완한다. D-1은 실데이터 수집·표준화가 먼저 |
