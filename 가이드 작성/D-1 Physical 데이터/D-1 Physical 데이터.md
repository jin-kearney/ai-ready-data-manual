...
6. [How — 어떻게 준비·운영하나](#how)
    - [6.1 대상 태그 정의와 수집 연결](#s61)
    - [6.2 표준화와 기초 정제](#s62)
    - [6.3 초기 적재와 검증](#s63)
    - [6.4 운영 — 태그 추가·설비 교체](#s64)
    - [6.5 운영 — 정제 기준·실시간 지연·역할](#s65)
...
이 장은 Physical 데이터가 무엇이고 무엇으로 이루어지는지를 설명한다. 수집 연결과 표준화·정제의 구체적 절차는 [6장](#how)에서 다루고, 여기서는 정의와 범위, 수집 대상, 갖춰야 할 표준과 정제 기준을 정의한다.
...
어느 설비·태그부터 수집할지의 우선순위는 [3장](#when)에서, 각 태그에 무엇을 기록할지의 항목은 [6.1](#s61)과 [별첨 A](#ap-a)에서 다룬다.
...
표준화와 정제가 실제로 무엇을 바꾸는지를 가공·회전설비 예지보전 사례로 그린다. 이 사례는 [6장](#how)의 구축 절차까지 하나의 흐름으로 이어진다.
...
이 다섯 단계는 [2.1](#s21)의 정본 흐름(수집·연결 → 표준화 → 기초 정제 → 적재)에 핵심 태그 선정을 앞에 둔 것이다. 각 단계의 구체적 수행은 6장에서 같은 사례로 이어진다.
...
> **2층 연결:** 솔루션을 묶어서 평가·선정하려면 → [Tech Stack 비교 정본](../../전체%20목차/01%20Tech%20Stack%20비교%20(솔루션×주제).md). 이 절은 Physical 데이터 관점에서 연결 방식과 저장 솔루션의 기능을 비교한다.
...
태그 하나에 기록하는 전체 항목이다. 본문 [6.1](#s61)에는 대표 항목만 두었고, 여기서 전체를 정리한다. 실제 운영에서는 외부 마스터시트로 관리할 수 있다.
