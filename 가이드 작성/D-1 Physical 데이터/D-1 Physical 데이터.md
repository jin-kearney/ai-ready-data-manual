...
> 실시간 경로는 과도한 전처리가 지연을 부르므로, 에지에서 스파이크 제거·단위 변환 같은 최소 정제만 하고 즉시 판단한다. 배치 경로는 시간 여유가 있으므로 결측 보간·리샘플링·중복 제거까지 정밀하게 처리한다. 둘을 한 경로로 합치지 않는다.
...
| 2026-07-02 | 0.10 | 고객: '묶다' 표현 지양 | §3.2 "둘을 한 경로로 묶지 않는다"→"…합치지 않는다" | 3.2 |
