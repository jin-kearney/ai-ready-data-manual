...
AI는 소프트웨어를 넘어 물리세계로 들어오고 있다. 센서·카메라로 현장을 인지하고 스스로 판단해 행동하는 이 흐름을 Physical AI라 부르며, 제조 현장에서는 정해진 규칙만 따르던 로봇을 넘어 상황을 스스로 판단해 공정에 투입되는 로봇·휴머노이드로 나타난다([Kearney 인사이트](https://kearneyblog.co.kr/child/sub/insights/view.php?seq=177)). 이때 AI가 세상을 인지하려면 센서 신호를 기계가 읽을 수 있는(machine-readable) 형태로 먼저 만들어야 한다.
...
- [Kearney — Physical AI 시대의 도래와 대비 방안](https://kearneyblog.co.kr/child/sub/insights/view.php?seq=177) — 컨설팅 인사이트, Physical AI 개념(인지·생각·행동)과 제조 적용 (2026-07-02)
...
| 2026-07-02 | 0.2 | 고객: Kearney Physical AI 인사이트 반영 요청 | Physical AI 맥락으로 Why 보강(어떤 AI든 machine-readable·표준화가 공통 전제), 희소 상황 합성데이터 보완을 경계에 추가. 데이터 준비 관점 유지(로봇·모델 구축은 범위 밖) | 1.3, 7장, 참고자료 |
