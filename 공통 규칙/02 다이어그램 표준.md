...
**H. 성숙도 단계·시간축 (`flowchart` 단계 사다리 / `gantt`)**
- 고도화 로드맵 섹션은 **(2026-06 결정) 현재 전 주제에서 두지 않는다** — 아래는 되살릴 때의 작성 기준이다(평소 사용 안 함).
- 고도화 로드맵은 **성숙도 단계 사다리**가 기본 — Lv0 없음 → Lv1 수기 표준화 → Lv2 도구·AI 보조 → Lv3 자율(좌→우 flowchart 또는 표). 각 단계에 「진단 신호」와 「다음 한 걸음」을 단다.
- `gantt`(달력 일정)는 **실제 합의된 착수일·기간이 있을 때만** — 예: 구축 Wave 일정(핵심 자산 → 범위 확대 → AI 연계). 임의 날짜로 채우지 않는다.
...
### 5-4. Roadmap — 성숙도 단계 사다리 (flowchart) — 되살릴 때만

> 로드맵 섹션은 현재 전 주제에서 두지 않는다(2026-06 결정). 아래는 추후 되살릴 경우의 작성 기준이다.

날짜가 아니라 *수준*으로 그린다. 각 단계 노드에 「이런 모습이면 이 수준」을, 화살표 라벨에 「다음 한 걸음」을 단다. 색은 단계가 오를수록 진해지는 파랑 계열로.

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#DCE8F5','primaryBorderColor':'#2456A4','primaryTextColor':'#10243f','lineColor':'#2456A4','fontSize':'13px'}}}%%
flowchart LR
    L0["Lv0 없음/임의<br/><small>표준 없이 개인별 제각각</small>"]:::l0
    L1["Lv1 수기 표준화<br/><small>양식·기준 합의, 사람이 채움</small>"]:::l1
    L2["Lv2 도구·AI 보조<br/><small>자동 추출·추천, 사람이 검수</small>"]:::l2
    L3["Lv3 자율·지속<br/><small>대부분 자동, 사람은 예외만</small>"]:::l3
    L0 -->|"양식·기준부터 정한다"| L1
    L1 -->|"도구 도입·AI 1차 자동화"| L2
    L2 -->|"검수 룰화·지속 운영"| L3
    classDef l0 fill:#EEF3FA,stroke:#2456A4,color:#10243f;
...
> 단계는 주제에 맞게 3~4개로. 표(`| 단계 | 진단 신호 | 다음 한 걸음 |`)로 대체해도 좋다 — 단계가 많고 설명이 길면 표가 더 빠르다.

### 5-4b. 구축 일정 (gantt) — 실제 일정이 있을 때만

성숙도가 아니라 **합의된 착수일·기간**을 보일 때만 쓴다. 임의 날짜로 채우지 않는다.
...
gantt
    ...
    적용 대상 확장        :a2, after a1, 4M
    section Wave 3 AI 연계
    자동화·서비스 연계    :a3, after a2, 5M
```
